...
- Domestic water and sanitary/storm sizing requests can generate proposal-only `resize_pipe` write-plan steps when the caller supplies exact pipe identity plus demand basis and the required office sizing standards. These handoffs stay `canCommit: false` and still require slope/invert/vent/code review before any commit.
...
- `domesticWaterPipeSizingRequests` and `sanitaryStormPipeSizingRequests` let domestic, sanitary, and storm foundations produce proposal-only `resize_pipe` steps from supplied demand requests without mutating Revit.
