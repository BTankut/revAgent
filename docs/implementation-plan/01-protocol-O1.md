...
| P-O1-14 | **WP12-C39 route rebind is capability-gated inside existing `session_resume`.** `route_rebind_proof_v1` binds fresh canonical document context, cache incarnation/revision, and current `hello_ack.connection_id`; Gateway commits discriminated route provenance, immutable proof receipt, normal ACK reconciliation, and `resume_ack` lease in one existing resume CAS. | Resolves the reserved-terminal C39 route/sequence deadlock without adding a message, consuming data sequence, copying stale routes, or relaxing current-route/principal/MCP/binding checks. |
