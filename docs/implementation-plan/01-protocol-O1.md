...
| O1-T8 | **Ratification & conditional freeze:** fold harness findings into spec/schemas, publish the protocol constant, produce `docs/plan/M1_O1_FREEZE_EVIDENCE.md`, and tag `rbp/v1.0.0` only after every spec §22 M1 item is linked and green. | O1-T6 | The executable candidate carries canonical `1.0` bytes while the freeze verdict remains pending; only the validated protected candidate commit is tagged, and the later evidence-record PR does not alter it. Later semantic change follows P-O1-9; MASTER_PLAN links the retained report. Real add-in and WP9 hands-on proof remain pilot gates, not M1 tag prerequisites. | 1 |
