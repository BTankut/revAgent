...
**Package-area labels used below** (coordinator: map to your P-numbering by area name if labels differ): **P1** Phase-1 Gateway core (north MCP, tool registry/policy, capability index + deferred schemas, Mode-B stubs, executor dispatch, RBP ingress; the in-house orchestration/LLM/context loop activates later per RES-23/29); **P2** Bridge + O1 protocol + add-in adaptation; **P3** Phase-1 immutable handler-module packaging from the unchanged Node/TS runtime plus docs MCP internalization (source relocation waits for Retire/freeze lift); **P4** Data layer + auth/licensing/audit (Postgres/O3, OIDC, device tokens, seats, O7 event schema); **P5** Phase-1 infra (office host, Compose, tunnel, warm standby, O10 drill); **P6** Installer/uninstaller/cutover tooling + CI/CD extension + bridge self-update signing (O9); **P7** Admin-plane migration (O11) + Phase-1 client scope (O8).
...
| M2 | External-client Gateway core: north MCP serves a capability index, `tool_search`/`tool_schema`, and a pinned callable set; immutable runtime/docs handler modules load without frozen-source relocation; Mode-B interfaces remain non-executable stubs; registry/policy/confirmation, bridge/internal executor dispatch, production RBP ingress, dedicated Gateway CI, and P6-T1 seam review pass on deterministic auth/device fixtures (P1 + P3) | M1 | 38 |
| M3 | Bridge skeleton: enrollment (bridge side, stub-proven — per RES-30 the real-Gateway exchange/revoke/reboot evidence is an M4 entry criterion), persistent WSS, invocation → add-in TCP framing, idempotency journal, batch-as-transaction-group (P2) | M1 | 15 |
| M4 | **Pre-production-auth vertical slice**: external MCP client (WP9 candidate) → gateway on office host → bridge → live Revit → result, incl. originating-preview-linked `confirm` flow; this is not DP-10 OAuth/hands-on acceptance (P1+P2+P5 minimal) | M2, M3 | 5 |
...
**Critical path:** M0 → M1 (O1 spec) → M2 ∥ M3 (single dev: serial, M2 then M3) → M4 (external-client vertical slice) → M6 (installer + self-update — pilot prerequisites) → M8 (pilot) → M9 → M10. **O1 is the first deliverable and the precondition for the pilot, exactly as the architecture states (O1: "First deliverable; precondition for the pilot").** M5 and M7 sit just off the critical path but gate M8 via entry criteria. WP2's reviewed 38-day executable M2 core supersedes the former 20-day placeholder; the rollup below includes that 18-day delta.

**Parallelizable lanes (single dev = backfill lanes during wait states, mainly pilot observation days and insurance window):** P7 dashboard/usage-intelligence reimplementation as event-stream consumers (~10 dd; only the parity *check* is pre-cutover); Phase-1 P1 external-client/registry/dispatcher hardening (including result/resource hygiene); the in-house context projection/planner/router, O2 code-exec sandbox implementation, O4 APS, and web client remain later activation/post-Retire work per RES-23/29 and P-SEQ-3.

### Calendar rollup

Updated critical-path effort is approximately **108 dev-days**; the full-program rollup is approximately
**138–153 dev-days** after replacing the old 20-day M2 placeholder with WP2's 38-day executable core. For one
senior developer plus an AI assistant, plan cutover around **23–24 working weeks** from kickoff and, with the
20% Build contingency plus the fixed pilot/insurance periods, approximately **6.5–7 months** to M10/NAS
retirement. The in-house-loop backlog remains outside Phase 1 per RES-23/29 but stays in the full-program total.
Pilot (at least five real working days) and insurance (two fixed weeks) are backfill-lane time, not idle time.
...
| R13 | **The Windows self-hosted runner is an interactive, session-bound listener rather than a Windows service.** Logout, sleep, or loss of the desktop session can terminate the listener while GitHub temporarily continues to report the runner as online; this is the likely cause of the observed 68-minute `runner_id=0` queue stall | M | H | Interim: keep the runner session signed in and awake, and treat an online-but-unassigned runner as a listener-liveness fault. Durable mitigation is a separately authorized operator task to install the runner with `svc.cmd install` under an explicitly reviewed service identity and ACL/secret boundary; this migration record does not authorize that host change | A matching self-hosted job remains at `runner_id=0` for more than 5 minutes while the runner API says online; listener PID is absent or no worker is assigned after logout/sleep |
...
- **Existing dirs untouched:** `installer/runtime-mcp-server/` stays in place with its own lockfile through Build; Phase 1 packages hash-bound handler modules without moving its source, and source relocation waits for Retire/freeze lift. Dependency-only manifest/lock cleanup is separately test-gated; `src/revit-plugin/`, `installer/`, `addons/`, `evals/`, `config/`, root `AGENTS.md`/`SKILL.md` remain outside the scaffold change.
