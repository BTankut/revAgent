...
- Added optional domain placement proposal handoff through `analyze_mep_system.placementRequests`: air-terminal, damper, valve, pump, fire-cabinet, or equipment requests are converted to proposal-only native `place_family_instance` steps with preview/approval/verify preconditions. Runtime stdio probe listed 13 tools and returned a valid top-level proposal containing placement steps.
