...
- Added native reroute fitting creation and expectation gates: `createRouteFittings` / `preferRouteFittings` attempt `Document.Create.NewElbowFitting` between adjacent route segments, while `requireRouteFittings` / `expectedRouteFittingCount` make unique fitting owners part of commit/verify success instead of a readback-only diagnostic.
...
- Added rational-method storm runoff and storm pipe sizing proposal foundations; rainfall/runoff/capacity standards are explicit inputs and outputs stay `canCommit: false`.
...
- Added top-level `writePlanProposal` output to `analyze_mep_system`; it aggregates proposal-only domain `writePlanSteps` such as HVAC `resize_duct`, hydronic `resize_pipe`, and equipment schedule-note steps into a normalized write-plan object for prepare/preview review while keeping `canCommit: false`.
- Added optional domain placement proposal handoff through `analyze_mep_system.placementRequests`: air-terminal, damper, valve, pump, fire-cabinet, or equipment requests are routed by discipline and converted to proposal-only native `place_family_instance` steps with preview/approval/verify preconditions. Runtime stdio probes listed 13 tools, verified discipline filtering, and returned valid top-level proposals containing placement steps.
- Tightened office-standard completeness gating so production-final calculations explicitly require HVAC duct velocity slots, hydronic pipe velocity slots, domestic water pressure-loss method and fixture-unit standard, sanitary stack/vent node sets, and simultaneous fire cabinet count. Runtime probe confirms the expanded missing-standard sets are visible without mutating Revit.
- Added `dataCompleteness` to HVAC duct and hydronic pipe resize proposals so production-review readiness reports sample/target/row/step counts, skipped no-flow/no-size counts, local-loss completeness, and explicit blocker messages. Runtime probe confirms complete/no-blocker summaries with complete HVAC/hydronic standards.
- Re-ran a user-approved disposable live commit sanity test after the proposal aggregation change: source duct `392168` stayed unchanged, copied duct `1021032` received `Comments = Codex disposable live commit 2026-05-05T00:00:00Z`, verify matched, cleanup deleted `1021032`, and final counts returned to `744` ducts / `488` pipes.
...
- Added runtime/plugin validation for `expectedRouteFittingCount`; plugin builds with `route-fitting-gate` and `route-fitting-create` outputs passed. Direct-loaded live test `route-fitting-create-live-20260505` created two route ducts plus elbow fitting `1020909`, returned `createdRouteFittingCount: 1`, `routeFittingCount: 1`, `routeFittingRefCount: 2`, cleaned up all three created elements, and final counts returned to `728` ducts / `936` duct fittings.
- Direct-loaded pipe fitting live test `pipe-fitting-create-live-20260505` created two route pipes plus pipe elbow fitting `1020916`, returned `createdRouteFittingCount: 1`, `routeFittingCount: 1`, `routeFittingRefCount: 2`, cleaned up all three created elements, and final counts returned to `488` pipes / `489` pipe fittings.
- Direct-loaded pipe source replacement live test `pipe-reroute-source-replace-live-20260505` created disposable source pipe `1020918`, committed `commit_reroute` to delete it and create two replacement pipe segments, native verify succeeded, cleanup restored `488` pipes / `489` pipe fittings. A combined source-replacement-plus-required-fitting probe timed out with no count residue; pipe fitting creation is covered by the separate passing smoke above.
...
- Re-generated plugin patch artifact as `21/21` from `origin/main..feature/native-write-plan-executor` and verified it applies cleanly with `git am --3way` on a temporary plugin `origin/main` worktree.
...
- Analysis-generated top-level `writePlanProposal` validation now receives the same office standards context, so enforced parameter allowlists also block unsafe proposal steps emitted by `analyze_mep_system`.
- `analyze_mep_system` now returns top-level `officeStandardsCompleteness`, aggregating per-engine missing office standards into one production-review gate with per-discipline rows. Runtime stdio probe surfaced `28` missing standards without mutating Revit.
- `officeStandardsCompleteness` now includes `officeStandardsInputTemplate`, a fillable machine-readable override skeleton for the exact missing standards. Runtime stdio probe returned `28` template paths with HVAC/domestic/fire placeholders and `mutateModel: false`; the same handoff is captured in `docs/revit-mep-office-standards-input-template.json`.
- Added `docs/revit-mep-project-critical-data-template.json` to capture project-specific critical path/circuit local-loss data, HVAC/hydronic flow maps, domestic/sanitary/storm/fire sizing requests, and model evidence needed before production-final sizing review.
- Added `docs/revit-mep-project-critical-data-live-sample.json` with sample-only live `inspect_elements` data for the current test model's HVAC path and hydronic circuit; it is marked engineer-review-required and non-committable.
- Added `handoff-templates.test.js` so the office standards and project-critical data JSON handoffs are checked against runtime missing-standard keys and `analyze_mep_system` argument names.
- Added `handoff_input_validator.js` and tests so filled office-standards/project-critical-data handoffs are checked before production review; placeholder standards, incomplete critical-path pressure flags, and sample-only live data remain non-committable.
- Surfaced the same guard through top-level `analyze_mep_system.handoffValidation` without adding another public MCP tool; runtime probe returned valid/non-mutating handoff validation for complete test standards plus a domestic-water sizing request.
- Added top-level `productionReadiness` to combine office standards, proposal data completeness, project-critical handoff completeness, and generated write-plan validation blockers. Runtime stdio probe showed missing standards and enforced parameter allowlist failure in one non-mutating readiness summary.
- `productionReadiness` now returns `nextRequiredInputs`, pointing to office-standards, project-critical-data, or proposal-validation handoff artefacts needed to clear current blockers. Project-critical handoff errors/blockers are included even when proposal data rows are otherwise complete.
- Runtime handler smoke confirmed complete office standards no longer hide missing project-critical local-loss data: the summary returned `projectCriticalDataComplete: false`, one `project_critical_data` next input, and `mutateModel: false`.
- Runtime handler smoke also confirmed a complete HVAC project-critical handoff clears the full readiness gate without mutating Revit: live target duct `392168`, `1260 m3/h` design flow, local-loss element `392203`, and complete local-loss pressure `0.903187266396887 Pa` returned `completeForProductionReview: true`, all readiness sub-gates true, `blockerCount: 0`, empty `nextRequiredInputs`, and one valid proposal-ready `resize_duct` step.
- Added top-level `productionSizingReview`, a non-committable final-review artefact derived only after production readiness is clear. Live runtime handler smoke on duct `392168` returned one `ready_for_engineer_final_review` row, `resize-duct-392168`, selected path pressure basis `4.0727263472465 Pa`, and `mutateModel: false`.
- Hydronic live runtime handler smoke confirmed the same final-review artefact on pipe `513756` with `5.0 L/s` design flow and fitting `513769` local-loss pressure `193.936 Pa`: readiness and sizing review were both complete, step `resize-pipe-513756` was linked, selected path pressure basis was `824.828359037195 Pa`, and `mutateModel: false`.
- Domestic/sanitary/storm/fire runtime handler smoke confirmed request-based production review packages now clear readiness and sizing review together without Revit mutation: four ready rows linked to `resize-domestic-water-pipe-601`, `resize-sanitary-pipe-701`, `resize-storm-pipe-702`, and `resize-fire-pipe-801`.
...
- Post-goal scenario library expansion: run more project-specific reroute/fitting cases as real office projects expose representative constraints. This is follow-up coverage, not a blocker for the current platform goal.
