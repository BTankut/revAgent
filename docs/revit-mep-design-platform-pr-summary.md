...
- Added native reroute fitting expectation gates: `requireRouteFittings` / `expectedRouteFittingCount` make route fitting references part of commit/verify success instead of a readback-only diagnostic.
...
- Added runtime/plugin validation for `expectedRouteFittingCount`; plugin build with `route-fitting-gate` output passed. Plans can now require route fitting references for production reroute acceptance, while still leaving broader automatic fitting creation/live production coverage as future work.
...
- Re-generated plugin patch artifact as `20/20` and verified it applies cleanly with `git am --3way` on a temporary plugin `main` worktree.
