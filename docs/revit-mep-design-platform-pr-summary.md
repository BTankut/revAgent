...
- Implemented initial operation catalog for parameter writes, pin/unpin, delete, view operations, movement, placement, duct/pipe creation, duct/pipe resize, and native schedule create/update.
...
- Added reroute fitting-reference verify readback in plugin patch `09/10`; plugin `Debug 2022|x64` build passed and live read-only probe on L-shaped replacement ducts `1020965`, `1020967`, `1020969` returned `routeFittingRefCount: 0`, making the no-separate-fitting behavior explicit.
...
- Re-generated plugin patch artifact as `11/11` and verified it applies cleanly with `git am --3way` on a temporary plugin `main` worktree.
