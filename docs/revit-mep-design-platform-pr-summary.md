...
- Implemented initial operation catalog for parameter writes, pin/unpin, delete, copy, view operations, movement, placement, duct/pipe creation, duct/pipe resize, and native schedule create/update.
...
- Re-generated plugin patch artifact as `12/12` and verified it applies cleanly with `git am --3way` on a temporary plugin `main` worktree.
