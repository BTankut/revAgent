...
- `officeStandardsCompleteness` now includes `officeStandardsInputTemplate`, a fillable machine-readable override skeleton for the exact missing standards. Runtime stdio probe returned `28` template paths with HVAC/domestic/fire placeholders and `mutateModel: false`.
