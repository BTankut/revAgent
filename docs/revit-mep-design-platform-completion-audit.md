...
| Engineering validation calculations | `engineering-calculations.test.js` checks duct/pipe calculations, rooted tree branch flow aggregation, weighted shortest-path traversal, fan pressure basis, pump head basis, critical path/critical circuit selection, and cycle/disconnected warnings; `domain-foundation-calculations.test.js` checks domestic water, sanitary/storm, fire/sprinkler, clash, equipment, and reporting foundations. | Done |
| Full engineering engines | Deterministic foundations now cover branch flow aggregation, weighted graph traversal, fan/pump basis, orthogonal clash reroute preview, and read-only report rows/CSV previews. Production flow-direction inference, hydraulic network balancing, production equipment schedule integration, reroute solving/commit verification, and report file export workflows are not complete. | Partially Done |
...
- Full production engineering engines remain beyond the current implemented foundation: flow-direction inference, hydraulic balancing, production reroute solving, equipment schedule integration, and approved report file export are still outstanding.

## Completion Decision

Do not mark the goal complete yet. The platform foundation is implemented, normal native socket preview/verify works, the user-approved disposable-model write-plan commit/verify acceptance test passed, and targeted live connector pathfinding plus fan/pump/report foundations are now in place. Remaining blockers are outside the current committed foundation: the plugin upstream is archived/read-only so its branch cannot be pushed, and full production engineering engines still need flow-direction inference, hydraulic balancing, reroute solving, equipment schedule integration, and approved report file export workflows.
