...
| Full engineering engines | Deterministic foundations now cover branch flow aggregation, weighted graph traversal, least-loss flow direction inference, fan/pump basis, live hydronic resistance extraction/calibration, live fitting/accessory/equipment local-loss extraction/reporting, targeted local-loss extraction input for known critical-path/circuit ids, local-loss pressure contribution into fan/pump basis, hydronic critical-circuit balancing loss, single/coupled-loop Hardy-Cross balancing, multi-candidate clash reroute solving, native reroute geometry/count/length/clearance/source-replacement verify, equipment schedule/report proposals with approved native note update execution, native model schedule population, live BOQ/report rows, and approved report file export. Production-calibrated final sizing from complete critical-path local-loss datasets and production reroute reconnection are not complete. | Partially Done |
