...
static void Acl(SafeFileHandle h,Identity expected){if(!expected.Same(Id(h)))throw new InvalidOperationException("fixture_parent_identity_drift");IntPtr o,g,d,s,x;uint r=GetSecurityInfo(h,SE,OWNER|DACL,out o,out g,out d,out s,out x);if(r!=0||x==IntPtr.Zero)throw new IOException("fixture_parent_acl_failed",(int)r);try{uint n=GetSecurityDescriptorLength(x);byte[] b=new byte[n];Marshal.Copy(x,b,0,(int)n);RawSecurityDescriptor sd=new RawSecurityDescriptor(b,0);SecurityIdentifier me=WindowsIdentity.GetCurrent().User,sys=new SecurityIdentifier(WellKnownSidType.LocalSystemSid,null),adm=new SecurityIdentifier(WellKnownSidType.BuiltinAdministratorsSid,null);if(sd.Owner==null||!sd.Owner.Equals(me)||sd.DiscretionaryAcl==null)throw new InvalidOperationException("fixture_parent_acl_untrusted");const int W=unchecked((int)0x500D0156);foreach(GenericAce z in sd.DiscretionaryAcl){QualifiedAce q=z as QualifiedAce;KnownAce k=z as KnownAce;if(q==null||k==null||q.SecurityIdentifier==null||q.AceQualifier!=AceQualifier.AccessAllowed)throw new InvalidOperationException("fixture_parent_acl_unknown");if((k.AccessMask&W)!=0&&!q.SecurityIdentifier.Equals(me)&&!q.SecurityIdentifier.Equals(sys)&&!q.SecurityIdentifier.Equals(adm))throw new InvalidOperationException("fixture_parent_acl_untrusted");}}finally{LocalFree(x);}if(!expected.Same(Id(h)))throw new InvalidOperationException("fixture_parent_identity_drift");}
