...
try {
    ...
    Assert-True ($candidateReadinessIndex -ge 0 -and $tryBeforeCandidateReadinessIndex -ge 0 -and $tryBeforeCandidateReadinessIndex -lt $candidateReadinessIndex -and $finallyAfterCandidateReadinessIndex -gt $candidateReadinessIndex) "NAS publisher must clean candidate channel artifacts even when candidate readiness fails before stable promotion."
    ...
    Assert-True ($publisherText -match 'function Invoke-RevAgentFileSystemRetry' -and $publisherText -match 'function Remove-RevAgentDirectoryWithRetry' -and $publisherText -match 'copy-staging' -and $publisherText -match 'Promote staged copy') "NAS publisher must replace tools and release payloads through retryable staged directory promotion for SMB shares."
    Assert-True ($publisherText -match 'function Wait-RevAgentPathAbsent' -and $publisherText -match 'Wait-RevAgentPathAbsent -Path \$Path') "NAS publisher must wait for removed share paths to disappear before reusing the same directory name."
