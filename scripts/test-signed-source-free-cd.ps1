...
"@
    ...
    Assert-True ($publisherText -match 'Refresh-revAgent-LocalBootstrap-STABLE\.cmd' -and $publisherText -match 'Set-RevAgentStableBootstrapToolsExact' -and $publisherText -match 'bootstrap install completed' -and $publisherText -match 'VerificationOnly >nul 2>nul' -and $publisherText -match 'call "%REFRESH%"' -and $publisherText -match 'stable bootstrap refresh completed') "Production stable launcher template must install missing clean-machine bootstraps, auto-refresh stale protected local bootstraps, and publish the matching bootstrap helper tools."
