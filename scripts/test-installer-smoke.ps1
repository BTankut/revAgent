...
'@
    ...
    Assert-True ($stableLauncherText -match '%ProgramData%\\DPE\\revAgent\\bootstrap\\Start-revAgent-Update\.ps1' -and $stableLauncherText -notmatch 'Install-revAgent-Updater-GUI\.ps1' -and $stableLauncherText -match 'Refresh-revAgent-LocalBootstrap-STABLE\.cmd' -and $stableLauncherText -match 'bootstrap install completed' -and $stableLauncherText -match 'call "%REFRESH%"' -and $stableLauncherText -match 'returned without creating the protected local bootstrap' -and $stableLauncherText -notmatch 'goto launch_updater') "Stable launcher must call only the protected local bootstrap, route clean machines through first-install bootstrap, fail visibly if refresh returns without creating the protected bootstrap, and leave successful refresh GUI launch to the refresh tool."
    Assert-True ($legacyStableLauncherText -match '%ProgramData%\\DPE\\revAgent\\bootstrap\\Start-revAgent-Update\.ps1' -and $legacyStableLauncherText -notmatch 'Install-revAgent-Updater-GUI\.ps1' -and $legacyStableLauncherText -match 'Refresh-revAgent-LocalBootstrap-STABLE\.cmd' -and $legacyStableLauncherText -match 'bootstrap install completed' -and $legacyStableLauncherText -match 'call "%REFRESH%"' -and $legacyStableLauncherText -match 'returned without creating the protected local bootstrap' -and $legacyStableLauncherText -notmatch 'goto launch_updater') "Legacy stable launcher must keep the same clean-install and stale-bootstrap auto-refresh behavior as the revAgent launcher."
    Assert-True ($bootstrapRefreshCmdText -match '%ProgramData%\\DPE\\revAgent\\bootstrap\\Start-revAgent-Update\.ps1' -and $bootstrapRefreshCmdText -match 'returned without creating the protected local bootstrap' -and $bootstrapRefreshCmdText -match 'pause') "Bootstrap refresh CMD must not return success silently when the protected local bootstrap was not created."
