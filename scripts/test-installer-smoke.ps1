...
'@
    ...
    Assert-True ($stableLauncherText -match '%ProgramData%\\DPE\\revAgent\\bootstrap\\Start-revAgent-Update\.ps1' -and $stableLauncherText -notmatch 'Install-revAgent-Updater-GUI\.ps1' -and $stableLauncherText -match 'Refresh-revAgent-LocalBootstrap-STABLE\.cmd' -and $stableLauncherText -match 'call "%REFRESH%"') "Stable launcher must call only the protected local bootstrap, and automatically route stale-bootstrap failures through the signed stable refresh tool."
    Assert-True ($legacyStableLauncherText -match '%ProgramData%\\DPE\\revAgent\\bootstrap\\Start-revAgent-Update\.ps1' -and $legacyStableLauncherText -notmatch 'Install-revAgent-Updater-GUI\.ps1' -and $legacyStableLauncherText -match 'Refresh-revAgent-LocalBootstrap-STABLE\.cmd' -and $legacyStableLauncherText -match 'call "%REFRESH%"') "Legacy stable launcher must keep the same stale-bootstrap auto-refresh behavior as the revAgent launcher."
    ...
    Assert-True ($bootstrapRefreshText -match 'Get-ProtectedBootstrapState' -and $bootstrapRefreshText -match 'New-RevAgentAuthenticatedReleaseInbox' -and $bootstrapRefreshText -match 'install-revagent-local-bootstrap\.ps1' -and $bootstrapRefreshText -match 'ConfirmIndependentlyAuthenticatedSource' -and $bootstrapRefreshText -match 'Start-Process -FilePath \$localLauncher') "Bootstrap refresh tool must verify through the protected local bootstrap, stage the signed local-bootstrap installer, and relaunch the local protected updater."
