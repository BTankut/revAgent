...
try {
    ...
    Assert-True ($updateText -match 'hasAcceptedSignedRelease' -and $updateText -match '\[Math\]::Max\(\s+\$highestAcceptedReleaseSequence') "Updater must not lower the stored signed-release high-watermark."
