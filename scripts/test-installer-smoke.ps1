...
try {
    ...
    Assert-True ($findHandlerCode -match 'FilterElementIdRule' -and $findHandlerCode -match 'BuildLevelParameterElementFilter' -and $findHandlerCode -match 'new ElementId\(\(int\)builtInParameter\)' -and $findHandlerCode -match 'flatFilters\.Add' -and $findHandlerCode -match 'INSTANCE_SCHEDULE_ONLY_LEVEL_PARAM' -and $findHandlerCode -match 'RBS_START_LEVEL_PARAM') "find_elements bridge level prefilter must preserve MEP fallback level-parameter matches with a flat OR filter."
    Assert-True ($findHandlerCode -match 'if \(searchDocument == null\)[\s\S]+return levelIds') "find_elements bridge must guard null search documents before level prefilter collectors."
    ...
    Assert-True ($findHandlerCode -match 'MatchesCollectorLevelName\(level\.Name, linkedDocument\)' -and $findHandlerCode -match 'ContainsExact\(levelName, _levelNamesFromLevelIds\)' -and $findHandlerCode -match 'ContainsExact\(levelName, _levelNamesFromLevelIds\)[\s\S]+_levelNames\.Count > 0 && !ContainsAny\(levelName, _levelNames\)') "find_elements bridge must apply linked-document levelIds exactly and levelNames independently."
    ...
    Assert-True ($statusToolCode -match 'revit-mcp-runtime-tools\.34') "Runtime tool surface version must be bumped when exported tool behavior/schema changes."
    ...
    Assert-True ($setParameterToolCode -match 'operation: z\.enum\(\["set", "clear"\]\)') "set_element_parameter must expose explicit set/clear operations."
