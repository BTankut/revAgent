...
try {
    ...
    Assert-True ($coordinationImageToolCode -match 'cropImageToModelProjection') "Coordination image export must crop from model bounding-box/camera projection, not primarily from raster color detection."
    ...
    Assert-True ($coordinationImageToolCode -match 'target_highlight_pixels_not_detected') "Coordination image export must warn, not fail, when raster target pixels are not detected."
    Assert-True ($coordinationImageToolCode -match 'croppedToModelProjection') "Coordination image export must report whether model-projection crop was used."
    ...
    Assert-True ($coordinationImageToolCode -match 'projectionDesiredSide') "Coordination image export must size model-projection crops from targetMinFillRatio."
