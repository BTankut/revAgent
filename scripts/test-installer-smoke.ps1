...
try {
    ...
    Assert-True ($findHandlerCode -match 'FilterElementIdRule' -and $findHandlerCode -match 'BuildLevelParameterElementFilter' -and $findHandlerCode -match 'new ElementId\(\(int\)builtInParameter\)' -and $findHandlerCode -match 'INSTANCE_SCHEDULE_ONLY_LEVEL_PARAM' -and $findHandlerCode -match 'RBS_START_LEVEL_PARAM') "find_elements bridge level prefilter must preserve MEP fallback level-parameter matches."
