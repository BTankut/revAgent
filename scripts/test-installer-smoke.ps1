...
'@
    ...
    $bootstrapRefreshCmdText = Get-Content -Raw -LiteralPath (Join-Path $RepoRoot "installer\nas\Refresh-revAgent-LocalBootstrap-STABLE.cmd")
    $bootstrapRefreshPath = Join-Path $RepoRoot "installer\nas\Refresh-revAgent-LocalBootstrap-STABLE.ps1"
    $bootstrapRefreshText = Get-Content -Raw -LiteralPath $bootstrapRefreshPath
    $bootstrapRefreshTokens = $null
    $bootstrapRefreshErrors = $null
    ...
    $legacyUpdaterAliasTexts = @(
        Get-Content -Raw -LiteralPath (Join-Path $RepoRoot 'installer\nas\Install-Revit-MCP-Updater-GUI.cmd')
        Get-Content -Raw -LiteralPath (Join-Path $RepoRoot 'installer\nas\Install-revAgent-Updater.cmd')
        Get-Content -Raw -LiteralPath (Join-Path $RepoRoot 'installer\nas\Install-Revit-MCP-Updater.cmd')
    )
    Write-Host "Test PowerShell language-mode guards run before every updater entrypoint"
    $languageGuardPaths = @(
        'installer\nas\Install-revAgent-Updater-GUI.ps1',
    ...
    $languageGuardTexts = [System.Collections.Generic.List[string]]::new()
    foreach ($relativePath in $languageGuardPaths) {
        $entrypointPath = Join-Path $RepoRoot $relativePath
        ...
        Assert-Equal @($entrypointErrors).Count 0 "$relativePath does not parse."
        $firstStatement = @($entrypointAst.EndBlock.Statements | Select-Object -First 1)
        Assert-Equal $firstStatement.Count 1 "$relativePath has no executable entrypoint statement."
        Assert-True ($firstStatement[0] -is [Management.Automation.Language.IfStatementAst]) "$relativePath must check LanguageMode before any other executable statement."
        $guardText = [string]$firstStatement[0].Extent.Text
        Assert-True ($guardText -match '\$ExecutionContext\.SessionState\.LanguageMode' -and $guardText -match "-ne\s+'FullLanguage'" -and $guardText -match 'Smart App Control or a WDAC/AppLocker policy' -and $guardText -match 'exit\s+78') "$relativePath language-mode guard is incomplete."
        [void]$languageGuardTexts.Add(($guardText -replace '\s+', ' ').Trim())
    }
    Assert-Equal @($languageGuardTexts | Select-Object -Unique).Count 1 "Updater entrypoints must use one identical LanguageMode guard and exit code."
    Assert-True ($stableLauncherText -match '\[Environment\]::GetFolderPath\(\[Environment\+SpecialFolder\]::CommonApplicationData\)' -and $stableLauncherText -notmatch '%ProgramData%' -and $stableLauncherText -notmatch 'Install-revAgent-Updater-GUI\.ps1' -and $stableLauncherText -match 'Refresh-revAgent-LocalBootstrap-STABLE\.cmd' -and $stableLauncherText -match 'bootstrap install completed' -and $stableLauncherText -match 'call "%REFRESH%"' -and $stableLauncherText -match 'returned without creating the protected local bootstrap' -and $stableLauncherText -match 'set "BOOTSTRAP_EXIT=!ERRORLEVEL!"' -and $stableLauncherText -notmatch 'start "revAgent"|goto launch_updater') "Stable launcher must use canonical CommonApplicationData, call only the protected local bootstrap, and preserve the synchronous bootstrap result."
    Assert-True ($legacyStableLauncherText -match '\[Environment\]::GetFolderPath\(\[Environment\+SpecialFolder\]::CommonApplicationData\)' -and $legacyStableLauncherText -notmatch '%ProgramData%' -and $legacyStableLauncherText -notmatch 'Install-revAgent-Updater-GUI\.ps1' -and $legacyStableLauncherText -match 'Refresh-revAgent-LocalBootstrap-STABLE\.cmd' -and $legacyStableLauncherText -match 'bootstrap install completed' -and $legacyStableLauncherText -match 'call "%REFRESH%"' -and $legacyStableLauncherText -match 'returned without creating the protected local bootstrap' -and $legacyStableLauncherText -match 'set "BOOTSTRAP_EXIT=!ERRORLEVEL!"' -and $legacyStableLauncherText -notmatch 'start "revAgent"|goto launch_updater') "Legacy stable launcher must keep the same canonical synchronous fail-closed behavior as the revAgent launcher."
    Assert-True ([Linq.Enumerable]::SequenceEqual([byte[]][IO.File]::ReadAllBytes((Join-Path $RepoRoot 'installer\nas\revAgent Updater STABLE.cmd')), [byte[]][IO.File]::ReadAllBytes((Join-Path $RepoRoot 'installer\nas\Revit MCP Updater STABLE.cmd')))) 'Canonical and legacy STABLE launcher templates must remain byte-identical.'
    $cmdLanguageModeProbe = '$mode=$ExecutionContext.SessionState.LanguageMode'
    $cmdKnownFolderCall = '[Environment]::GetFolderPath([Environment+SpecialFolder]::CommonApplicationData)'
    Assert-True ($stableLauncherText.IndexOf($cmdLanguageModeProbe) -ge 0 -and $stableLauncherText.IndexOf($cmdLanguageModeProbe) -lt $stableLauncherText.IndexOf($cmdKnownFolderCall) -and $stableLauncherText -match 'REVAGENT_LANGUAGE_MODE=' -and $stableLauncherText -match 'REVAGENT_COMMON_APP_DATA=' -and $stableLauncherText -match 'Smart App Control or a WDAC/AppLocker policy' -and $stableLauncherText -match 'exit /b 78') 'STABLE launcher must fail with the friendly exact exit 78 sentinel before a constrained child can reach the FullLanguage-only KnownFolder method.'
    Assert-True ($bootstrapRefreshCmdText.IndexOf($cmdLanguageModeProbe) -ge 0 -and $bootstrapRefreshCmdText.IndexOf($cmdLanguageModeProbe) -lt $bootstrapRefreshCmdText.IndexOf($cmdKnownFolderCall) -and $bootstrapRefreshCmdText -match 'REVAGENT_LANGUAGE_MODE=' -and $bootstrapRefreshCmdText -match 'REVAGENT_COMMON_APP_DATA=' -and $bootstrapRefreshCmdText -match 'Smart App Control or a WDAC/AppLocker policy' -and $bootstrapRefreshCmdText -match 'exit /b 78') 'Refresh CMD must fail with the friendly exact exit 78 sentinel before a constrained child can reach the FullLanguage-only KnownFolder method.'
    Assert-True ($legacyUpdaterStubText -match 'This launcher is deprecated; opening revAgent Updater STABLE' -and $legacyUpdaterStubText -match '%~dp0revAgent Updater STABLE\.cmd' -and $legacyUpdaterStubText -match '%~dp0tools\\revAgent Updater STABLE\.cmd' -and $legacyUpdaterStubText -match 'call "%STABLE%" %\*' -and $legacyUpdaterStubText -notmatch 'SECURITY STOP') 'Canonical legacy updater stub must delegate to the managed STABLE flow from either tools or the NAS root.'
    foreach ($legacyUpdaterAliasText in $legacyUpdaterAliasTexts) {
        Assert-True ($legacyUpdaterAliasText -match 'Install-revAgent-Updater-GUI\.cmd' -and $legacyUpdaterAliasText -match 'call "%TARGET%" %\*' -and $legacyUpdaterAliasText -notmatch 'SECURITY STOP') 'Legacy updater alias must remain a thin compatibility delegate without its old clean-machine dead end.'
    }
    ...
    foreach ($exitCode in $friendlyBootstrapExitMessages.Keys) {
    ...
    Assert-True ($snapshotBrokerText -match 'function Repair-RevAgentBrokerLegacyWorkRoot' -and $snapshotBrokerText -match 'Set-RevAgentBrokerDirectorySecurity -Path \$installRoot' -and $snapshotBrokerText -match "foreach \(\`$protectedLeaf in @\('machine-logs', 'machine-state'\)\)" -and $snapshotBrokerText -match "foreach \(\`$userLeaf in @\('logs', 'user-state'\)\)" -and $snapshotBrokerText -match 'New-RevAgentBrokerUserWritableDirectorySecurity -TargetUserSid \$TargetUserSid') "Privileged broker must repair legacy updater ACLs while keeping only logs/user-state writable for the target user."
    Assert-True ($snapshotBrokerText.IndexOf('Repair-RevAgentBrokerLegacyWorkRoot -CanonicalInstallRoot $canonicalInstallRoot -TargetUserSid $TargetInteractiveUserSid') -ge 0 -and $snapshotBrokerText.IndexOf('Repair-RevAgentBrokerLegacyWorkRoot -CanonicalInstallRoot $canonicalInstallRoot -TargetUserSid $TargetInteractiveUserSid') -lt $snapshotBrokerText.IndexOf('Initialize-RevAgentBrokerLogPath -Path $BrokerLogPath -CanonicalInstallRoot $canonicalInstallRoot')) "Privileged broker must repair legacy updater ACLs before validating/writing the protected broker log."
...
Assert-True ($updateText -notmatch '\$userChannelRoot\s*=\s*Split-Path[\s\S]{0,240}\$ReportsRoot\s*=\s*Join-Path\s+\$userReleaseRoot\s+["'']reports["'']') "User-phase reports must never fall back to the execution snapshot-local ChannelManifestPath."
    ...
    Assert-True ($signedCdWorkflowText -match '(?s)- name: Upload immutable signed release artifact\s+id: signed-release-artifact\s+if: \$\{\{ github\.event_name == ''workflow_dispatch'' && \(inputs\.publish_to_nas \|\| inputs\.publish_to_pilot\) \}\}\s+uses: actions/upload-artifact@[0-9a-f]{40}\s+# actions/upload-artifact@v7' -and $signedCdWorkflowText -match 'if-no-files-found:\s*error' -and $signedCdWorkflowText -match 'retention-days:\s*1' -and $signedCdWorkflowText -match 'overwrite:\s*false' -and $signedCdWorkflowText -match 'include-hidden-files:\s*true') "Only an explicit publish dispatch may upload one immutable, short-lived, complete release artifact; main-push validation stays local and fails on an empty publish tree."
    ...
    Assert-True ($npmCiFunctionText -match '\$previousNpmIgnoreScripts = \[Environment\]::GetEnvironmentVariable\("npm_config_ignore_scripts", "Process"\)' -and $npmCiFunctionText -match 'try\s*\{' -and $npmCiFunctionText -match '\$env:npm_config_ignore_scripts = "true"' -and $npmCiFunctionText -match 'npm\s+ci\s+--ignore-scripts' -and $npmCiFunctionText -match 'finally\s*\{' -and $npmCiFunctionText -match 'Remove-Item Env:\\npm_config_ignore_scripts' -and $npmCiFunctionText -match '\$env:npm_config_ignore_scripts = \$previousNpmIgnoreScripts') "MCP package npm ci must disable lifecycle scripts process-locally, pass --ignore-scripts explicitly, and restore the caller environment through finally."
    ...
    Assert-True ($installerText -match 'Copied Codex AGENTS\.md from machine install' -and $installerText -notmatch 'ItemType HardLink') "Self-contained installer must not create AGENTS.md hardlinks under the current machine/user Codex integration model."
