...
try {
    ...
    Assert-True ($findHandlerCode -match 'IsLinkedOnlyExactIdentitySearch') "find_elements bridge must guard linkedOnly exact host-id lookups."
    Assert-True ($findHandlerCode -notmatch 'AddExactElementMatches\(linkDocument') "find_elements bridge must not apply host exact element ids inside linked documents."
    ...
    Assert-True ($searchPolicyCode -match '\(\?<\!\\\\p\{L\}\)' -and $searchPolicyCode -match '\(\?!\\\\p\{L\}\)') "Search policy concept stripping must use Unicode-aware word boundaries for Turkish terms."
