...
try {
    ...
    Assert-True ($localBootstrapText -match 'bootstrap-state\.json' -and $localBootstrapText -match 'independentlyAuthenticated' -and $localBootstrapText -match 'FileMode\]::CreateNew' -and $localBootstrapText -match 'PSModulePath' -and $localBootstrapText -match 'installerLibReleaseSnapshot' -and $localBootstrapText -match 'privilegedSnapshotUpdate') "Local bootstrap must enforce protected state, effective read-only access, trusted modules, and signed snapshot-broker mapping."
...
Assert-True ($updateText -notmatch '\$userChannelRoot\s*=\s*Split-Path[\s\S]{0,240}\$ReportsRoot\s*=\s*Join-Path\s+\$userReleaseRoot\s+["'']reports["'']') "User-phase reports must never fall back to the execution snapshot-local ChannelManifestPath."
    ...
    Assert-True ($updateText -match 'Codex MCP registration cannot run in the elevated machine phase' -and $updateText -match 'Register-CodexMcpServersInConfig') "Legacy updater flow must fail closed in the machine phase and use direct authenticated-user config registration when unelevated."
