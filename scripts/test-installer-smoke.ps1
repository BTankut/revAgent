...
'@
    ...
    Assert-True ($guiText -match 'Import-Module \$script:GuiPermissionsModulePath -Force -PassThru' -and $guiText -match 'Remove-StaleRevAgentBootstrapTemporaryItems' -and $guiText -match '(?s)\$form\.Add_Shown\(\{\s*\$script:GuiStartupCompleted\s*=\s*\$true\s*Start-RevAgentGuiStartupMaintenance\s*\}\)') "GUI must invoke the exact protected stale-bootstrap cleanup command only after the Shown startup-complete marker."
    Assert-True ($permissionsText -match 'function Remove-StaleRevAgentBootstrapTemporaryItems' -and $permissionsText -match 'Local\\revAgentBootstrapTempCleanup' -and $permissionsText -match 'MaxItems\s*=\s*100000' -and $permissionsText -match 'MaxDepth\s*=\s*64' -and $permissionsText -match 'MaxPassesPerDirectory\s*=\s*32' -and $permissionsText -match 'OpenNoMutation' -and $permissionsText -match 'RemoveDirectoryLink' -and $permissionsText -match 'Export-ModuleMember[\s\S]*Remove-StaleRevAgentBootstrapTemporaryItems') "Protected permissions module must export bounded, mutexed, no-follow stale-bootstrap cleanup."
