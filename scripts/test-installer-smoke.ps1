...
try {
    ...
    Assert-True ($installTaskText -match 'function Copy-RevAgentManagedUpdaterToolFile' -and $installTaskText -match 'Remove-Item -LiteralPath \$Destination -Force' -and $installTaskText -match 'Copy-RevAgentManagedUpdaterToolFile -Source \(Join-Path \$PSScriptRoot "migrate-source-free-install\.ps1"\) -Destination \$localMigrationTool') "Updater task installer must replace stale ACL-blocked local updater tool files, including the migration tool."
    Assert-True ($installTaskText -match 'RevAgent\.DesktopLauncherCleanup\.psm1' -and $installTaskText -match 'Invoke-RevAgentLegacyDesktopLauncherCleanup' -and $installTaskText -match 'desktopLauncherCleanup') "Updater task installer must remove and report legacy Revit MCP desktop launchers."
    ...
    Assert-True ($updaterText -match 'function Copy-RevAgentManagedUpdaterToolFile' -and $updaterText -match 'Remove-Item -LiteralPath \$Destination -Force' -and $updaterText -match 'Copy-RevAgentManagedUpdaterToolFile -Source \$source -Destination \(Join-Path \$DestinationRoot \$toolName\)') "Updater fast-path tool refresh must replace stale ACL-blocked updater tool files."
    Assert-True ($updaterText -match 'RevAgent\.DesktopLauncherCleanup\.psm1' -and $updaterText -match 'Invoke-RevAgentLegacyDesktopLauncherCleanup' -and $updaterText -match 'desktopLauncherCleanup') "Updater must remove and report legacy Revit MCP desktop launchers."
    ...
    Assert-True ($installerText -match 'function Copy-RevAgentManagedUpdaterToolFile' -and $installerText -match 'Remove-Item -LiteralPath \$Destination -Force' -and $installerText -match 'Copy-RevAgentManagedUpdaterToolFile -Source \$source -Destination \(Join-Path \$DestinationRoot \$toolName\)') "Self-contained installer updater tool refresh must replace stale ACL-blocked updater tool files."
    Assert-True ($installerText -match 'RevAgent\.DesktopLauncherCleanup\.psm1' -and $installerText -match 'Invoke-RevAgentLegacyDesktopLauncherCleanup') "Self-contained installer must remove legacy Revit MCP desktop launchers."
