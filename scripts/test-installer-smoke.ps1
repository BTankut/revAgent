...
try {
    ...
    Assert-True ($findHandlerCode -match 'FilterElementIdRule' -and $findHandlerCode -match 'BuildLevelParameterElementFilter' -and $findHandlerCode -match 'INSTANCE_SCHEDULE_ONLY_LEVEL_PARAM' -and $findHandlerCode -match 'RBS_START_LEVEL_PARAM') "find_elements bridge level prefilter must preserve MEP fallback level-parameter matches."
    ...
    Assert-True ($findHandlerCode -match '_levelIds\.Count > 0\)[\s\S]+_levelNamesFromLevelIds\.Count == 0[\s\S]+_levelNames\.Count > 0 && !ContainsAny\(levelName, _levelNames\)') "find_elements bridge must apply linked-document levelIds and levelNames as independent AND filters."
