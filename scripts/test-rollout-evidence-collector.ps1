...
Assert-True ($scriptText -match '\$migrationArgs = @\(') "Collector must build source-free helper arguments as an array."
Assert-True ($scriptText -match "'dryRun'") "Collector must run source-free migration in dryRun mode only."
...
Assert-True ($scriptText -match '\$launcherArgs = @\(') "Collector must build desktop launcher helper arguments as an array."
Assert-True ($scriptText -match "'ScanLocal'") "Collector must collect per-machine desktop launcher evidence."
...
Assert-True ($scriptText -match '\$desktopLauncherReportPath') "Remote desktop launcher scan must write to a local staged output file."
...
Assert-True ($scriptText -match '\$expectedMachinesCsv') "Collector must pass aggregate expected machines as one CSV argument."
Assert-True ($scriptText -notmatch '\+ @\(\$targets \| ForEach-Object \{ \[string\]\$_.Computer \}\) \+ @\("-OutputJson"\)') "Collector must not pass expected machines as loose positional arguments."
