...
$fixtures = @(
    ...
    [pscustomobject]@{ name = 'anchor-punctuation-name'; success = $false; content = 'key: &.u value' },
    [pscustomobject]@{ name = 'alias-punctuation-name'; success = $false; content = '*.u: value' },
    [pscustomobject]@{ name = 'anchor-unicode-name'; success = $false; content = 'key: &ü value' },
    [pscustomobject]@{ name = 'alias-unicode-name'; success = $false; content = '*ü: value' },
    [pscustomobject]@{ name = 'anchor-bang-name'; success = $false; content = 'key: &!name value' },
    [pscustomobject]@{ name = 'alias-percent-name'; success = $false; content = 'key: *%name' },
    ...
    [pscustomobject]@{ name = 'safe-quoted-anchor-alias-text'; success = $true; content = 'note: "&not-an-anchor *not-an-alias"' },
    [pscustomobject]@{ name = 'safe-run-block-shell-text'; success = $true; content = "run: |`n  echo one && echo two`n  printf '%s\\n' *.yml`nuses: actions/checkout@$sha`n" },
    [pscustomobject]@{ name = 'block-sequence-dedent-tag'; success = $false; content = "- name: |`n      payload`n    uses: actions/checkout@v6`n" },
    [pscustomobject]@{ name = 'block-sequence-dedent-dynamic'; success = $false; content = "- name: |`n      payload`n    uses: `${{ github.repository }}/action@main`n" },
    [pscustomobject]@{ name = 'block-sequence-dedent-reusable'; success = $false; content = "- name: |`n      payload`n    uses: owner/repo/.github/workflows/release.yml@$sha`n" },
    [pscustomobject]@{ name = 'block-sequence-dedent-docker'; success = $false; content = "- name: |`n      payload`n    uses: docker://alpine:3.20`n" },
    [pscustomobject]@{ name = 'block-description-dedent'; success = $false; content = "description: >`n  payload`nuses: actions/checkout@v6`n" },
    [pscustomobject]@{ name = 'block-explicit-indent'; success = $false; content = "description: |2`n  payload`nuses: actions/checkout@v6`n" },
    [pscustomobject]@{ name = 'block-folded-chomp'; success = $false; content = "description: >-`n  payload`nuses: actions/checkout@v6`n" },
    [pscustomobject]@{ name = 'block-blank-comment-first-payload'; success = $false; content = "description: |`n`n  # payload`n    payload`nuses: actions/checkout@v6`n" },
    [pscustomobject]@{ name = 'block-safe-indented-content-and-pinned-sibling'; success = $true; content = "run: |`n  uses: ignored-in-block`n  echo one && echo two`n  printf '%s\\n' *.yml`nuses: actions/checkout@$sha`n" },
    [pscustomobject]@{ name = 'block-comment-first-tag-sibling'; success = $false; content = "description: |`n  # payload`nuses: actions/checkout@v6`n" },
    [pscustomobject]@{ name = 'block-comment-first-dynamic-sibling'; success = $false; content = "description: |`n  # payload`nuses: `${{ github.repository }}/action@main`n" },
    [pscustomobject]@{ name = 'block-comment-first-reusable-sibling'; success = $false; content = "description: |`n  # payload`nuses: owner/repo/.github/workflows/release.yml@$sha`n" },
    [pscustomobject]@{ name = 'block-comment-first-docker-sibling'; success = $false; content = "description: |`n  # payload`nuses: docker://alpine:3.20`n" },
    [pscustomobject]@{ name = 'block-ordinary-then-comment-payload'; success = $false; content = "description: |2`n  payload`n  # still payload`nuses: actions/checkout@v6`n" },
    [pscustomobject]@{ name = 'block-blank-then-comment-payload'; success = $false; content = "description: |`n`n  # payload`nuses: actions/checkout@v6`n" },
    [pscustomobject]@{ name = 'block-folded-comment-first-payload'; success = $false; content = "description: >-`n  # payload`nuses: actions/checkout@v6`n" },
    [pscustomobject]@{ name = 'block-safe-comment-uses-and-pinned-sibling'; success = $true; content = "run: |`n  # uses: actions/checkout@v6`n  payload`nuses: actions/checkout@$sha`n" },
    [pscustomobject]@{ name = 'empty-sequence-block-tag-sibling'; success = $false; content = "- name: |`n  uses: actions/checkout@v6`n" },
    [pscustomobject]@{ name = 'empty-sequence-block-dynamic-sibling'; success = $false; content = "- name: |`n  uses: `${{ github.repository }}/action@main`n" },
    [pscustomobject]@{ name = 'empty-sequence-block-reusable-sibling'; success = $false; content = "- name: |`n  uses: owner/repo/.github/workflows/release.yml@$sha`n" },
    [pscustomobject]@{ name = 'empty-sequence-block-docker-sibling'; success = $false; content = "- name: |`n  uses: docker://alpine:3.20`n" },
    [pscustomobject]@{ name = 'empty-sequence-quoted-key-sibling'; success = $false; content = "- `"name`": |`n  uses: actions/checkout@v6`n" },
    [pscustomobject]@{ name = 'empty-sequence-explicit-chomp-sibling'; success = $false; content = "- name: >-2`n  uses: actions/checkout@v6`n" },
    [pscustomobject]@{ name = 'nonempty-sequence-block-pinned-sibling'; success = $true; content = "- name: |`n    payload`n  uses: actions/checkout@$sha`n" },
    [pscustomobject]@{ name = 'empty-top-level-block-tag-sibling'; success = $false; content = "description: |`nuses: actions/checkout@v6`n" },
