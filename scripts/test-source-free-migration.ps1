...
Assert-True ($migrationText -match 'Join-Path \(\[Environment\]::SystemDirectory\) "WindowsPowerShell\\v1\.0\\powershell\.exe"' -and $migrationText -match '\[void\]\$updateArgs\.Add\("-File"\)' -and $migrationText -match '\[void\]\$updateArgs\.Add\(\$updaterPath\)') "Migration commit mode must launch the updater through the canonical Windows PowerShell -File host so transcripts do not inherit encoded wrapper commands or PATH resolution."
...
Assert-True ($updaterText -match 'RevAgentDistributionIntegrityModule = if \(\$MachinePhaseOnly\)' -and $updaterText -match 'RevAgentPreImportIntegrityModule' -and $updaterText -match 'Import-Module .*RevAgent\.DistributionIntegrity\.psm1.*-PassThru' -and $updaterText -match 'function Get-UpdaterDistributionIntegrityCommand' -and $updaterText -match 'Get-UpdaterDistributionIntegrityCommand -Name "Test-RevAgentReleaseDistributionIntegrity" -Required') "Updater must reuse the pinned pre-import verifier in machine mode and call helpers through the imported module object during nested migration runs."
