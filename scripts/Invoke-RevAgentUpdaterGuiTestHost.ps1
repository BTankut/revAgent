...
namespace RevAgent.CleanFixtureHost
{
    ...
    internal static class Native
    {
        ...
        internal static void VerifyAcl(SafeFileHandle handle,Identity expected){if(!expected.Same(ReadIdentity(handle)))throw new InvalidOperationException("fixture_host_pin_drift");IntPtr owner,group,dacl,sacl,descriptor;uint status=GetSecurityInfo(handle,SE_FILE_OBJECT,OWNER_SECURITY_INFORMATION|DACL_SECURITY_INFORMATION,out owner,out group,out dacl,out sacl,out descriptor);if(status!=0||descriptor==IntPtr.Zero)throw new IOException("fixture_handle_acl_read_failed",unchecked((int)status));try{uint length=GetSecurityDescriptorLength(descriptor);if(length==0||length>1024*1024)throw new InvalidOperationException("fixture_acl_descriptor_uncertain");byte[] bytes=new byte[length];Marshal.Copy(descriptor,bytes,0,(int)length);RawSecurityDescriptor security=new RawSecurityDescriptor(bytes,0);SecurityIdentifier me=WindowsIdentity.GetCurrent().User,system=new SecurityIdentifier(WellKnownSidType.LocalSystemSid,null),admins=new SecurityIdentifier(WellKnownSidType.BuiltinAdministratorsSid,null),trustedInstaller=new SecurityIdentifier("S-1-5-80-956008885-3418522649-1831038044-1853292631-2271478464");Func<SecurityIdentifier,bool> trusted=value=>value!=null&&(value.Equals(me)||value.Equals(system)||value.Equals(admins)||value.Equals(trustedInstaller));if(security.Owner==null||!trusted(security.Owner)||security.DiscretionaryAcl==null)throw new InvalidOperationException("fixture_acl_untrusted");const int writeMask=unchecked((int)0x500D0156);bool inherited=false;foreach(GenericAce generic in security.DiscretionaryAcl){if((generic.AceFlags&AceFlags.Inherited)!=0)inherited=true;else if(inherited)throw new InvalidOperationException("fixture_acl_noncanonical");QualifiedAce ace=generic as QualifiedAce;KnownAce known=generic as KnownAce;if(ace==null||known==null||ace.SecurityIdentifier==null||ace.AceQualifier!=AceQualifier.AccessAllowed)throw new InvalidOperationException("fixture_acl_unknown_or_deny");if((known.AccessMask&writeMask)!=0&&!trusted(ace.SecurityIdentifier))throw new InvalidOperationException("fixture_acl_untrusted");}}finally{LocalFree(descriptor);}}
