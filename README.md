...
Connected same-direction pipe overlaps are guarded as a separate `normalize_pipe_header_overlap` write-plan operation. Valid preview data must identify exactly two pipe ids, require same system type, collinear overlap, same diameter, both opposite ends connected, branch connectivity preservation, fitting replacement permission, rollback preview, `commitBatchSize: 1`, heartbeat, timeout recovery, unchanged device connectivity, and zero clash increase. The native command now previews and commits one pair at a time, with transaction rollback, for direct fitting-to-branch pipe tees, short pipe-offset branch tees, and orphan open fitting cleanup. It projects a real pipe-owned branch connector to the header centerline, adjusts only collinear branch endpoints, deletes obsolete overlap/fitting/offset geometry, splits the header with `PlumbingUtils.BreakCurve`, and creates the tee through Revit API. Broad raw dynamic commits for these connected overlap pairs remain unsafe; use typed preview/commit plus post-commit audit.
