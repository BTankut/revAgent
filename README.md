...
Connected same-direction pipe overlaps are guarded as a separate `normalize_pipe_header_overlap` write-plan operation. Valid preview data must identify exactly two pipe ids, require same system type, collinear overlap, same diameter, both opposite ends connected, branch connectivity preservation, fitting replacement permission, pipe-owned branch connectors, rollback preview, `commitBatchSize: 1`, heartbeat, timeout recovery, unchanged device connectivity, and zero clash increase. Native execution is intentionally not enabled in the starter executor yet; live Revit 2022 probes showed `PlumbingUtils.BreakCurve` can split a header in rollback, but branch/tee normalization can hang or fail when the existing branch is behind a pipe fitting. Dynamic commits for these connected overlap pairs are therefore treated as unsafe until a native per-pair normalizer is implemented.
