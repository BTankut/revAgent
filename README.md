...
`analyze_mep_system` includes read-only engineering foundations for connector summaries, optional targeted connector pathfinding, BOQ-only live collectors, hydronic resistance sampling, fan/pump basis calculations, and deterministic issue/design-log/BOQ/hydraulic report rows. `export_boq_report` and `export_clash_report` write-plans can export approved CSV/JSON report rows without mutating the Revit model. Engineering results with missing office standards remain proposals and return `canCommit: false`.
