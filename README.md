...
The Revit add-in command payload still provides low-level `send_code_to_revit`, selection, active-view commands, native command status, and the native `execute_write_plan` executor internally. Public domain-specific actions such as creating ducts or resizing pipes remain write-plan operations, not separate public MCP tools.

`get_revit_mcp_status` exposes the command gate/handshake state so operators can see whether the add-in is idle, which method is active, and which `planId`/mode is currently running. Normal model commands are serialized by the runtime gate and by the Revit add-in busy state to avoid overlapping ExternalEvent execution.

Endpoint stitching is guarded as write-plan data until a native executor command implements per-pair transaction finalization. `connect_ducts` and `connect_pipes` plans must identify exactly two elements, use `connectionMode: "endpoint_to_endpoint"`, `commitBatchSize: 1`, rollback preview evidence, heartbeat, timeout recovery, and post-commit audit. Live Revit 2022 testing showed raw dynamic connector/fitting commits can succeed only after strict candidate filtering; invalid same-direction or otherwise unsuitable pairs can timeout during commit finalization, so broad repeated dynamic endpoint stitching commits are treated as unsafe.

Connected same-direction pipe overlaps are guarded as a separate `normalize_pipe_header_overlap` write-plan operation. Valid preview data must identify exactly two pipe ids, require same system type, collinear overlap, same diameter or explicit `allowReducingBranch`, both opposite ends connected, branch connectivity preservation, fitting replacement permission, rollback preview, `commitBatchSize: 1`, heartbeat, timeout recovery, unchanged device connectivity, and zero clash increase. The native command now previews and commits one pair at a time, with transaction rollback, for direct fitting-to-branch pipe tees, reducing smaller-branch tees, short pipe-offset branch tees, and orphan open fitting cleanup. It projects a real pipe-owned branch connector to the header centerline, adjusts only collinear branch endpoints, deletes obsolete overlap/fitting/offset geometry, splits the header with `PlumbingUtils.BreakCurve`, and creates the tee through Revit API. Broad raw dynamic commits for these connected overlap pairs remain unsafe; use typed preview/commit plus post-commit audit.

The bundled add-in starts the socket service automatically after a Revit document opens. The `Revit MCP Switch` ribbon button remains available as a manual open/close fallback.

`analyze_mep_system` includes read-only engineering foundations for connector summaries, optional targeted connector pathfinding, BOQ-only live collectors, hydronic resistance sampling, HVAC/hydronic local-loss parameter extraction, fan/pump basis calculations, domain placement proposal handoff, domestic water/sanitary/storm/fire pipe sizing proposal handoff, and deterministic issue/design-log/BOQ/hydraulic/local-loss/local-loss-pressure report rows. Numeric local-loss pressure drops can be carried into fan pressure or pump head basis outputs as explicit additional loss inputs, and `localLossElementIds` can target a known critical-path/circuit element set instead of taking only first-sample category rows. Optional `placementRequests` for air terminals, dampers, valves, pumps, fire cabinets, or equipment become proposal-only `place_family_instance` write-plan steps and still require preview/approval/verify before any model write. Optional `domesticWaterPipeSizingRequests`, `sanitaryStormPipeSizingRequests`, and `firePipeSizingRequests` can produce proposal-only `resize_pipe` write-plan steps from complete demand and office sizing standards; fire steps are critical-risk and still require fire-engineer review. The tool also returns top-level `officeStandardsCompleteness`, `productionReadiness`, and `handoffValidation` gates so missing standards, incomplete proposal data, invalid handoff input, and invalid generated write-plan steps are visible before preview/commit. `productionReadiness.nextRequiredInputs` points to the exact office-standards, project-critical-data, or proposal-validation handoff needed to clear blockers. `officeStandardsCompleteness.officeStandardsInputTemplate` includes a machine-readable missing-key payload that can be filled and passed back as `analyze_mep_system.officeStandards`; the same handoff is available as `docs/revit-mep-office-standards-input-template.json`. Project-critical sizing data can be captured with `docs/revit-mep-project-critical-data-template.json`; filled handoffs are guarded by `build/tools/handoff_input_validator.js`, surfaced through `analyze_mep_system.handoffValidation`, and covered by `handoff_input_validator.test.js` before production review. `export_boq_report` and `export_clash_report` write-plans can export approved CSV/JSON report rows without mutating the Revit model. Engineering results with missing office standards remain proposals and return `canCommit: false`.
