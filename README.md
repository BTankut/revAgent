...
`analyze_mep_system` includes read-only engineering foundations for connector summaries, optional targeted connector pathfinding, BOQ-only live collectors, hydronic resistance sampling, HVAC/hydronic local-loss parameter extraction, fan/pump basis calculations, domain placement proposal handoff, domestic water and sanitary/storm pipe sizing proposal handoff, and deterministic issue/design-log/BOQ/hydraulic/local-loss/local-loss-pressure report rows. Numeric local-loss pressure drops can be carried into fan pressure or pump head basis outputs as explicit additional loss inputs, and `localLossElementIds` can target a known critical-path/circuit element set instead of taking only first-sample category rows. Optional `placementRequests` for air terminals, dampers, valves, pumps, fire cabinets, or equipment become proposal-only `place_family_instance` write-plan steps and still require preview/approval/verify before any model write. Optional `domesticWaterPipeSizingRequests` and `sanitaryStormPipeSizingRequests` can produce proposal-only `resize_pipe` write-plan steps from complete demand and office sizing standards. `export_boq_report` and `export_clash_report` write-plans can export approved CSV/JSON report rows without mutating the Revit model. Engineering results with missing office standards remain proposals and return `canCommit: false`.
