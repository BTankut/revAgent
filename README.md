...
Endpoint stitching is intentionally guarded as preview-only write-plan data until a native executor command implements per-pair transaction finalization. `connect_ducts` and `connect_pipes` plans must identify exactly two elements, use `connectionMode: "endpoint_to_endpoint"`, `commitBatchSize: 1`, rollback preview evidence, heartbeat, timeout recovery, and post-commit audit. Live Revit 2022 testing showed raw dynamic `Connector.ConnectTo` rollback probes can pass while actual commit finalization times out, so repeated dynamic endpoint stitching commits are treated as unsafe.
