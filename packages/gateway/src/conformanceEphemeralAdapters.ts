...
export class SqliteConformanceProtocolStore implements GatewayProtocolStore {
  ...
  async #runStartupExclusive<T>(work: () => Promise<StoreOutcome<T>>): Promise<StoreOutcome<T>> { return this.#withLock(async () => { let database: Database.Database; try { database = this.#db(); } catch { return storeFailure("unavailable", "conformance SQLite store is closed"); } const token = randomBytes(24).toString("hex"); const deadline = Date.now() + STARTUP_LOCK_WAIT_MS; let acquired = false; while (!acquired && Date.now() < deadline) { try { database.exec("BEGIN IMMEDIATE"); const lock = database.prepare<unknown[], { owner_token: string | null; lease_expires_at_ms: number; version: number }>("SELECT owner_token, lease_expires_at_ms, version FROM conformance_startup_lock WHERE id = 1").get(); if (lock !== undefined && (lock.owner_token === null || lock.lease_expires_at_ms <= Date.now())) { database.prepare("UPDATE conformance_startup_lock SET owner_token = ?, lease_expires_at_ms = ?, version = ? WHERE id = 1 AND version = ?").run(token, Date.now() + STARTUP_LOCK_LEASE_MS, lock.version + 1, lock.version); acquired = true; } database.exec("COMMIT"); } catch { try { if (database.inTransaction) database.exec("ROLLBACK"); } catch { } } if (!acquired) await new Promise<void>((resolve) => setTimeout(resolve, 25)); } if (!acquired) return storeFailure("unavailable", "conformance startup lease timed out"); try { return await work(); } finally { try { database.exec("BEGIN IMMEDIATE"); database.prepare("UPDATE conformance_startup_lock SET owner_token = NULL, lease_expires_at_ms = 0, version = version + 1 WHERE id = 1 AND owner_token = ?").run(token); database.exec("COMMIT"); } catch { try { if (database.inTransaction) database.exec("ROLLBACK"); } catch { } } } }); }
