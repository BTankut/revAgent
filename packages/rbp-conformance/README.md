...
It is deliberately an evidence scaffold, not a conformance result. New run
...
- `journal_snapshot` and optional `case_evidence` are strict
  `rbp-case-evidence/v1` JSON documents. Every required assertion must identify
  ...
  expected value, and actual value tuple.
...
- `leak_metrics` is `rbp-conformance-leaks/v1` JSON and must exactly match the
  report timing and fd/memory/journal/orphan-process counters.
