...
describe("canonical production bootstrap and external launcher", () => {
  ...
  const attestationRequestTimeoutContractMs = 30_000;

  it("times out and terminates a child that connects without a request", async () => {
    if (process.platform !== "win32") return;
    const launcherSource = readFileSync(launcher, "utf8");
    const readTimeoutFunction = launcherSource.match(
      /function Read-LineWithTimeout \{[\s\S]*?^\}/mu,
    )?.[0];
    expect(readTimeoutFunction).toContain(
      `$readTask.Wait(${String(attestationRequestTimeoutContractMs)})`,
    );
