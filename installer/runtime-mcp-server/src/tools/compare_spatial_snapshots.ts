...
import type { SpatialDiffInput } from "../spatial/spatialTypes.js";
import {
    getSpatialStore,
...
export function registerCompareSpatialSnapshotsTool(server: ToolServer) {
    server.tool(
        "compare_spatial_snapshots",
        "[SPATIAL_DIFF_READ_ONLY] Deterministically compare two explicit immutable complete snapshots with compatible scopes. Classifies added/removed elements, source availability, transforms/movement, geometry, properties, connectors, connectivity, and affected-neighborhood proximity changes. Stale or unknown snapshots remain valid historical inputs; the result cites both snapshot and revision ids and never claims current state. Partial snapshots and incompatible scopes fail closed. This tool never writes Revit.",
        {
