...
const budgetDefaults: Record<string, { maxElapsedMs: number; timeoutMs: number; maxMatches: number }> = {
...
function normalizeCountMode(value: unknown) {
    const normalized = String(value ?? "").trim();
    if (/^unique_?text$/i.test(normalized)) return "uniqueText";
    if (/^unique_?tag$/i.test(normalized)) return "uniqueTag";
    if (/^unique_?tagged_?element$/i.test(normalized)) return "uniqueTaggedElement";
    return "occurrence";
}

function isTagCountMode(countMode: string) {
    return countMode === "uniqueTag" || countMode === "uniqueTaggedElement";
}

function defaultGlobalCap(searchBudget: string, fast: number, balanced: number, deep: number) {
    if (searchBudget === "deep") return deep;
    if (searchBudget === "balanced") return balanced;
    return fast;
}

function resolveSources(args: JsonObject) {
...
function buildCountScanPolicy(args: JsonObject) {
...
function sourceTypeForAnnotationEvidence(row: JsonObject) {
...
function buildCountEvidenceRows(payload: JsonObject) {
...
function normalizeGroupKeyName(value: unknown) {
    const normalized = String(value ?? "").trim();
    if (/^source_?type$/i.test(normalized)) return "sourceType";
    if (/^(profile|profileName)$/i.test(normalized)) return "profile";
    if (/^(pattern|patternName)$/i.test(normalized)) return "pattern";
    if (/^(matchedCode|matchedText|uniqueText)$/i.test(normalized)) return "matchedText";
    if (/^tagFamilyType$/i.test(normalized)) return "tagFamilyType";
    if (/^(taggedElement|taggedElementId)$/i.test(normalized)) return "taggedElement";
    if (/^view$/i.test(normalized)) return "view";
    if (/^sheet$/i.test(normalized)) return "sheet";
    return normalized;
}

function groupFieldsForRow(row: JsonObject, groupBy: string[]) {
...
function groupKeyForFields(fields: JsonObject) {
...
function countTokenForRow(row: JsonObject, countMode: string) {
...
function computeFallbackCounts(rows: JsonObject[], countMode: string, groupBy: string[]) {
...
function groupByForPayload(payload: JsonObject, args?: JsonObject) {
...
function countModeForPayload(payload: JsonObject, args?: JsonObject) {
...
function buildCountSummary(payload: JsonObject, args?: JsonObject) {
...
function inferCountLastRead(payload: JsonObject) {
    const evidenceRows = buildCountEvidenceRows(payload);
...
function preserveCountCompatibilityFields(result: JsonObject, args?: JsonObject) {
...
export function normalizeCountAnnotationsResult(payload: JsonObject, args: JsonObject = {}, elapsedMs?: number) {
...
function buildGuardedInvalidCountMode(args: JsonObject) {
    return buildBroadScanGuardedResult({
        action: "count_annotations",
...
export function registerCountAnnotationsTool(server: ToolServer) {
    server.tool("count_annotations", "[ANNOTATION_COUNT_READ_ONLY] Read-only native Revit annotation inventory/count for DrawingSheet text notes, placed schedule cells, and viewport tag evidence. Use sheetQuery/sheetIds first; project-wide annotation counts require allowExpensiveSearch=true. Supports occurrence, uniqueText, uniqueTag, and uniqueTaggedElement count modes with bounded regex profiles.", {
        ...
        sources: z.array(z.enum(["sheet_text_notes", "placed_schedule_cells", "viewport_tags", "sheetTextNotes", "placedScheduleCells", "placedScheduleCell", "scheduleCells", "scheduleCell", "viewportTags"])).optional().describe("Annotation sources. Defaults to sheet_text_notes + placed_schedule_cells + viewport_tags except tag-specific count modes, which default to viewport_tags."),
