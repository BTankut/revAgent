...
function clampInt(value: unknown, fallback: number, minimum: number, maximum: number) {
...
function cleanPositiveIds(value: unknown) {
...
function cleanExactStrings(value: unknown) {
    return [...new Set((Array.isArray(value) ? value : [])
...
function resolveSourceScope(value: unknown) {
...
function resolveNameMatchMode(value: unknown) {
    return String(value ?? "") === "exact" ? "exact" : "contains";
}

export function buildInspectLevelsParams(args: JsonObject) {
    return {
...
function buildScanPolicy(args: JsonObject) {
...
function normalizeLinkedSourceLevelSelector(value: unknown) {
...
function normalizeLevelRow(row: JsonObject) {
    return {
...
function inferScanStoppedReason(payload: JsonObject) {
    if (unavailableSourceCount(payload) > 0) return "read_failed";
    if (readNativeResultField(payload, "truncated") === true) return "max_items";
...
export function normalizeInspectLevelsResult(payload: JsonObject, args: JsonObject, elapsedMs: number) {
    const levels = normalizedLevelRows(payload);
    ...
    server.tool(
        ...
        async (args) => {
            ...
            try {
                ...
                return formatJsonContent(normalizeInspectLevelsResult(
