...
import type { SpatialQueryInput } from "../spatial/spatialTypes.js";
import {
    getSpatialStore,
...
export function registerQuerySpatialContextTool(server: ToolServer) {
    server.tool(
        "query_spatial_context",
        "[SPATIAL_QUERY_READ_ONLY] Query one explicit, complete, currently-live spatial snapshot. mode=retrieve returns a bounded filtered subgraph; mode=operation runs one deterministic relation operation (relation_between, nearest_elements, elements_within, clearance_between, trace_connectivity, locate_in_space, or above_below). Geometry and topology are computed by the runtime, never by the LLM. Every operation echoes inputs and reports basis, precisionClass, verdictCapability, and evidence ids. Phase 1b clearance is context/screening evidence only and never a live clash or clearance verdict. This tool never writes Revit.",
        {
        ...
        async (args: JsonObject = {}) => {
            try {
                ...
                const input: SpatialQueryInput = args.mode === "retrieve"
                    ? {
