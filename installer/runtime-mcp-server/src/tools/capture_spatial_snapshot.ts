...
function cleanIntegerIds(values: unknown) {
    if (!Array.isArray(values)) {
        return [];
    }
    return [...new Set(values
...
export function resolveSpatialCapturePolicy(args: JsonObject = {}) {
    const pageTargetBytes = clampInteger(
        ...
        DEFAULT_SPATIAL_PAGE_TARGET_BYTES,
        MIN_SPATIAL_PAGE_TARGET_BYTES,
        MAX_SPATIAL_PAGE_TARGET_BYTES,
    );
    ...
    const timeoutMs = clampInteger(
        args.timeoutMs,
        Math.max(DEFAULT_SPATIAL_TIMEOUT_MS, maxElapsedMs + 15000),
        Math.max(1000, maxElapsedMs + 1000),
        MAX_SPATIAL_TIMEOUT_MS,
    );
    return {
        pageTargetBytes,
        maxElements,
...
export function buildSpatialCaptureParams(args: JsonObject, policy = resolveSpatialCapturePolicy(args)) {
    return {
        ...
        // A page call is read-only and remains visible in task history. Avoid a
        // modal completion window blocking the cursor-driven next page.
        suppressTaskStatusWindow: true,
        // Spatial model names/ids must never leak through Revit task history.
        // The outer MCP activity is still identifiable by the public tool name.
        taskName: "Capture spatial snapshot page",
        taskId: undefined,
    };
}

function hasExplicitLevelScope(params: ReturnType<typeof buildSpatialCaptureParams>) {
    return params.levelIds.length > 0 || params.levelNames.length > 0;
}

function guardedNeedsScope(policy: ReturnType<typeof resolveSpatialCapturePolicy>) {
    return {
        success: true,
        guarded: true,
        state: "guarded",
