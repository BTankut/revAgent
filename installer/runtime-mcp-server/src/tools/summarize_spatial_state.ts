...
import type { SpatialSummaryInput } from "../spatial/spatialTypes.js";
import {
    getSpatialStore,
...
import {
    probeSnapshotTrust,
    spatialToolFailure,
    MAX_SPATIAL_CONTEXT_TIMEOUT_MS,
} from "./spatial_context_tool_helpers.js";
...
export function registerSummarizeSpatialStateTool(server: ToolServer) {
    server.tool(
        "summarize_spatial_state",
        "[SPATIAL_SUMMARY_ADVISORY_READ_ONLY] Build a compact, deterministic per-level count/extent summary from one explicit complete and currently-live spatial snapshot. The result is advisory context only: advisory=true, verdictCapability=context_only, and quotableAsVerification=false. It never reports clash-free, clearance, occupancy-percentage, or other live verification claims and never writes Revit.",
        {
        ...
        async (args: JsonObject = {}) => {
            try {
                ...
                const input: SpatialSummaryInput = {
