...
import type { DuctingRules, EngineeringIssue, PointMm, ValidationResult } from "./types.js";

function pointsFromRoute(route: Record<string, unknown>): PointMm[] {
    const direct = valueByFields(route, ["pointsMm", "points_mm", "points"]);
    if (Array.isArray(direct)) {
        const points = direct.map(pointFromValue).filter((point): point is PointMm => !!point);
    ...
    if (Array.isArray(segments)) {
        ...
        for (const rawSegment of segments) {
            ...
            if (start && points.length === 0) points.push(start);
...
function lengthFromPoints(points: PointMm[]): number {
    let total = 0;
...
function elbowCount(points: PointMm[]): number {
    let count = 0;
...
export function validateRoutePreview(routeCandidates: Record<string, unknown>[] = [], rules: DuctingRules = {}): ValidationResult & { options: Record<string, unknown>[] } {
...
function graphArray(graph: Record<string, unknown>, field: string): Record<string, unknown>[] {
...
function edgeEnds(edge: Record<string, unknown>): [string | undefined, string | undefined] {
...
function summaryNumber(graph: Record<string, unknown>, fields: string[]): number | undefined {
    const summary = asRecord(graph.summary);
    return numberByFields(summary, fields) ?? numberByFields(graph, fields);
}

export function validateConnectedDuctNetwork(graphInput: Record<string, unknown> | undefined, expectedNodeIds: string[] = [], rules: DuctingRules = {}): ValidationResult {
    const issues: EngineeringIssue[] = [];
    ...
    const nodeIds = nodes.map(nodeId).filter((id): id is string => !!id);
...
function nativeSegments(input: Record<string, unknown> | Record<string, unknown>[] | undefined): Record<string, unknown>[] {
    if (!input) return [];
    if (Array.isArray(input)) return input.map(asRecord);
    const record = asRecord(input);
    if (Array.isArray(record.segments)) return record.segments.map(asRecord);
    if (Array.isArray(record.segmentResults)) return record.segmentResults.map(asRecord);
    return [record];
}

export function validateNativeSizing(input: Record<string, unknown> | Record<string, unknown>[] | undefined, rules: DuctingRules = {}): ValidationResult {
    const issues: EngineeringIssue[] = [];
    ...
    for (const segment of segments) {
        ...
        for (const [dimension, diff] of [["width", widthDiff], ["height", heightDiff], ["diameter", diameterDiff]] as const) {
...
function dimensionDiff(segment: Record<string, unknown>, dimension: "width" | "height" | "diameter"): number | undefined {
    ...
    if (design === undefined || native === undefined) return undefined;
