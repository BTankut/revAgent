...
export function summarizeSpatialExtractionTelemetryParams(params: JsonObject = {}, operationName?: unknown) {
    ...
    const operation = params.operation && typeof params.operation === "object" && !Array.isArray(params.operation)
        ? params.operation as JsonObject
        : {};
...
export function summarizeSpatialExtractionTelemetryResponse(response: any, error: any = null) {
    ...
    const continuationKindValue = String(
        getValueCaseInsensitiveLocal(object, ["continuationKind", "ContinuationKind"]) ?? "",
    ).trim().toLowerCase();
    const continuationKind = continuationKindValue === "work" ? "work" : null;
    const countArray = (...names: string[]) => {
    ...
    return {
        ...
        continuationKind,
        preparationPhase: safeSpatialCode(
            getValueCaseInsensitiveLocal(preparation, ["phase", "Phase"]),
            SPATIAL_WORK_PHASE_CODES,
        ),
        preparationStepOrdinal: coerceNumber(
            getValueCaseInsensitiveLocal(preparation, ["stepOrdinal", "StepOrdinal"]),
        ),
        preparationProcessed: coerceNumber(
            getValueCaseInsensitiveLocal(preparation, ["processed", "Processed"]),
        ),
        preparationTotal: coerceNumber(
            getValueCaseInsensitiveLocal(preparation, ["total", "Total"]),
        ),
        pageOrdinal,
        recordCount,
