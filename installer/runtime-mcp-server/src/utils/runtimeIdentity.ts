...
export function readJsonFile(filePath: string | undefined | null): any {
    try {
...
export function readUpdaterConfig(): any {
...
export function readInstalledState(extraCandidates: string[] = []): any {
    const installRoot = getInstallRoot();
    const candidates = [
...
export function parseBuildHash(version: unknown): string | null {
...
export function sanitizeTelemetryPathSegment(value: unknown, fallback = "unknown"): string {
    const text = String(value || "").trim();
    if (!text) {
        return fallback;
    }
