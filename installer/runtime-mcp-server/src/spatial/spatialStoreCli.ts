import {
    SpatialStore,
    type SpatialPurgeOptions,
    type SpatialPurgePreview,
} from "./spatialStore.js";

type SpatialStoreCliCommand = "preview" | "purge";

interface ParsedSpatialStoreCliArgs {
    command: SpatialStoreCliCommand;
    selector: SpatialPurgeOptions;
    selectorSummary: Record<string, unknown>;
    confirm: boolean;
}

interface SpatialStoreCliStore {
    previewPurge(options: SpatialPurgeOptions): SpatialPurgePreview;
    purge(options: SpatialPurgeOptions): ReturnType<SpatialStore["purge"]>;
    close(): void;
}

export interface SpatialStoreCliDependencies {
    createStore?: () => SpatialStoreCliStore;
}
...
function parseSpatialStoreCliArgs(args: readonly string[]): ParsedSpatialStoreCliArgs {
    ...
    for (let index = 1; index < args.length; index += 1) {
        ...
        if (arg === "--all") {
            if (all) throw new SpatialStoreCliUsageError("--all may be specified only once.");
            all = true;
            continue;
        }
        if (arg === "--confirm") {
            if (confirm) throw new SpatialStoreCliUsageError("--confirm may be specified only once.");
            confirm = true;
            continue;
        }
        if (arg === "--document-key") {
    ...
    if (selectorCount !== 1) {
        throw new SpatialStoreCliUsageError(
            "Exactly one selector is required: --all, --document-key <key>, or one or more --snapshot-id <id>.",
        );
    }
...
function previewResult(
    parsed: ParsedSpatialStoreCliArgs,
    preview: SpatialPurgePreview,
) {
    return {
...
export function runSpatialStoreCli(
    args: readonly string[],
    output: (value: unknown) => void = emitJson,
    dependencies: SpatialStoreCliDependencies = {},
): number {
    let store: SpatialStoreCliStore | null = null;
    try {
        ...
        try {
            ...
            // The operation result is already emitted. WAL recovery handles a
            // close failure on the next local store open.
        }
    }
}
