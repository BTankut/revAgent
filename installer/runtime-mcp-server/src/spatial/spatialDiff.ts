...
function hasIncomparableSourceSessions(
    baseSources: readonly SpatialSourceRevisionRecord[],
    headSources: readonly SpatialSourceRevisionRecord[],
): boolean {
    const base = sourceMap(baseSources);
    const head = sourceMap(headSources);
    const isSessionOnly = (source: SpatialSourceRevisionRecord) => {
    ...
    for (const [sourceKey, before] of [...base.entries()].sort(([left], [right]) => compareText(left, right))) {
        ...
        if (isSessionOnly(before) && (!after || before.documentSessionId !== after.documentSessionId)) return true;
        if (after && isSessionOnly(after) && before.documentSessionId !== after.documentSessionId) return true;
    }
    for (const [sourceKey, after] of [...head.entries()].sort(([left], [right]) => compareText(left, right))) {
        if (isSessionOnly(after) && !base.has(sourceKey)) return true;
    }
    return false;
}

function fingerprintVersions(nodes: readonly SpatialStoredNode[]): string[] {
...
function collectAllNodes(
    ...
    while (nodes.length < maximum) {
        ...
        if (!page.hasMore) return { nodes, truncated: false };
        if (!page.nextNodeId || page.nextNodeId === afterNodeId) return { nodes, truncated: true };
        afterNodeId = page.nextNodeId;
    }
    return { nodes, truncated: true };
}

function sourceMap(sources: readonly SpatialSourceRevisionRecord[]) {
...
function compareSources(
    baseSources: readonly SpatialSourceRevisionRecord[],
    headSources: readonly SpatialSourceRevisionRecord[],
): { availability: SpatialSourceChange[]; transforms: SpatialSourceChange[] } {
    const base = sourceMap(baseSources);
    const head = sourceMap(headSources);
    const availability: SpatialSourceChange[] = [];
    const transforms: SpatialSourceChange[] = [];
...
function propertyFields(node: SpatialStoredNode): Record<string, unknown> {
    return {
...
function changedFieldNames(before: Record<string, unknown>, after: Record<string, unknown>): string[] {
...
function nodeChange(
    before: SpatialStoredNode,
    after: SpatialStoredNode,
...
function hasAabbOnlyShapeEvidence(node: SpatialStoredNode): boolean {
...
interface ProximityCollection {
    relations: Map<string, SpatialComputedRelation>;
    truncated: boolean;
    candidateCount: number;
}

function relationPairKey(firstNodeId: string, secondNodeId: string): string {
    return firstNodeId < secondNodeId
        ? `${firstNodeId}\u001f${secondNodeId}`
        : `${secondNodeId}\u001f${firstNodeId}`;
}

function collectAffectedProximity(
    store: SpatialStore,
    snapshotId: string,
    ...
    const relations = new Map<string, SpatialComputedRelation>();
    let truncated = false;
    ...
    for (const nodeId of [...new Set(affectedNodeIds)].sort(compareText)) {
        ...
        while (candidateCount < maximumCandidates) {
            ...
            for (const candidate of page.nodes) {
                if (candidate.nodeId === node.nodeId) continue;
                ...
                if (relation && relation.separationMm <= radiusMm) relations.set(key, relation);
            }
...
function compareProximity(
    base: ProximityCollection,
    head: ProximityCollection,
    maximumOutputChanges: number,
): { changes: SpatialComputedRelation[]; truncated: boolean; observedChangeCount: number } {
    const changes: SpatialComputedRelation[] = [];
...
function compareSpatialSnapshotsCore(store: SpatialStore, input: SpatialDiffInput): SpatialSnapshotDiffResult {
    ...
    for (const policyKey of ["coordinateFrame", "lengthUnit", "captureConsistency"] as const) {
        ...
        if (basePolicy && headPolicy && basePolicy !== headPolicy) {
            return guarded(
                "incomparable_scopes",
                `Snapshot diff requires a common ${policyKey} policy.`,
                [`base_${policyKey}:${basePolicy}`, `head_${policyKey}:${headPolicy}`],
            );
        }
    }
    if (hasIncomparableSourceSessions(baseSnapshot.sourceRevisions, headSnapshot.sourceRevisions)) {
        return guarded(
            "incomparable_scopes",
            "Session-only document identities cannot be diffed across different documentSessionId values.",
        );
    }
    ...
    if (baseCapability.adapter !== headCapability.adapter) {
        return guarded(
            ...
            "Mixed v0.2/v0.3 diffs are unsupported because native v0.3 and legacy-derived fingerprints do not share a comparable algorithm basis.",
            [...baseCapability.limitations, ...headCapability.limitations],
        );
    }
    ...
    if (legacy && input.allowLegacyV02 !== true) {
        return guarded(
            ...
            "A full Phase 1b diff requires v0.3 snapshots. Set allowLegacyV02 only for an explicitly capability-limited historical diff.",
            [...baseCapability.limitations, ...headCapability.limitations],
        );
    }
    if (!legacy) {
        ...
        const topologyComplete = (value: typeof baseTopology) => Boolean(value
            && value.readComplete
        ...
        if (!topologyComplete(baseTopology) || !topologyComplete(headTopology)) {
            return guarded(
                ...
                "A full v0.3 diff requires complete connector reads and committed-snapshot membership validation in both snapshots.",
                [
    ...
    if (baseNodes.truncated || headNodes.truncated) {
        return guarded(
            "max_items",
            "The bounded snapshot diff could not load every node; no incomplete diff was presented as complete.",
    ...
    if (!legacy) {
        ...
        if (baseVersionInvalid || headVersionInvalid || commonVersionMismatch) {
            return guarded(
                "snapshot_capability_mismatch",
                "A precise v0.3 diff requires one common fingerprints.version across every node in both snapshots.",
                [
    ...
    const added: SpatialStoredNode[] = [];
    const removed: SpatialStoredNode[] = [];
    const moved: SpatialNodeChange[] = [];
    const geometryChanges: SpatialNodeChange[] = [];
    const geometryIndeterminate: SpatialNodeChange[] = [];
    const propertyChanges: SpatialNodeChange[] = [];
    const connectorChanges: SpatialNodeChange[] = [];
    const connectivityChanges: SpatialNodeChange[] = [];
    const sourceAvailabilityChanges: SpatialSourceChange[] = [];
    const transformChanges: SpatialSourceChange[] = [];
    let truncated = false;
    ...
    function offerChange<T>(bucket: T[], value: T, observedKey: string): void {
    ...
    const affectedHeadIds = new Set<string>();
    ...
    for (const nodeId of [...new Set([...baseById.keys(), ...headById.keys()])].sort(compareText)) {
        ...
        if (!legacy && !shapeChanged && aabbOnlyEvidence && geometryEvidenceChanged) {
            affectedBaseIds.add(nodeId);
            affectedHeadIds.add(nodeId);
            offerChange(geometryIndeterminate, nodeChange(
                before,
                after,
                before.geometryFingerprint,
                after.geometryFingerprint,
                ["aabb_or_geometry_fingerprint"],
            ), "geometryIndeterminate");
        }
        if (propertyChanged) {
            const beforeFields = propertyFields(before);
            const afterFields = propertyFields(after);
            offerChange(propertyChanges, nodeChange(
                before,
                after,
                beforeProperty,
                afterProperty,
                changedFieldNames(beforeFields, afterFields),
            ), "property");
        }
        if (!legacy && before.nodeKind === "connector" && (movedChanged || shapeChanged || propertyChanged)) {
            offerChange(
                connectorChanges,
                nodeChange(before, after, before.geometryFingerprint, after.geometryFingerprint),
                "connector",
            );
        }
        if (topologyChanged) {
            offerChange(
                connectivityChanges,
                nodeChange(before, after, beforeTopology, afterTopology),
                "connectivity",
            );
        }
    }
    ...
    const proximity = compareProximity(
        collectAffectedProximity(
            store,
            baseSnapshotId,
            [...affectedBaseIds],
            proximityRadiusMm,
            maximumProximityPairs,
            maximumProximityCandidates,
        ),
        collectAffectedProximity(
            store,
            headSnapshotId,
        ...
        Math.max(0, maximumChanges - returnedChangeCount),
    );
...
export function compareSpatialSnapshots(store: SpatialStore, input: SpatialDiffInput): SpatialSnapshotDiffResult {
    ...
    if (!result.guarded && Buffer.byteLength(canonicalJson(result), "utf8") > MAX_DIFF_RESPONSE_BYTES) {
        return guarded(
            "max_bytes",
            "The bounded diff result exceeded the runtime response budget; lower maxChanges or narrow the snapshot scope.",
            [`max_response_bytes:${MAX_DIFF_RESPONSE_BYTES}`],
        );
    }
    return result;
}
