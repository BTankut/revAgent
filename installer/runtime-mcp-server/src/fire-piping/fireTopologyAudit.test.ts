...
const riserValveEdge = tree.orientedEdges.find((edge: any) => edge.edgeId === "e-riser-valve");
assert.equal(riserValveEdge.fromNodeId, "riser-1");
assert.equal(riserValveEdge.toNodeId, "valve-1");
assert.equal(tree.solverAdapter.links.filter((link: any) => link.lengthMm === 6000).length, 1);
assert.equal(tree.solverAdapter.links.filter((link: any) => link.lengthMm === 4000).length, 1);
