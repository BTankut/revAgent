import { cleanText, finiteNumber, firstDefined, isJsonObject, sha256Canonical, } from "./spatialCanonical.js";
...
function parseProfile(payload) {
    ...
    return {
        ...
        widthMm,
        heightMm,
        insulationThicknessMm: nonNegative(profile.insulationThicknessMm
...
export function spatialGeometry(node) {
    ...
    const centerline = Array.isArray(centerlineObject?.points)
        ? centerlineObject.points.map(point3).filter((item) => item !== null)
        : [];
    ...
    const boundaryLoops = Array.isArray(geometry.boundaryLoops)
        ? geometry.boundaryLoops
            ...
            .map((loop) => loop.map(point3).filter((item) => item !== null))
...
export function inflateAabb(aabb, distanceMm) {
...
export function mergeAabbs(left, right) {
    if (!left)
        return right;
    if (!right)
        return left;
...
export function aabbRelation(sourceNodeId, targetNodeId, source, target) {
    const gaps = [
...
function polylineDistance(first, second) {
...
function roundOuterRadius(profile) {
...
export function relationBetweenNodes(source, target) {
...
export function aboveBelowRelation(source, target, toleranceMm = 0) {
    if (!source.aabb || !target.aabb)
        return null;
...
function segmentIntersectsBoundaryWithinVerticalExtent(segmentStart, segmentEnd, boundaryStart, boundaryEnd, extent) {
    ...
    const zAt = (parameter) => segmentStart[2] + (segmentEnd[2] - segmentStart[2]) * parameter;
    const parameterHasVerticalOverlap = (minimum, maximum) => {
        const lower = Math.max(0, Math.min(minimum, maximum));
        const upper = Math.min(1, Math.max(minimum, maximum));
        if (lower > upper + EPSILON_MM)
            return false;
    ...
    if (Math.abs(crossRs) > EPSILON_MM) {
        ...
        if (segmentParameter < -EPSILON_MM || segmentParameter > 1 + EPSILON_MM
            || boundaryParameter < -EPSILON_MM || boundaryParameter > 1 + EPSILON_MM)
            return false;
    ...
    if (Math.abs(crossQr) > EPSILON_MM)
        return false;
...
function segmentOverlapsVerticalExtent(start, end, extent) {
...
function clippedSegmentSamplesForVerticalExtent(start, end, extent) {
...
function appendUniqueSample(samples, candidate) {
    if (samples.some((sample) => sample.every((coordinate, axis) => Math.abs(coordinate - candidate[axis]) <= EPSILON_MM)))
        return;
    samples.push(candidate);
}
export function locateNodeInSpace(element, space) {
    ...
    for (let index = 1; index < elementGeometry.centerline.length; index += 1) {
        ...
        for (const sample of clippedSamples)
            appendUniqueSample(samples, sample);
        ...
        if (crossesVerticalBoundary && clippedSamples.some((sample) => pointInLoops2d(sample, spaceGeometry.boundaryLoops) !== "outside")) {
    ...
    for (const sample of samples) {
        ...
        if (!verticallyInside || horizontal === "outside")
            outsideSampleCount += 1;
    ...
    for (let index = 1; index < elementGeometry.centerline.length && !segmentBoundaryCrossing; index += 1) {
        const start = elementGeometry.centerline[index - 1];
        const end = elementGeometry.centerline[index];
        if (!segmentOverlapsVerticalExtent(start, end, space.aabb))
            continue;
        for (const loop of spaceGeometry.boundaryLoops) {
            for (let edgeIndex = 0; edgeIndex < loop.length; edgeIndex += 1) {
                if (segmentIntersectsBoundaryWithinVerticalExtent(start, end, loop[edgeIndex], loop[(edgeIndex + 1) % loop.length], space.aabb)) {
                    segmentBoundaryCrossing = true;
                    break;
                }
            }
            if (segmentBoundaryCrossing)
                break;
        }
    }
        ...
        for (let index = 0; index < elementLoop.length && !segmentBoundaryCrossing; index += 1) {
            ...
            for (const spaceLoop of spaceGeometry.boundaryLoops) {
                for (let edgeIndex = 0; edgeIndex < spaceLoop.length; edgeIndex += 1) {
                    if (segmentIntersectsBoundaryWithinVerticalExtent(start, end, spaceLoop[edgeIndex], spaceLoop[(edgeIndex + 1) % spaceLoop.length], space.aabb)) {
                        segmentBoundaryCrossing = true;
                        break;
                    }
                }
...
function normalizedPoints(points, anchor) {
    return points.map((point) => subtract(point, anchor));
}
export function derivedPlacementFingerprint(node) {
    if (node.placementFingerprint)
        return node.placementFingerprint;
...
export function derivedShapeFingerprint(node) {
    if (node.shapeFingerprint)
        return node.shapeFingerprint;
    const geometry = spatialGeometry(node);
    const anchor = geometry.point ?? geometry.centerline[0] ?? (geometry.aabb ? aabbCenter(geometry.aabb) : [0, 0, 0]);
...
export function derivedPropertyFingerprint(node) {
    if (node.propertyFingerprint)
        return node.propertyFingerprint;
    return sha256Canonical({
...
export function derivedTopologyFingerprint(node) {
    if (node.topologyFingerprint)
        return node.topologyFingerprint;
    const peers = firstDefined(node.payload, [
        ["connectedToNodeIds"],
        ["peerNodeIds"],
        ["topology", "connectedToNodeIds"],
    ]);
    if (node.nodeKind !== "connector" && !Array.isArray(peers))
        return null;
    return sha256Canonical({
