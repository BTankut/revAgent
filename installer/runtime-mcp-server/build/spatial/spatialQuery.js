import { aboveBelowRelation, inflateAabb, locateNodeInSpace, mergeAabbs, relationBetweenNodes, spatialGeometry, } from "./spatialGeometry.js";
import { traceSpatialConnectivity } from "./spatialTopology.js";
import { buildSpatialQueryFingerprint, decodeSpatialQueryCursor, encodeSpatialQueryCursor, SpatialQueryCursorError, } from "./spatialQueryCursor.js";
...
export function spatialCapabilityCoverage(schemaVersion) {
...
function guarded(reason, message, warnings = [], scanPolicy = {}, snapshotCoverage) {
    return {
        success: true,
        guarded: true,
        state: "guarded",
...
function queryWarnings(input, capability) {
    return [...new Set([
...
function requiresIndexedV03Filters(filters) {
...
function hasRetrieveScope(filters) {
    if (!filters)
        return false;
...
function toStoreQuery(snapshotId, filters, limit, afterNodeId) {
    return {
        snapshotId,
...
function intersectAabbs(left, right) {
    const minMm = [
...
function prepareRetrieveFilters(store, snapshotId, filters) {
    ...
    if (withinSpaceNodeIds.length === 0)
    ...
    for (const space of spaces) {
        ...
        if (!space.aabb || geometry.boundaryLoops.length === 0) {
            return guarded("unsupported_geometry", "withinSpaceNodeIds requires stored boundary loops and vertical extents for every selected space.");
        }
        union = mergeAabbs(union, space.aabb);
    }
    if (!union)
        return { storeFilters, spaces, empty: true };
...
function queryRetrieveNodePage(store, snapshotId, prepared, limit, afterNodeId) {
    if (prepared.empty) {
    ...
    while (selected.length < limit && scannedNodeCount < maximumScannedNodes) {
        const page = store.queryStoredNodes(toStoreQuery(snapshotId, prepared.storeFilters, Math.min(1_000, maximumScannedNodes - scannedNodeCount), scanAfterNodeId));
        for (let index = 0; index < page.nodes.length; index += 1) {
            const node = page.nodes[index];
            scannedNodeCount += 1;
            scanAfterNodeId = node.nodeId;
            if (prepared.spaces.some((space) => space.nodeId === node.nodeId))
                continue;
            const containments = prepared.spaces.map((space) => locateNodeInSpace(node, space));
            if (containments.some((containment) => containment.status === "inside" || containment.status === "boundary"))
                selected.push(node);
        ...
        if (page.nodes.length > 0)
            scanAfterNodeId = page.nodes.at(-1).nodeId;
        hasMore = true;
    }
    if (scannedNodeCount >= maximumScannedNodes)
        hasMore = true;
    return {
        nodes: selected,
        hasMore,
...
function collectStoredNodes(store, snapshotId, filters, maximum) {
    const nodes = [];
    let afterNodeId = null;
    let truncated = false;
    while (nodes.length < maximum) {
        const page = store.queryStoredNodes(toStoreQuery(snapshotId, filters, Math.min(1_000, maximum - nodes.length), afterNodeId));
...
function collectPreparedStoredNodes(store, snapshotId, prepared, maximum) {
    const nodes = [];
    let afterNodeId = null;
    let truncated = false;
    while (nodes.length < maximum) {
        const page = queryRetrieveNodePage(store, snapshotId, prepared, Math.min(1_000, maximum - nodes.length), afterNodeId);
        if (page.unsupportedNodeId) {
            return {
                nodes: [],
                truncated: false,
                guarded: guarded("unsupported_geometry", "within-space filtering encountered an AABB-only candidate that cannot be classified as inside or outside.", [`unsupported_node_id:${page.unsupportedNodeId}`]),
            };
        }
        nodes.push(...page.nodes);
        if (!page.hasMore)
...
function operationNodePair(store, snapshotId, sourceNodeId, targetNodeId) {
    const ids = [cleanText(sourceNodeId), cleanText(targetNodeId)];
    if (ids.some((value) => !value))
        return null;
    const nodes = store.getStoredNodesByIds(snapshotId, ids);
    const byId = new Map(nodes.map((node) => [node.nodeId, node]));
    const source = byId.get(ids[0]);
    const target = byId.get(ids[1]);
    return source && target ? [source, target] : null;
}
function operationRequirements(operation, capability, snapshotComplete) {
    if (["nearest_elements", "elements_within", "locate_in_space", "trace_connectivity"].includes(operation.name)
        && !snapshotComplete) {
        return guarded("incomplete_snapshot", `${operation.name} requires a complete snapshot because missing nodes could change the deterministic result.`);
    }
    if (operation.name === "trace_connectivity" && !capability.topology) {
        return guarded("unsupported_snapshot_capability", "trace_connectivity requires a v0.3 snapshot with explicit connector peer topology.", capability.limitations);
    }
    if (operation.name === "clearance_between" && !capability.analyticClearance) {
        return guarded("unsupported_snapshot_capability", "clearance_between requires a v0.3 snapshot with explicit supported profile dimensions.", capability.limitations);
    }
    return null;
}
function operationResultBase(snapshot, input, capability, startedAt) {
    return {
        success: true,
        guarded: false,
        state: "completed",
...
function runPairOperation(store, snapshotId, operation) {
...
function runNeighborOperation(store, snapshotId, operation) {
    ...
    if (!anchor || !anchor.aabb || distanceMm === null || distanceMm < 0)
        return null;
    ...
    if (candidates.guarded)
        return { guarded: candidates.guarded };
...
function runLocateOperation(store, snapshotId, operation, capability) {
    const node = store.getStoredNode(snapshotId, operation.nodeId);
    if (!node || !node.aabb)
        return null;
    const explicitSpaceIds = (operation.spaceNodeIds ?? []).map(cleanText).filter((value) => value !== null);
    if (capability.adapter === "legacy_v02" && explicitSpaceIds.length === 0) {
        return { guarded: guarded("needs_scope", "Legacy v0.2 containment requires explicit spaceNodeIds because indexed spatial-role projections were introduced in v0.3.", capability.limitations) };
    }
    ...
    if (computed.length > 0 && computed.every((row) => row.containment.status === "unsupported")) {
        return { guarded: guarded("unsupported_geometry", "locate_in_space requires stored point, centerline, or boundary evidence plus a readable space boundary; AABB-only containment is not supported.") };
    }
    return {
...
function querySpatialContextCore(store, input) {
    const startedAt = Date.now();
    const snapshotId = cleanText(input.snapshotId);
    if (!snapshotId)
        return guarded("needs_scope", "query_spatial_context requires an explicit snapshotId.");
    const snapshot = store.getSnapshotRecord(snapshotId);
    if (!snapshot)
        return guarded("snapshot_not_found", `Spatial snapshot ${snapshotId} was not found in the local store.`);
    const capability = spatialCapabilityCoverage(snapshot.schemaVersion);
    if (!capability) {
        return guarded("unsupported_snapshot_schema", `Spatial snapshot schema ${snapshot.schemaVersion} is not supported by the Phase 1b runtime adapter.`);
    }
    if (!completeSnapshot(snapshot)) {
        return guarded("incomplete_snapshot", "Deterministic spatial queries require a complete, non-partial snapshot with coverageStatus=complete.", [
    ...
    if (input.requireCurrent && input.trust?.liveness !== "current") {
        return guarded("snapshot_not_current", "The requested current-state query requires a live liveness probe returning current.", queryWarnings(input, capability));
    }
    if (input.mode === "retrieve") {
        if (!hasRetrieveScope(input.filters)) {
            return guarded("needs_scope", "retrieve mode requires an explicit node, category, role, level, system, AABB, elevation-band, or within-space filter; whole-snapshot dumps are not supported.");
        }
        if (capability.adapter === "legacy_v02" && requiresIndexedV03Filters(input.filters)
            ...
            return guarded("unsupported_snapshot_capability", "Legacy v0.2 metadata filters require explicit nodeIds; v0.3 adds indexed property projections.", capability.limitations);
        }
        ...
        let nodePageStartAfterNodeId = null;
        let expectedNodePageEndId = null;
        let afterEdgeId = null;
        try {
            if (input.cursor) {
        ...
        if (page.unsupportedNodeId) {
            return guarded("unsupported_geometry", "within-space retrieval encountered an AABB candidate without point, centerline, or boundary evidence; the node was not silently classified outside.", [`unsupported_node_id:${page.unsupportedNodeId}`]);
        }
        const edgeContinuation = expectedNodePageEndId !== null;
        if (edgeContinuation && page.pageEndNodeId !== expectedNodePageEndId) {
            return guarded("invalid_cursor", "The immutable snapshot did not reproduce the node page bound to this edge continuation cursor.");
        }
    ...
    if (["relation_between", "clearance_between", "above_below"].includes(operation.name)) {
        const pair = runPairOperation(store, snapshotId, operation);
        if (!pair)
            return guarded("node_not_found_or_geometry_unsupported", "Both explicit nodes and supported stored geometry are required.");
    ...
    if (operation.name === "nearest_elements" || operation.name === "elements_within") {
        ...
        if (!result)
            return guarded("node_not_found_or_geometry_unsupported", "The anchor node, a bounded distance, and stored AABB geometry are required.");
        if (result.guarded)
    ...
    if (operation.name === "trace_connectivity") {
        ...
        if (!topology || !topology.readComplete || !topology.targetMembershipValidated
            ...
            return guarded("incomplete_topology_coverage", "trace_connectivity requires complete connector reads and committed-snapshot validation for every peer reference.", [
        ...
        if (result.unsupportedOwnerNodeIds.length > 0 && result.reachedTarget !== true) {
            return guarded("internal_topology_unsupported", "The trace reached a multi-port owner whose internal connector routing cannot be proven from same-system/domain evidence or an explicit pass-through category.", result.unsupportedOwnerNodeIds.map((ownerNodeId) => `unsupported_owner_node_id:${ownerNodeId}`));
        }
        return {
            ...base,
            operation: operation.name,
    ...
    if (operation.name === "locate_in_space") {
        ...
        if (!result)
            return guarded("node_not_found_or_geometry_unsupported", "locate_in_space requires an existing node with stored geometry.");
        if ("guarded" in result && result.guarded)
            return result.guarded;
        return {
            ...base,
            operation: operation.name,
...
export function querySpatialContext(store, input) {
    ...
    if (!result.guarded && Buffer.byteLength(canonicalJson(result), "utf8") > MAX_QUERY_RESPONSE_BYTES) {
        return guarded("max_bytes", "The bounded spatial query result exceeded the runtime response budget; narrow filters or lower item/depth limits.", [], { maxResponseBytes: MAX_QUERY_RESPONSE_BYTES });
    }
    return result;
}
