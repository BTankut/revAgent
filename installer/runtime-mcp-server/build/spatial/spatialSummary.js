...
function legacyIndexedFilterRequested(input) {
...
function summarizeSpatialStateCore(store, input) {
    const startedAt = Date.now();
    const snapshotId = cleanText(input.snapshotId);
    if (!snapshotId)
        return guarded("needs_scope", "summarize_spatial_state requires an explicit snapshotId.");
    ...
    if (!snapshot.complete || snapshot.partial || snapshot.coverageStatus !== "complete") {
        return guarded("incomplete_snapshot", "Spatial state summaries require a complete, non-partial snapshot with coverageStatus=complete.", [
    ...
    if (input.requireCurrent && input.trust?.liveness !== "current") {
        return guarded("snapshot_not_current", "The requested current-state summary requires a live liveness probe returning current.", warnings);
    }
    if (capability.adapter === "legacy_v02" && legacyIndexedFilterRequested(input)
        && (!input.filters?.nodeIds || input.filters.nodeIds.length === 0)) {
        return guarded("unsupported_snapshot_capability", "Legacy v0.2 summary metadata filters require explicit nodeIds.", warnings);
    }
    ...
    for (const node of nodes) {
        ...
        if (input.includeSystems !== false)
            increment(level.nodesBySystem, node.systemKey);
    ...
    if (orderedLevels.length > maximumLevels)
        truncated = true;
    const resultLevels = orderedLevels.slice(0, maximumLevels).map((level) => ({
...
export function summarizeSpatialState(store, input) {
    ...
    if (!result.guarded && Buffer.byteLength(canonicalJson(result), "utf8") > MAX_SUMMARY_RESPONSE_BYTES) {
        return guarded("max_bytes", "The advisory summary exceeded the runtime response budget; narrow filters or lower maxLevels.", [`max_response_bytes:${MAX_SUMMARY_RESPONSE_BYTES}`]);
    }
    return result;
}
