...
function parseCapturedAt(value, fallbackMs) {
...
function mapNode(value) {
    ...
    const nodeRef = isObject(readField(node, "nodeRef")) ? readField(node, "nodeRef") : node;
    const elementRef = isObject(readField(node, "elementRef"))
        ? readField(node, "elementRef")
        : isObject(readField(nodeRef, "elementRef"))
            ? readField(nodeRef, "elementRef")
            : {};
...
function mapSourceRevision(value) {
...
function retryableInterruption(payload) {
...
function unknownLiveness(reason, now) {
    return {
        liveness: "unknown",
...
function unwrapProbePayload(value) {
    let current = isObject(value) ? value : {};
    for (let depth = 0; depth < 3; depth += 1) {
        if (readField(current, "success") !== undefined || !isObject(readField(current, "result"))) {
            break;
        }
        current = readField(current, "result");
    }
    return current;
}
export function validateSpatialLivenessProbe(rawProbe, expectedSources, now = Date.now) {
    ...
    const aggregate = rowLiveness.includes("unknown")
        ? "unknown"
...
export async function probeStoredSpatialSnapshotLiveness(store, snapshotId, probe, now = Date.now) {
    let sourceRevisions;
    try {
...
function nonNegativeCountMap(value) {
    if (!isObject(value)) {
        return null;
    }
    const result = {};
...
function resolveCommitEvidence(payload) {
    ...
    const unmaterializedOmissionsByClassification = nonNegativeCountMap(readField(coverage, "unmaterializedOmissionsByClassification"));
...
function contractFailure(message, details, scanPolicy, elapsedMs) {
    return {
        success: false,
        guarded: false,
        state: "failed",
...
function workContractFailure(message, details, scanPolicy, elapsedMs) {
    return {
...
export async function captureSpatialSnapshotAtomic(input, dependencies) {
    ...
    const maxWorkSteps = Math.max(1, Math.min(SPATIAL_CAPTURE_MAX_WORK_STEPS, dependencies.maxWorkSteps ?? SPATIAL_CAPTURE_MAX_WORK_STEPS));
    const maxCaptureElapsedMs = Math.max(1_000, Math.min(MAX_SPATIAL_CAPTURE_MAX_ELAPSED_MS, input.maxCaptureElapsedMs ?? DEFAULT_SPATIAL_CAPTURE_MAX_ELAPSED_MS));
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
        ...
        let captureId = "";
        let expected = null;
        let expectedWorkInvariant = null;
        let previousPageHash = null;
        let previousWorkCursor = null;
        let previousWorkPhase = null;
        let previousWorkProcessed = null;
        let previousWorkTotal = null;
        let expectedOrdinal = 0;
        let workStepCount = 0;
        let staged = false;
        let interruptedThisAttempt = false;
        const nodeIds = new Set();
        const pageRoundTripsMs = [];
        const nativeUiOccupancyMs = [];
        const preparationRoundTripsMs = [];
        const preparationNativeUiOccupancyMs = [];
            ...
            for (let requestOrdinal = 0; requestOrdinal < maxPages + maxWorkSteps; requestOrdinal += 1) {
                ...
                if (isSpatialWorkContinuationCandidate(rawPayload)) {
                    ...
                    if (staged || expectedOrdinal > 0) {
                        dependencies.store.abandonCapture(captureId);
                        return workContractFailure("Spatial preparation resumed after data-page staging had already started.", { expectedOrdinal, workStepCount }, input.scanPolicy, now() - overallStartedAt);
                    }
                    const preparation = rawPayload.preparation;
                    ...
                    if (expectedWorkInvariant && canonicalJson(workInvariant) !== canonicalJson(expectedWorkInvariant)) {
                        return workContractFailure("Spatial preparation capture/scope/source-binding metadata changed inside one capture.", {
                    ...
                    if (invalidStep || invalidPhase || invalidSamePhaseProgress || !nextWorkCursor || nextWorkCursor === previousWorkCursor) {
                        return workContractFailure("Spatial preparation cursor, phase, or progress monotonicity failed.", {
                    ...
                    if (workStepCount >= maxWorkSteps) {
                        return workContractFailure("Spatial preparation exceeded the hard work-continuation bound.", { maxWorkSteps }, input.scanPolicy, now() - overallStartedAt);
                    }
                    workStepCount += 1;
                ...
                if ((captureId && receivedCaptureId !== captureId)
                    ...
                    if (captureId && staged)
                        dependencies.store.abandonCapture(captureId);
                    return workContractFailure("The first spatial data page did not preserve the prepared capture/source-binding invariant.", {
                ...
                if (!commitEvidence) {
                    dependencies.store.abandonCapture(captureId);
                    return contractFailure("Final spatial counts/coverage could not be reconciled into atomic commit expectations.", { counts: payload.counts, coverage: payload.coverage, payloadBytes: payload.payloadBytes }, input.scanPolicy, now() - overallStartedAt);
                }
                ...
                const liveness = await probeStoredSpatialSnapshotLiveness(dependencies.store, summary.snapshotId, dependencies.probeLiveness, now);
