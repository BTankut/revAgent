...
function node(report, nodeId) {
    const found = report.nodes.find((item) => item.nodeId === nodeId);
    assert.ok(found, `Expected node '${nodeId}' in audit report.`);
    return found;
}
function sizing(report, nodeId) {
    const found = report.sizingAudit.find((item) => item.nodeId === nodeId);
...
assert.equal(node(tree, "riser-1").distanceFromSource, 0);
...
assert.ok(tree.reducerReport.transitions.some((item) => item.status === "reducer_present"));
...
const riserValveEdge = tree.orientedEdges.find((edge) => edge.edgeId === "e-riser-valve");
assert.equal(riserValveEdge.fromNodeId, "riser-1");
assert.equal(riserValveEdge.toNodeId, "valve-1");
assert.equal(tree.solverAdapter.links.filter((link) => link.lengthMm === 6000).length, 1);
assert.equal(tree.solverAdapter.links.filter((link) => link.lengthMm === 4000).length, 1);
