...
function segmentHitsObstacle(start, end, obstacle) {
    if (!obstacleAppliesAtZ(obstacle, start.z) && !obstacleAppliesAtZ(obstacle, end.z))
        return false;
...
function createCoordinateGrid(source, target, obstacles, bounds, gridStepMm, marginMm, routeZ) {
...
function findGridPath(source, target, obstacles, bounds, gridStepMm, marginMm, maxExpansions) {
    ...
    if (pointBlocked(source) || pointBlocked(target))
...
function buildRoute(source, target, obstacles, options) {
