import { aabbFromValue, asRecord, makeIssue, numberByFields, round, stringByFields, validationStatus, } from "./helpers.js";
function normalizeSystem(value) {
    const text = String(value ?? "").trim().toLowerCase();
    if (["supply", "sa", "besleme"].includes(text))
        return "supply";
    if (["return", "ra", "donus"].includes(text))
        return "return";
    if (["exhaust", "ea", "egzoz"].includes(text))
        return "exhaust";
    return undefined;
}
export function normalizeDiffuserCatalog(input = []) {
    const result = [];
    input.forEach((raw, index) => {
        ...
        if (!system || maxFlowLps === undefined || maxFlowLps <= 0)
            return;
        result.push({
...
function flowForSystem(assignment, system) {
    if (system === "supply")
        return assignment.flows.supplyLps;
    if (system === "return")
        return assignment.flows.returnLps;
    return assignment.flows.exhaustLps;
}
function selectDiffuserType(system, totalAirflowLps, catalog, rules) {
    const issues = [];
    ...
    for (const type of available) {
        for (let count = 1; count <= maxDiffusers; count++) {
            ...
            if (perDiffuser < type.minFlowLps || perDiffuser > type.maxFlowLps)
                continue;
            ...
            if (!best || score < best.score)
                best = { type, count, flow: perDiffuser, score };
        }
    }
    if (!best) {
...
function candidateGrid(space, count, rules) {
    const issues = [];
...
function findPlenum(space, plenums) {
    return plenums.find((plenum) => {
        ...
        if (sourceSpace && sourceSpace === space.id)
            return true;
...
function validatePlenum(space, candidates, plenums, intersections, rules) {
    const issues = [];
...
export function buildDiffuserPlans(assignments, catalogInput = [], rules = {}, plenumVolumes = [], plenumObstacleIntersections = []) {
    ...
    const plans = assignments.map((assignment) => {
        ...
        const candidates = [];
        const planIssues = [];
        for (const system of ["supply", "return", "exhaust"]) {
