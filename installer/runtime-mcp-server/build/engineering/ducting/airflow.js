import { aabbFromValue, asRecord, makeIssue, numberByFields, pointFromValue, stringByFields, validationStatus, } from "./helpers.js";
const EMPTY_FLOW = { supplyLps: 0, returnLps: 0, exhaustLps: 0 };
function flowFromRow(row, kind) {
...
export function normalizeAirBalanceRows(input = []) {
    return input.map((raw, index) => {
        const record = asRecord(raw);
        return {
...
function addIndex(index, indexKey, row) {
    if (!indexKey.replace(/\|/g, ""))
        return;
    const rows = index.get(indexKey) ?? [];
    rows.push(row);
    index.set(indexKey, rows);
}
function getUnique(index, indexKey) {
    const rows = index.get(indexKey);
    return rows && rows.length === 1 ? rows[0] : undefined;
}
export function buildAirflowAssignments(spacesInput = [], rowsInput = []) {
    const spaces = normalizeSpaces(spacesInput);
    const rows = normalizeAirBalanceRows(rowsInput);
    const issues = [];
    const byId = new Map();
    const byNumberLevel = new Map();
...
export function summarizeAirflow(assignments) {
    const totals = assignments.reduce((acc, assignment) => {
        ...
        if (assignment.flows.supplyLps + assignment.flows.returnLps + assignment.flows.exhaustLps > 0)
            acc.spacesWithAirflow++;
        return acc;
