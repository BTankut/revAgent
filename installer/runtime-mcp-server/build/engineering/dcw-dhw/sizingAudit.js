...
function readFixtureDemand(node) {
    const generic = firstDefined(readEngineering(node, "fixtureUnits", "FixtureUnits"), readProperties(node, PROPERTY_KEY_ALIASES.genericFixtureUnits));
...
function collectTopologyFindings(graph) {
    ...
    return toArray(readField(topology, "findings", "Findings")).map((item) => finding(String(readField(item, "severity", "Severity") || "warning").toLowerCase(), readField(item, "code", "Code") || "topology_finding", readField(item, "message", "Message") || "Connector graph topology finding.", {
