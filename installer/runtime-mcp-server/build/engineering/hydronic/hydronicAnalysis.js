...
export function calculateVelocityMps(flowLps, diameterM) {
...
export function calculateReynoldsNumber(flowLps, diameterM, densityKgM3, dynamicViscosityPaS) {
...
export function calculateDarcyFrictionFactor(reynoldsNumber, diameterM, roughnessMm) {
    ...
    const denominator = Math.pow(Math.log10(relativeRoughness / 3.7 + 5.74 / Math.pow(reynoldsNumber, 0.9)), 2);
    return 0.25 / denominator;
}
export function equivalentLengthFromK(kValue, diameterM, frictionFactor) {
...
export function analyzeHydronicPipingGraph(rawGraph, options = {}) {
    ...
    const segmentCalculations = [];
    const segmentPressureById = new Map();
    const missingSegmentIds = new Set();
    for (const node of graph.nodes) {
...
function normalizeGraph(rawGraph, roleOverrides) {
    ...
    const nodes = rawNodes.map((raw, index) => normalizeNode(raw, index, roleOverrides));
    const nodeById = new Map(nodes.map((node) => [node.id, node]));
    const edges = rawEdges.map((raw, index) => normalizeEdge(raw, index)).filter((edge) => edge !== null);
    return { nodes, edges, nodeById };
}
function normalizeNode(raw, index, roleOverrides) {
...
function classifyRole(parts) {
    ...
    if (/pump|circulator|pompa|sirkulasyon/.test(text))
        return "pump";
    if (/fan coil|\bfcu\b|coil|serpantin|batarya|heat exchanger|esanj/.test(text))
        return "coil";
    if (/balanc|balans|commissioning valve|circuit setter/.test(text))
        return "balancing_valve";
    if (/control valve|2-way|3-way|two way|three way|motorized|actuat/.test(text))
        return "control_valve";
    if (/valve|vana|check valve|shut.?off|ball valve|gate valve|butterfly/.test(text))
        return "valve";
    if (/pipe fitting|fitting|elbow|tee|reducer|dirsek|te\b|transition/.test(text))
        return "fitting";
    if (/pipe accessory|accessory|strainer|filter|separator|meter|sensor|flex|compensator|expansion/.test(text))
        return "accessory";
    if (/\bpipe curves\b|\bpipe segment\b|\bpiping segment\b|\bboru\b/.test(text) || text.includes("ost_pipecurves"))
        return "pipe_segment";
    if (/mechanical equipment|terminal|unit heater|radiator|radiant|convect/.test(text))
        return "terminal";
    return "unknown";
}
function calculateSegment(kind, id, label, role, lengthM, diameterM, flowLps, equivalentLengthM, kValue, roughnessMm, hazenWilliamsC, options, source, target) {
...
function resolveNodeFlow(node, downstreamDemand, upstreamDemand, adjacency) {
...
function isTerminalDemandNode(node, adjacency) {
    if (!node)
        return false;
    if (node.role === "coil" || node.role === "terminal")
        return true;
    const outgoing = adjacency.get(node.id) || [];
    return outgoing.length === 0 && node.role !== "pump" && node.role !== "pipe_segment" && node.role !== "fitting";
}
function resolveStartNodes(nodes, pumps, reverseAdjacency, startNodeIds) {
    const explicit = (startNodeIds || []).map((id) => String(id));
    if (explicit.length > 0)
        return explicit;
    if (pumps.length > 0)
        return pumps.map((node) => node.id);
...
function resolveEndNodes(nodes, adjacency, endNodeIds) {
    const explicit = (endNodeIds || []).map((id) => String(id));
    if (explicit.length > 0)
        return explicit;
    const terminals = nodes.filter((node) => node.role === "coil" || node.role === "terminal");
    if (terminals.length > 0)
        return terminals.map((node) => node.id);
    return nodes.filter((node) => (adjacency.get(node.id) || []).length === 0).map((node) => node.id);
}
function enumerateHydronicPaths(starts, ends, nodeById, adjacency, segmentPressureById, missingSegmentIds, maxPaths) {
    const endSet = new Set(ends);
    const paths = [];
    let truncated = false;
    const pathCost = (nodeIds, edgeIds) => {
    ...
    for (const start of starts) {
        const visit = (current, nodePath, edgePath, visited) => {
...
function selectCriticalPath(paths) {
    if (paths.length === 0)
        return null;
...
function summarizeTerminalPaths(paths, criticalPath, nodeById) {
    if (!criticalPath)
        return [];
    const byTerminal = new Map();
    for (const path of paths) {
        if (!path.terminalNodeId)
            continue;
...
function findDirectedCycles(nodes, adjacency, maxCycles) {
    const cycles = [];
    const visited = new Set();
    const stack = new Set();
    const path = [];
    const visit = (nodeId) => {
        ...
        for (const edge of adjacency.get(nodeId) || []) {
            if (!visited.has(edge.target)) {
                ...
                if (startIndex >= 0)
                    cycles.push([...path.slice(startIndex), edge.target]);
            }
        }
        stack.delete(nodeId);
        path.pop();
...
function findPossibleReversedDirections(starts, ends, adjacency, reverseAdjacency) {
    const findings = [];
    for (const start of starts) {
        const forwardReach = reachableFrom(start, adjacency);
        for (const end of ends) {
            if (forwardReach.has(end))
                continue;
...
function equipmentSummary(node) {
    return {
        node_id: node.id,
        role: node.role,
        label: node.label,
...
function readEquivalentLengthM(raw) {
...
function readFlowLps(raw) {
    ...
    if (positive(flow) && unit) {
        if (/l\/?s|liter/.test(unit))
            return flow;
        if (/m3\/?h|m\^3\/?h|cubic meters per hour/.test(unit))
            return flow / 3.6;
        if (/m3\/?s|m\^3\/?s|cubic meters per second/.test(unit))
            return flow * 1000;
        if (/gpm/.test(unit))
            return flow * 0.0630902;
    }
    return null;
}
...
function firstDefinedPath(source, paths) {
    for (const path of paths) {
        const value = getPath(source, path);
