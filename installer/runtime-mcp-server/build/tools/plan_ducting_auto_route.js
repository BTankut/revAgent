...
export function registerPlanDuctingAutoRouteTool(server) {
    server.tool("plan_ducting_auto_route", "Plan dry-run duct routes from source points to target points using a 3D obstacle-aware grid (A*). Supports multi-elevation routing with vertical risers when allowedElevationsMm contains more than one value. Produces routeCandidates that can be reviewed by evaluate_ducting_design. This tool never writes Revit elements.", {
        ...
        obstacles: z.array(passthroughObject).optional().describe("Obstacle AABBs from spatial extraction. Each obstacle is an object with an `id` plus an AABB field. Accepted AABB formats: `{aabbMm: {minX,minY,minZ,maxX,maxY,maxZ}}`, `{aabbMm: [minX,minY,minZ,maxX,maxY,maxZ]}`, `{aabb_mm: {min:[x,y,z], max:[x,y,z]}}`. Merged with spatialZone.obstacles by id (user-provided wins)."),
        ...
        ductHalfWidthMm: z.number().nonnegative().optional().describe("Half-width used for horizontal (X/Y) obstacle clearance filtering in mm. Obstacles are expanded by `clearanceMm + ductHalfWidthMm` on X/Y so the duct centerline keeps its physical half-width away from the obstacle face. Defaults to 0 (preserves legacy behaviour where the caller bakes the half-width into `clearanceMm`)."),
        ductHalfHeightMm: z.number().nonnegative().optional().describe("Half-height used for vertical (Z) obstacle clearance filtering in mm. Obstacles are expanded by `clearanceMm + ductHalfHeightMm` on Z. Defaults to 150."),
