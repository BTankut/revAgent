import{McpServer as nm}from"@modelcontextprotocol/sdk/server/mcp.js";import{StdioServerTransport as rm}from"@modelcontextprotocol/sdk/server/stdio.js";import{z as Pe}from"zod";import*as Gr from"net";var tn=32*1024*1024,nn=class{host;port;socket;logErrors;isConnected=!1;responseCallbacks=new Map;buffer=Buffer.alloc(0);framingMode=process.env.REVIT_MCP_FRAMING==="legacy"?"legacy":"length-prefixed";constructor(t,n,r={}){this.host=t,this.port=n,this.logErrors=r.logErrors!==!1,this.socket=new Gr.Socket,this.setupSocketListeners()}setupSocketListeners(){this.socket.on("connect",()=>{this.isConnected=!0}),this.socket.on("data",t=>{this.buffer=Buffer.concat([this.buffer,t]),this.processBuffer()}),this.socket.on("close",()=>{this.isConnected=!1}),this.socket.on("error",t=>{this.logErrors&&console.error("RevitClientConnection error:",t),this.isConnected=!1})}processBuffer(){for(;this.buffer.length>0;){if(this.buffer.length>tn){this.rejectPending(new Error(`revAgent response exceeded ${tn} bytes`)),this.buffer=Buffer.alloc(0);return}if(this.isLikelyLegacyJson(this.buffer)){if(!this.processLegacyJsonBuffer())return;continue}if(!this.isLikelyLengthPrefixed(this.buffer)||!this.processLengthPrefixedBuffer())return}}isLikelyLegacyJson(t){let n=0;for(;n<t.length&&[32,9,10,13].includes(t[n]);)n++;return n<t.length&&t[n]===123}isLikelyLengthPrefixed(t){if(t.length<4)return!0;let n=t.readUInt32BE(0);return n>0&&n<=tn}processLegacyJsonBuffer(){try{let t=this.buffer.toString("utf8"),n=this.extractFirstJsonObject(t);if(!n)return!1;let r=JSON.parse(n.json);return this.handleResponseObject(r,n.json),this.buffer=Buffer.from(n.remaining,"utf8"),!0}catch{return!1}}extractFirstJsonObject(t){let n=0,r=!1,o=!1,i=!1,a=0;for(let s=0;s<t.length;s++){let l=t[s];if(!i){if(/\s/.test(l))continue;if(l!=="{")return null;i=!0,a=s,n=1;continue}if(o){o=!1;continue}if(l==="\\"){o=!0;continue}if(l==='"'){r=!r;continue}if(!r){if(l==="{")n++;else if(l==="}"&&(n--,n===0))return{json:t.slice(a,s+1),remaining:t.slice(s+1)}}}return null}processLengthPrefixedBuffer(){if(this.buffer.length<4)return!1;let t=this.buffer.readUInt32BE(0);if(t<=0||t>tn)return this.rejectPending(new Error(`Invalid revAgent response frame length: ${t}`)),this.buffer=Buffer.alloc(0),!1;if(this.buffer.length<4+t)return!1;let r=this.buffer.subarray(4,4+t).toString("utf8");try{let o=JSON.parse(r);this.handleResponseObject(o,r)}catch(o){this.rejectPending(new Error(`Failed to parse revAgent response: ${o instanceof Error?o.message:String(o)}`))}return this.buffer=this.buffer.subarray(4+t),!0}handleResponseObject(t,n){let o=t&&t.id!==void 0&&t.id!==null?String(t.id):"default",i=this.responseCallbacks.get(o);if(i){i(n),this.responseCallbacks.delete(o);return}if(t&&t.error&&this.responseCallbacks.size===1){let a=this.responseCallbacks.entries().next().value;if(a){let[s,l]=a;l(n),this.responseCallbacks.delete(s)}return}if(t&&t.error&&this.responseCallbacks.size>1)for(let[a,s]of this.responseCallbacks.entries())s(n),this.responseCallbacks.delete(a)}rejectPending(t){for(let[n,r]of this.responseCallbacks.entries())r(JSON.stringify({jsonrpc:"2.0",id:n,error:{code:-32e3,message:t instanceof Error?t.message:String(t)}})),this.responseCallbacks.delete(n)}connect(){if(this.isConnected)return!0;try{return this.socket.connect(this.port,this.host),!0}catch(t){return console.error("Failed to connect:",t),!1}}disconnect(){this.socket.end(),this.isConnected=!1}generateRequestId(){return Date.now().toString()+Math.random().toString().substring(2,8)}async sendCommand(t,n={},r={}){return t!=="mcp_status"&&r.statusPreflight!==!1&&await this.ensureReadyForCommand(t,r),await this.sendCommandRequest(t,n,r)}async ensureReadyForCommand(t,n={}){let r=n.statusTimeoutMs||Math.min(n.timeoutMs||3e3,3e3),o=await this.sendCommandRequest("mcp_status",{},{timeoutMs:r,statusPreflight:!1}),i=o&&typeof o=="object"?o.activeTask:null;if(!i)return;let a=i.taskName||i.method||"revAgent task",s=typeof i.elapsedMs=="number"?`, elapsed ${this.formatElapsed(i.elapsedMs)}`:"";throw new Error(`revAgent is busy with "${a}"${s}. Wait for it to finish before sending "${t}".`)}formatElapsed(t){let n=Math.max(0,Math.floor(t/1e3)),r=Math.floor(n/3600),o=Math.floor(n%3600/60),i=n%60;return[r,o,i].map(a=>String(a).padStart(2,"0")).join(":")}async sendCommandRequest(t,n={},r={}){let o=r.framing||this.framingMode;try{return await this.sendCommandRequestOnce(t,n,{...r,framing:o})}catch(i){if(o==="length-prefixed"&&r.allowLegacyFallback!==!1&&this.isFramingFallbackError(i))return this.framingMode="legacy",await this.sendCommandRequestOnce(t,n,{...r,framing:"legacy"});throw i}}isFramingFallbackError(t){let n=t instanceof Error?t.message:String(t);return/Invalid JSON|Invalid JSON-RPC request|Invalid (?:Revit MCP|revAgent) response frame length/i.test(n)}sendCommandRequestOnce(t,n={},r={}){return new Promise((o,i)=>{let a;try{this.isConnected||this.connect();let s=this.generateRequestId(),l={jsonrpc:"2.0",method:t,params:n,id:s};this.responseCallbacks.set(s,h=>{clearTimeout(a);try{let m=JSON.parse(h);m.error?i(new Error(m.error.message||"Unknown error from Revit")):o(m.result)}catch(m){m instanceof Error?i(new Error(`Failed to parse response: ${m.message}`)):i(new Error(`Failed to parse response: ${String(m)}`))}}),this.writeCommand(l,r.framing||this.framingMode);let u=r.timeoutMs||12e4;a=setTimeout(()=>{this.responseCallbacks.has(s)&&(this.responseCallbacks.delete(s),i(new Error(`Command timed out after ${this.formatElapsed(u)}: ${t}`)))},u),typeof a.unref=="function"&&a.unref()}catch(s){clearTimeout(a),i(s)}})}writeCommand(t,n){let r=Buffer.from(JSON.stringify(t),"utf8");if(n==="length-prefixed"){let o=Buffer.alloc(4);o.writeUInt32BE(r.length,0),this.socket.write(Buffer.concat([o,r]));return}this.socket.write(r)}};import*as ie from"fs";import*as Gn from"os";import*as Mt from"path";var Ma=process.env.REVIT_MCP_HOST||process.env.REVIT_HOST||"localhost",Ur=Fe(process.env.REVIT_MCP_PORT||process.env.REVIT_PORT,8080),Jr=process.env.REVIT_MCP_INSTANCE_REGISTRY||Mt.join(Gn.tmpdir(),"revit-mcp-instances.json"),$r=Mt.join(Gn.tmpdir(),"revit-mcp-command-locks"),Qr=8e3,Na=600*1e3,Ea=250;function ka(e){return new Promise(t=>setTimeout(t,e))}function Fe(e,t){if(e==null||e===""){if(t!==void 0)return t;throw new Error("Invalid revAgent port: empty value")}let n=Number.parseInt(String(e),10);if(!Number.isFinite(n)||n<1||n>65535)throw new Error(`Invalid revAgent port: ${e}`);return n}function Hr(e){return e?(Array.isArray(e)?e:String(e).split(",")).map(n=>String(n).trim()).filter(Boolean).map(n=>Fe(n)):[]}function He(e){return e?String(e).trim():Ma}function Pa(e){return String(e).replace(/[^a-zA-Z0-9_.-]/g,"_")}function Aa(e){return Mt.join($r,`${Pa(e.host)}-${e.port}.lock`)}function Xr(e){return e&&typeof e=="object"&&"code"in e?String(e.code):null}function Oa(e){let t=new Set,n=[];for(let r of e){let o=He(r.host),i=Fe(r.port),a=`${o}:${i}`;t.has(a)||(t.add(a),n.push({...r,host:o,port:i}))}return n}function Yr(){try{if(!ie.existsSync(Jr))return[];let e=JSON.parse(ie.readFileSync(Jr,"utf8"));if(Array.isArray(e))return e;if(e&&Array.isArray(e.instances))return e.instances;if(e&&e.targets&&typeof e.targets=="object")return Object.entries(e.targets).map(([t,n])=>({...typeof n=="object"&&n?n:{},name:t}))}catch{}return[]}function Va(e,t){let n=String(t).toLowerCase();return[e.name,e.id,e.target,e.pid,e.title,e.documentTitle,e.path,e.pathName].filter(o=>o!=null).some(o=>String(o).toLowerCase()===n)}function Da(e){let t=Yr().find(n=>Va(n,e));return t?{name:t.name||t.id||String(e),host:He(t.host),port:Fe(t.port),source:"registry",metadata:t}:null}function Fa(e,t){let n=String(e||"").trim();if(!n)return null;if(/^\d+$/.test(n))return{host:He(t),port:Fe(n),source:"target-port"};let r=n.match(/^(.+):(\d+)$/);return r?{host:He(r[1]),port:Fe(r[2]),source:"target-host-port"}:null}function La(e={}){let t=He(e.host),n=e.port!==void 0&&e.port!==null?Fe(e.port):null;if(n)return{host:t,port:n,source:"explicit"};let r=e.target||process.env.REVIT_MCP_TARGET;if(r){let o=Fa(r,t);if(o)return o;let i=Da(r);if(i)return i;throw new Error(`Unknown revAgent target '${r}'. Use a port number, host:port, or a registered instance name.`)}return{host:t,port:Ur,source:"default"}}function Kr(e={}){let t=He(e.host),n=[];if(e.includeRegistry!==!1)for(let a of Yr())a.port&&n.push({name:a.name||a.id||a.title||a.documentTitle,host:He(a.host),port:Fe(a.port),source:"registry",metadata:a});let r=Hr(e.ports),o=Hr(process.env.REVIT_MCP_PORTS),i=o.length>0?o:[Ur,8081,8082,8083,8084,8085];for(let a of r.length>0?r:i)n.push({host:t,port:a,source:r.length>0?"explicit":"scan"});return Oa(n)}function ja(e){try{let t=ie.statSync(e);Date.now()-t.mtimeMs>Na&&ie.rmSync(e,{recursive:!0,force:!0})}catch(t){if(!t||Xr(t)==="ENOENT")return}}async function Ba(e,t=Qr){let n=Aa(e),r=Date.now();for(ie.mkdirSync($r,{recursive:!0});;)try{return ie.mkdirSync(n,{recursive:!1}),ie.writeFileSync(Mt.join(n,"owner.json"),JSON.stringify({pid:process.pid,startedAt:new Date().toISOString(),target:e},null,2)),()=>{try{ie.rmSync(n,{recursive:!0,force:!0})}catch{}}}catch(o){if(!o||Xr(o)!=="EEXIST")throw o;if(ja(n),Date.now()-r>=t)throw new Error(`revAgent target ${e.host}:${e.port} is busy; a previous Revit command is still running. Refusing to send another request.`);await ka(Ea)}}async function be(e,t={}){let n=La(t),r=t.skipLock===!0?()=>{}:await Ba(n,t.lockWaitMs||Qr),o=new nn(n.host,n.port,{logErrors:t.logSocketErrors!==!1});try{return o.isConnected||await new Promise((i,a)=>{let s,l=()=>{o.socket.removeListener("connect",l),o.socket.removeListener("error",u),clearTimeout(s),i()},u=()=>{o.socket.removeListener("connect",l),o.socket.removeListener("error",u),clearTimeout(s),a(new Error(`connect to revAgent target ${n.host}:${n.port} failed`))};o.socket.on("connect",l),o.socket.on("error",u),o.connect(),s=setTimeout(()=>{o.socket.removeListener("connect",l),o.socket.removeListener("error",u),a(new Error(`connect to revAgent target ${n.host}:${n.port} timed out`))},t.connectTimeoutMs||5e3),typeof s.unref=="function"&&s.unref()}),await e(o,n)}finally{o.disconnect(),r()}}import tr from"node:crypto";import nr from"node:os";import ut from"node:path";var za=[{name:"Parameter.Set",pattern:/\.Set\s*\(/i},{name:"Parameter.SetValueString",pattern:/\.SetValueString\s*\(/i},{name:"Parameter.ClearValue",pattern:/\.ClearValue\s*\(/i},{name:"Schedule.SetCellText",pattern:/\.\s*SetCellText\s*\(/i},{name:"Schedule table edit",pattern:/\.\s*(InsertRow|RemoveRow|InsertColumn|RemoveColumn|SetCellStyle|SetMergedCell)\s*\(/i},{name:"Document.Delete",pattern:/\.\s*Delete\s*\(/i},{name:"ElementTransformUtils",pattern:/ElementTransformUtils/i},{name:"Location.Move",pattern:/\.Move\s*\(/i},{name:"Element.ChangeTypeId",pattern:/\.ChangeTypeId\s*\(/i},{name:"Connector.ConnectTo",pattern:/\.ConnectTo\s*\(/i},{name:"Connector.DisconnectFrom",pattern:/\.DisconnectFrom\s*\(/i},{name:"FamilySymbol.Activate",pattern:/\.Activate\s*\(/i},{name:"NewFamilyInstance",pattern:/NewFamilyInstance/i},{name:"Create API",pattern:/\.(Create|New[A-Z]\w*)\s*\(/},{name:"View visibility/overrides",pattern:/\.(HideElements|UnhideElements|HideElementsTemporary|IsolateElementsTemporary|SetElementOverrides)\s*\(/i},{name:"Geometry join/cut",pattern:/(JoinGeometryUtils|SolidSolidCutUtils|InstanceVoidCutUtils|PartUtils)/i},{name:"Parameter binding edit",pattern:/\.(ParameterBindings|ParameterMap)\s*\.\s*(Insert|ReInsert|Remove)\s*\(/i},{name:"Revit property assignment",pattern:/\b(document|doc|element|view|view3d|targetView|activeView|familyInstance|instance|symbol|level|parameter|param|location)\s*\.\s*(Pinned|Name|Scale|ViewTemplateId|CropBox|CropBoxActive|CropBoxVisible|SketchPlane|Curve|Point)\s*=/i},{name:"Manual Transaction",pattern:/new\s+(Transaction|SubTransaction|TransactionGroup)\s*\(|(Transaction|SubTransaction|TransactionGroup)\s*\(/i}];function Nt(e){return za.filter(t=>t.pattern.test(e)).map(t=>t.name)}import Jn from"node:fs";import me from"node:path";import{fileURLToPath as qa}from"node:url";function lt(e){return/^(1|true|yes|on)$/i.test(String(e||"").trim())}function ct(e){try{return!e||!Jn.existsSync(e)?null:JSON.parse(Jn.readFileSync(e,"utf8").replace(/^\uFEFF/,""))}catch{return null}}function Hn(){let e=qa(import.meta.url),t=me.dirname(e),n=[me.resolve(t,"..",".."),me.resolve(t,"..")];for(let r of n)if(Jn.existsSync(me.join(r,"package.json")))return r;return n[0]}function Zr(){let e=Hn(),t=me.dirname(e);return t&&t!==e?t:e}function Et(){return process.env.ProgramData||process.env.PROGRAMDATA||"C:\\ProgramData"}function eo(){let e=Zr(),t=[process.env.REVAGENT_UPDATER_CONFIG,me.join(e,"updater","updater-config.json"),me.join(Et(),"DPE","revAgent","updater","updater-config.json"),me.join(Et(),"DPE","RevitMCP","updater","updater-config.json")].filter(Boolean);for(let n of t){let r=ct(n);if(r)return r}return null}function kt(e=[]){let t=Zr(),n=[me.join(t,"updater","installed.json"),...e,me.join(Et(),"DPE","revAgent","updater","installed.json"),me.join(Et(),"DPE","RevitMCP","updater","installed.json")];for(let r of n){let o=ct(r);if(o)return o}return null}function Pt(e){let t=String(e||"").match(/-([0-9a-f]{7,40})$/i);return t?t[1]:null}function to(){return me.join(Et(),"DPE","revAgent","state","telemetry")}function Ue(e){return(String(e||"").trim()||"unknown-machine").toUpperCase()}function rn(e,t="unknown"){let n=String(e||"").trim();return n&&n.replace(/[<>:"/\\|?*\x00-\x1F\s]+/g,"_").replace(/_+/g,"_").replace(/^[._-]+|[._-]+$/g,"")||t}import sn from"node:fs";import no from"node:path";var on=new Map,an=new Map,At=0,Un=0;async function ro(e,t){await sn.promises.mkdir(no.dirname(e),{recursive:!0}),await sn.promises.writeFile(e,`${JSON.stringify(t,null,2)}
`,"utf8")}async function $n(e,t){await sn.promises.mkdir(no.dirname(e),{recursive:!0}),await sn.promises.appendFile(e,`${JSON.stringify(t)}
`,"utf8")}function oo(e,t){let r=(on.get(e)||Promise.resolve()).catch(()=>{}).then(()=>$n(e,t));return on.set(e,r),r.finally(()=>{on.get(e)===r&&on.delete(e)}).catch(()=>{}),r}function Qn(e,t,n){if(n.disabled())return!1;if(At>=n.maxInFlight())return Un++,!1;At++;let o=(an.get(e)||Promise.resolve()).catch(()=>{}).then(()=>t(e));return an.set(e,o),o.catch(()=>{Un++}).finally(()=>{an.get(e)===o&&an.delete(e),At=Math.max(0,At-1)}),!0}function io(e){return{inFlight:At,dropped:Un,maxInFlight:e}}var Wa=new Set(["completed","failed","guarded"]);function Ot(e,t,n){return e?.[n]!==void 0&&e?.[n]!==null?e[n]:t?.[n]??null}function ln(e,t){return e??t??null}function Vt(e){return String(e?.state||"").toLowerCase()}function Yn(e){return Wa.has(String(e||"").toLowerCase())}function ao(e){return e!=null&&e!==""}function so(e){let t=Date.parse(String(e?.finishedAtUtc||e?.startedAtUtc||""));return Number.isFinite(t)?t:0}function Ga(e,t){let n=Yn(t?.state),r=Yn(e?.state);return n?t||null:r?e||null:t||e||null}function Ja(e,t){return Vt(t)==="failed"?t||null:Vt(e)==="failed"&&e||null}function Xn(e,t,n,r){let o=String(e||"").toLowerCase(),i=Vt(n)===o,a=Vt(t)===o;return i&&a?Ot(n,t,r):i?Ot(n,null,r):a?Ot(t,null,r):null}function Ha(e,t=""){if(!e||typeof e!="object")return t;if(ao(e.requestId))return`request:${e.requestId}`;if(ao(e.id))return`id:${e.id}`;let n=e.method||"",r=e.taskName||"",o=e.startedAtUtc||"";return n||r||o?`task:${n}|${r}|${o}`:t}function Ua(e,t){let n=Ga(e,t),r={...e||{},...t||{}};for(let o of["id","requestId","method","wrapperAction","logicalToolName","taskName","parentTaskName","parentTaskId","startedAtUtc","requestBytes","responseBytes","port"])r[o]=Ot(t,e,o);return r.state=ln(n?.state,Ot(t,e,"state")),Yn(r.state)?(r.finishedAtUtc=ln(Xn(r.state,e,t,"finishedAtUtc"),n?.finishedAtUtc),r.elapsedMs=ln(Xn(r.state,e,t,"elapsedMs"),n?.elapsedMs)):(r.finishedAtUtc=null,r.elapsedMs=null),Vt(r)==="failed"?r.error=ln(Xn(r.state,e,t,"error"),Ja(e,t)?.error):r.error=null,r}function $a(e,t,n=100){let r=Math.max(1,Math.min(200,Number(n)||100)),o=new Map,i=(a,s)=>{for(let[l,u]of(Array.isArray(a)?a:[]).entries()){if(!u||typeof u!="object")continue;let h=Ha(u,`${s}:${l}`),m=o.get(h);o.set(h,m?Ua(m,u):u)}};return i(t,"cached"),i(e,"current"),[...o.values()].sort((a,s)=>so(s)-so(a)).slice(0,r)}function lo(e,t){let n=e&&typeof e=="object"?e:null,r=t&&typeof t=="object"?t:null;if(!n&&!r)return null;let o=n?.recentHistoryCapacity??r?.recentHistoryCapacity??100,i=$a(n?.recentTasks,r?.recentTasks,o),a=Math.max(Number(n?.recentHistoryCount)||0,Number(r?.recentHistoryCount)||0,i.length);return{...r||{},...n||{},activeTask:n?.activeTask||null,recentTasks:i,recentHistoryCount:a,recentHistoryCapacity:o}}var Qa="revagent.telemetry.v1",Xa="revagent.live.status.v1",ho="revagent.live.activity.v1",fn=tr.randomUUID(),fo=new Date().toISOString(),Ya=0,Dt=new Map,$e=[],go=null,cn=null,co=null;function rr(){return lt(process.env.REVAGENT_TELEMETRY_DISABLED)}function Ka(e){return tr.createHash("sha256").update(String(e||""),"utf8").digest("hex")}function dt(e){return Ka(e).slice(0,16)}function un(e,t=400){let n=String(e||"");return n.length<=t?{text:n,truncated:!1}:{text:`${n.slice(0,t)}...[truncated ${n.length-t} chars]`,truncated:!0}}function Za(e){return String(e||"").split(/\r\n|\r|\n/).length}function Qe(e,t,n,r){let o=Number.parseInt(String(e??""),10);return Number.isFinite(o)?Math.max(n,Math.min(r,o)):t}function es(){return Qe(process.env.REVAGENT_TELEMETRY_TEXT_CHARS,1e3,0,1e4)}function ts(){return Qe(process.env.REVAGENT_TELEMETRY_CODE_CHARS,4e3,0,1e5)}function Xe(){return rr()||lt(process.env.REVAGENT_LIVE_STATUS_DISABLED)}function or(){return Qe(process.env.REVAGENT_LIVE_STATUS_RECENT,50,5,200)}function ir(){return Qe(process.env.REVAGENT_LIVE_STATUS_MAX_IN_FLIGHT,32,1,64)}function yo(){return Qe(process.env.REVAGENT_LIVE_STATUS_HEARTBEAT_MS,5e3,0,6e4)}function ns(e,t){let n=String(e||""),r={hash:dt(n),length:n.length,present:n.length>0};if(t>0){let o=un(n,t);r.text=o.text,r.textTruncated=o.truncated}return r}function rs(e){let t=String(e||""),n={hash:dt(t),length:t.length,lineCount:Za(t),writePatternCount:Nt(t).length,writePatterns:Nt(t).slice(0,12),hasManualTransaction:/new\s+(Transaction|SubTransaction|TransactionGroup)\s*\(|\b(Transaction|SubTransaction|TransactionGroup)\s*\(/i.test(t)},r=ts();if(r>0){let o=un(t,r);n.preview=o.text,n.previewTruncated=o.truncated}return n}function os(e,t){let n=new Set(["transactionMode","responseMode","planMode","planCandidateMode","targetVisualStyle","intent","imageFormat","cameraOrientation","viewType","category","discipline","cropBasis","searchBudget","linkScope","reason","scanStoppedReason"]);if(typeof t=="boolean"||typeof t=="number")return t;if(typeof t=="string")return n.has(e)?t:ns(t,es())}function dn(e={}){let t={keys:[]};if(!e||typeof e!="object")return t;let n=Object.keys(e).sort();t.keys=n.filter(r=>r!=="code"&&r!=="parameters");for(let r of n){let o=e[r];if(r==="code"){t.code=rs(o);continue}if(r==="parameters"){t.parameters={count:Array.isArray(o)?o.length:o==null?0:1};continue}if(/elementIds$/i.test(r)&&Array.isArray(o)){t[r]={count:o.length};continue}if(Array.isArray(o)){t[r]={count:o.length};continue}if(o&&typeof o=="object"){t[r]={keys:Object.keys(o).sort()};continue}let i=os(r,o);i!==void 0&&(t[r]=i)}return t}function bo(e){if(e&&typeof e=="object"){if(Me(e,["success","Success"])===!1)return e;if("result"in e&&e.result!==null&&e.result!==void 0)return e.result;if("result"in e)return e}return e&&typeof e=="object"&&"result"in e?e.result:e}function Me(e,t){if(!e||typeof e!="object")return;for(let r of t)if(Object.prototype.hasOwnProperty.call(e,r))return e[r];let n=Object.entries(e);for(let[r,o]of n)if(t.some(i=>r.toLowerCase()===i.toLowerCase()))return o}function So(e){let t=String(e||"").trim().toLowerCase();return t==="runtime"||t==="client"?t:null}function Ft(e,t=null){if(t)return{success:!1,errorMessage:un(t instanceof Error?t.message:String(t)).text,errorType:t instanceof Error?t.name:"Error"};let n=bo(e),r=n&&typeof n=="object"&&!Array.isArray(n),o=r?Me(n,["success","Success"]):void 0,i=r?Me(n,["state","State"]):void 0,a=r?Me(n,["action","Action"]):void 0,s=r?Me(n,["error","Error","errorMessage","ErrorMessage"]):void 0,l=r?Me(n,["message","Message"]):void 0,u=r?Me(n,["guardSource","GuardSource"]):void 0,h=typeof n=="string"?n:"",m=/^\s*ERROR\s*:/i.test(h)?h:"",g=String(i||"").toLowerCase()==="guarded"||Me(n,["guarded","blocked","focusBlocked"])===!0||/blocked by safety|guarded|rejected write-looking code|does not support writeCommit|only executes with transactionMode 'none'/i.test(String(s||l||h||""));return{success:typeof o=="boolean"?o:!s&&!m,guarded:g,guardSource:g?So(u)||"runtime":null,state:i||null,action:a||null,responseKind:Array.isArray(n)?"array":n===null?"null":typeof n,responseKeys:r?Object.keys(n).sort().slice(0,40):[],errorMessage:s||m?un(s||m).text:null,messageHash:l?dt(l):null}}function uo(e,t=null){if(t)return Ft(null,t);try{let n=e?.content?.find?.(r=>r?.type==="text")?.text;if(typeof n=="string"&&n.trim().startsWith("{"))return Ft(JSON.parse(n))}catch{}return{success:!0,guarded:!1,responseKind:e===null?"null":typeof e,responseKeys:e&&typeof e=="object"?Object.keys(e).sort().slice(0,40):[]}}function is(){return Qe(process.env.REVAGENT_TELEMETRY_CONTEXT_ELEMENTS,12,0,100)}function wo(e){if(typeof e!="string")return e;let t=e.trim();if(!t.startsWith("{")&&!t.startsWith("[")&&!t.startsWith('"'))return e;try{let n=JSON.parse(t);return typeof n=="string"?wo(n):n}catch{return e}}function as(e){try{let t=e?.content?.find?.(n=>n?.type==="text")?.text;if(typeof t=="string")return wo(t)}catch{}return e}function Le(e){return e&&typeof e=="object"&&!Array.isArray(e)?e:null}function A(e,t){return Me(e,t)}function O(e,t,n=5){if(n<0||e===null||e===void 0)return;if(Array.isArray(e)){for(let i of e.slice(0,50)){let a=O(i,t,n-1);if(a!=null&&a!=="")return a}return}let r=Le(e);if(!r)return;let o=A(r,t);if(o!=null&&o!=="")return o;for(let i of Object.values(r)){let a=O(i,t,n-1);if(a!=null&&a!=="")return a}}function mn(e,t,n=5,r=[]){if(n<0||e===null||e===void 0||r.length>=20)return r;if(Array.isArray(e)){for(let i of e.slice(0,50))mn(i,t,n-1,r);return r}let o=Le(e);if(!o)return r;for(let[i,a]of Object.entries(o))t.some(s=>i.toLowerCase()===s.toLowerCase())&&Array.isArray(a)&&r.push(a),mn(a,t,n-1,r);return r}function Zn(e,t,n=5,r=[]){if(n<0||e===null||e===void 0||r.length>=20)return r;if(Array.isArray(e)){for(let i of e.slice(0,50))Zn(i,t,n-1,r);return r}let o=Le(e);if(!o)return r;for(let[i,a]of Object.entries(o))t.some(s=>i.toLowerCase()===s.toLowerCase())&&Le(a)&&r.push(a),Zn(a,t,n-1,r);return r}function B(e){return e==null?null:typeof e=="string"?e:typeof e=="number"||typeof e=="boolean"?String(e):null}function Ne(e){return typeof e=="number"&&Number.isFinite(e)?e:typeof e=="string"&&/^-?\d+$/.test(e.trim())?Number.parseInt(e.trim(),10):null}function xo(e,t=25){return[...new Set((Array.isArray(e)?e:[]).map(n=>Ne(n)).filter(n=>Number.isFinite(n)))].slice(0,t)}function ss(e={}){let t=[];e.elementId!==void 0&&t.push(e.elementId),e.viewId!==void 0&&t.push(e.viewId);for(let[n,r]of Object.entries(e||{}))/elementIds$/i.test(n)&&Array.isArray(r)&&t.push(...r);return xo(t,50)}function mo(e){let t=Le(e);if(!t)return null;let n=Ne(A(t,["id","Id","elementId","ElementId"])),r=B(A(t,["name","Name"])),o=B(A(t,["category","Category","categoryName","CategoryName"])),i=B(A(t,["typeName","TypeName","familyName","FamilyName"])),a=B(A(t,["levelName","LevelName","level","Level"])),s=B(A(t,["roomName","RoomName","room","Room"])),l=B(A(t,["roomNumber","RoomNumber"])),u=B(A(t,["spaceName","SpaceName","space","Space"])),h=B(A(t,["spaceNumber","SpaceNumber"]));return!n&&!r&&!o&&!i&&!a&&!s&&!u?null:{id:n,name:r,category:o,typeName:i,levelName:a,roomName:s,roomNumber:l,spaceName:u,spaceNumber:h}}function ls(e){let t=new Set;return e.filter(n=>{if(!n)return!1;let r=n.id?`id:${n.id}`:JSON.stringify(n);return t.has(r)?!1:(t.add(r),!0)})}function cs(e,t){let n=mn(e,["elements","Elements","selectionElements","SelectionElements"]),r=Zn(e,["chosenElement","ChosenElement","targetElement","TargetElement"]),o=[];for(let i of r)o.push(mo(i));for(let i of n)for(let a of i.slice(0,t))o.push(mo(a));return ls(o).slice(0,t)}function us(e){let t=O(e,["selectionIds","SelectionIds"],4);return Array.isArray(t)?xo(t,50):[]}function ds(e){let t=mn(e,["files","Files"],4),n=[];for(let r of t)for(let o of r.slice(0,12)){let i=Le(o);i&&n.push({path:B(A(i,["path","Path"])),fileName:B(A(i,["fileName","FileName"])),bytes:Ne(A(i,["bytes","Bytes"])),width:Ne(A(i,["width","Width"])),height:Ne(A(i,["height","Height"])),finalPixelSizeMatchesRequest:A(i,["finalPixelSizeMatchesRequest","FinalPixelSizeMatchesRequest"])})}return n.filter(r=>r.path||r.fileName)}function Kn(e,t){let n=O(e,t,4);return Le(n)?{id:Ne(A(n,["id","Id","viewId","ViewId"])),name:B(A(n,["name","Name","viewName","ViewName"])),type:B(A(n,["type","Type","viewType","ViewType"]))}:null}function ms(e,t=20){return[...new Set(e.filter(n=>typeof n=="string"&&n.trim()).map(n=>n.trim()))].slice(0,t)}function ps(e=[],t="",n="",r=""){let o=`${e.join(" ")} ${t} ${n} ${r}`.toLowerCase();return/\bm\d{2,}[a-z]?\b/i.test(o)?"mechanical_hvac":/\bp\d{2,}[a-z]?\b/i.test(o)?"mechanical_piping":/\be\d{2,}[a-z]?\b/i.test(o)?"electrical":/\bs\d{2,}[a-z]?\b/i.test(o)?"structural":/\ba\d{2,}[a-z]?\b/i.test(o)?"architectural":/(duct|air terminal|mechanical equipment|diffuser|damper|hvac|fan coil|ahu|havaland|mekanik)/i.test(o)?"mechanical_hvac":/(pipe|plumbing|sanitary|domestic|hydronic|sprinkler|fire|piping|boru|yangın|yangin|temiz su|pis su)/i.test(o)?"mechanical_piping":/(electrical|cable|lighting|elektrik)/i.test(o)?"electrical":/(structural|beam|column|framing|statik|kiris|kolon)/i.test(o)?"structural":/(wall|door|window|room|space|architect|mimari)/i.test(o)?"architectural":/(schedule|sheet|drawing|revision|pafta|metraj|mahal listesi)/i.test(o)?"schedule_documentation":null}function hs(e,t){let n=e||t||"";return n?dt(n):null}function fs(e={},t=[]){for(let n of t){let r=e?.[n];if(typeof r=="string"&&r.trim())return r.trim()}return null}function gs(e={},t=[]){return t.map(n=>e?.[n]).filter(n=>typeof n=="string"&&n.trim()).map(n=>n.trim())}function ys(e={},t="",n=null,r=null,o=null,i={}){return[t,i.toolName,i.commandName,i.logicalToolName,...gs(e,["query","nameQuery","cellQuery","sheetQuery","scheduleNameQuery","scheduleQuery","rowTextQuery","planNameContains","category","discipline"]),...Array.isArray(e.rowTextQueries)?e.rowTextQueries:[],...Array.isArray(e.categoryNames)?e.categoryNames:[],n?.name,r?.name,o?.name].filter(s=>typeof s=="string"&&s.trim()).join(" ")}function bs(...e){let t=e.filter(i=>typeof i=="string"&&i.trim()).join(" ");if(!t)return null;let n=t.match(/\b(?:level|lvl|l)\s*[-_ ]?(\d{1,2})\b/i);if(n)return`Level ${n[1].padStart(2,"0")}`;let r=t.match(/\b(?:kat|floor)\s*[-_ ]?(\d{1,2})\b/i);if(r)return`Level ${r[1].padStart(2,"0")}`;let o=t.match(/\b(?:basement|bodrum|b)\s*[-_ ]?(\d{1,2})\b/i);return o?`Basement ${o[1].padStart(2,"0")}`:null}function Ss(e={}){let t=e.sourceEventType==="mcp.tool"?as(e.response):bo(e.response),n=Le(t),r=e.params||{},o=e.taskName||r.taskName||e.options?.taskName||e.logicalToolName||e.toolName||e.commandName||null,i=e.responseSummary||Ft(e.response,e.error),a=is(),s=a>0?cs(t,a):[],l=ms([...Array.isArray(r.categoryNames)?r.categoryNames.map(String):[],B(r.category),...s.map(_a=>_a.category)]),u=O(t,["document","Document"],3),h=B(O(t,["documentTitle","DocumentTitle"],5))||B(A(u,["title","Title","name","Name"])),m=B(O(t,["documentPath","DocumentPath"],5))||B(A(u,["path","Path","modelPath","ModelPath"])),g=Kn(t,["activeView","ActiveView","view","View"]),b=Kn(t,["beforeView","BeforeView","activeViewBefore","ActiveViewBefore"]),y=Kn(t,["afterView","AfterView"]),T=ss(r),P=us(t),te=ds(t),z=B(O(t,["levelName","LevelName","activePlanLevelName","ActivePlanLevelName"],5)),J=Ne(O(t,["levelId","LevelId","activePlanLevelId","ActivePlanLevelId"],5)),q=B(O(t,["roomName","RoomName"],5)),X=B(O(t,["roomNumber","RoomNumber"],5)),ye=B(O(t,["spaceName","SpaceName"],5)),Re=B(O(t,["spaceNumber","SpaceNumber"],5)),Ie=fs(r,["query","nameQuery","cellQuery","sheetQuery","scheduleNameQuery","scheduleQuery","rowTextQuery"]),ae=typeof r.outputDir=="string"?r.outputDir:B(O(t,["outputDir","OutputDir"],4)),_e=typeof r.filePrefix=="string"?r.filePrefix:B(O(t,["filePrefix","FilePrefix"],4)),Kt=ys(r,o||"",g,b,y,e),zr=z||bs(Kt),Ta=O(t,["inferredScope","InferredScope"],5),qr=O(t,["effectiveScope","EffectiveScope"],5),Zt=O(t,["riskPolicy","RiskPolicy","searchRiskPolicy","SearchRiskPolicy"],5),en=O(t,["scanPolicy","ScanPolicy"],5),Ra=O(t,["partial","Partial"],4),Wr=B(O(t,["scanStoppedReason","ScanStoppedReason"],4)),Ia=Ne(O(t,["scannedElementCount","ScannedElementCount"],4));return!(o||h||m||g||b||y||T.length||P.length||s.length||te.length||zr||q||ye||Ie||ae)?null:{eventType:"production.context",contextSchemaVersion:"revagent.production.context.v1",related:{sourceEventType:e.sourceEventType,toolName:e.toolName||null,commandName:e.commandName||null,logicalToolName:e.logicalToolName||null,executionKind:e.executionKind||null},runId:e.taskId||r.taskId||e.options?.taskId||dt(`${fn}|${e.sourceEventType||""}|${e.toolName||""}|${e.commandName||""}|${e.startedAtMs||""}|${o||""}`),operation:{taskName:o,query:Ie,action:i.action||B(O(t,["action","Action"],3)),durationMs:e.durationMs,success:i.success,guarded:i.guarded,state:i.state,errorMessage:i.errorMessage},project:{projectId:hs(m,h),documentTitle:h,documentPath:m,isFamilyDocument:O(t,["isFamilyDocument","IsFamilyDocument"],4),isReadOnly:O(t,["isReadOnly","IsReadOnly"],4),isModifiable:O(t,["isModifiable","IsModifiable"],4)},view:{active:g,before:b,after:y,activeViewChanged:O(t,["activeViewChanged","ActiveViewChanged"],4)},location:{levelId:J,levelName:zr,roomName:q,roomNumber:X,spaceName:ye,spaceNumber:Re},elements:{targetElementIds:T,selectionIds:P,selectionCount:Ne(O(t,["selectionCount","SelectionCount"],4)),categories:l,disciplineHint:ps(l,o||"",Kt,e.toolName||e.logicalToolName||e.commandName||""),samples:s,samplesTruncated:a>0&&s.length>=a},outputs:{outputDir:ae,filePrefix:_e,files:te},search:{query:Ie,inferredScope:Ta,effectiveScope:qr,riskPolicy:Zt,riskLevel:A(Zt,["riskLevel","RiskLevel"])||null,recommendedFirstScope:A(Zt,["recommendedFirstScope","RecommendedFirstScope"])||null,requiresUserControl:A(Zt,["requiresUserControl","RequiresUserControl"])===!0,scanPolicy:en,searchBudget:r.searchBudget||A(en,["searchBudget","SearchBudget"])||null,linkScope:r.linkScope||A(qr,["linkScope","LinkScope"])||null,planCandidateMode:r.planCandidateMode||A(en,["planCandidateMode","PlanCandidateMode"])||null,allowExpensiveSearch:r.allowExpensiveSearch===!0||A(en,["allowExpensiveSearch","AllowExpensiveSearch"])===!0,scannedElementCount:Ia,partial:Ra===!0,scanStoppedReason:Wr,needsScope:i.guarded&&i.state==="guarded"&&(A(n,["reason","Reason"])==="needs_scope"||Wr==="needs_scope")},response:{responseKeys:i.responseKeys||(n?Object.keys(n).sort().slice(0,40):[])}}}function er(e={}){let t=Ss(e);t&&Lt(t)}function vo(){let e=eo();return{disabled:rr(),localOnly:lt(process.env.REVAGENT_TELEMETRY_LOCAL_ONLY),localRoot:process.env.REVAGENT_TELEMETRY_ROOT||to(),reportsRoot:process.env.REVAGENT_REPORTS_ROOT||e?.reportsRoot||""}}function Co(e){let t=e.getUTCFullYear().toString(),n=String(e.getUTCMonth()+1).padStart(2,"0"),r=String(e.getUTCDate()).padStart(2,"0");return{year:t,month:n,day:r,ymd:`${t}-${n}-${r}`}}function ws(e){let t=vo();if(t.disabled)return[];let n=new Date(e.timestampUtc||Date.now()),r=Co(n),o=rn(Ue(e.machineName),"unknown-machine"),a=[{kind:"local",path:ut.join(t.localRoot,"events",`${r.ymd}.ndjson`)}];return!t.localOnly&&t.reportsRoot&&a.push({kind:"remote",path:ut.join(t.reportsRoot,"events",r.year,r.month,r.day,o,`${e.sessionId}.ndjson`)}),a}function xs(){let e=vo();return{disabled:Xe(),localOnly:e.localOnly||lt(process.env.REVAGENT_LIVE_STATUS_LOCAL_ONLY),localRoot:process.env.REVAGENT_LIVE_STATUS_LOCAL_ROOT||ut.join(e.localRoot,"live"),reportsRoot:process.env.REVAGENT_LIVE_STATUS_ROOT||(e.reportsRoot?ut.join(e.reportsRoot,"live"):"")}}function To(e=[]){let t=xs();if(t.disabled)return[];let r=["machines",rn(Ue(process.env.COMPUTERNAME||nr.hostname()),"unknown-machine"),...e],o=[{kind:"local",path:ut.join(t.localRoot,...r)}];return!t.localOnly&&t.reportsRoot&&o.push({kind:"remote",path:ut.join(t.reportsRoot,...r)}),o}function Ro(e){return!e||typeof e!="object"||Array.isArray(e)?null:{success:typeof e.success=="boolean"?e.success:null,guarded:e.guarded===!0,guardSource:e.guardSource||null,state:e.state||null,action:e.action||null,errorMessage:e.errorMessage||null,messageHash:e.messageHash||null}}function pn(e,t="summary"){if(!e)return null;let n={liveTaskId:e.liveTaskId,scope:e.scope,toolName:e.toolName||null,commandName:e.commandName||null,logicalToolName:e.logicalToolName||null,executionKind:e.executionKind||null,taskName:e.taskName||null,taskIdPresent:!!e.taskId,parentTaskName:e.parentTaskName||null,parentTaskIdPresent:!!e.parentTaskId,state:e.state,guardSource:e.guardSource||null,startedAtUtc:e.startedAtUtc,finishedAtUtc:e.finishedAtUtc||null,durationMs:e.durationMs??null,result:t==="full"?e.result||null:Ro(e.result)};return t!=="full"&&!n.result&&delete n.result,n}function po(e){if(!e||typeof e!="object")return null;let t=e.commandName||e.method||null,n=e.wrapperAction||e.logicalToolName||e.toolName||t;return{id:e.id||null,requestId:e.requestId||null,method:n||null,toolName:n||null,commandName:t,wrapperAction:e.wrapperAction||null,logicalToolName:e.logicalToolName||null,taskName:e.taskName||null,parentTaskName:e.parentTaskName||null,parentTaskIdPresent:!!(e.parentTaskIdPresent||e.parentTaskId),state:e.state||null,startedAtUtc:e.startedAtUtc||null,finishedAtUtc:e.finishedAtUtc||null,elapsedMs:e.elapsedMs??null,requestBytes:e.requestBytes??null,responseBytes:e.responseBytes??null,port:e.port||null,error:e.error||null}}function vs(e,t){if(t==="full")return e;let n=Ro(e.result),r={timestampUtc:e.timestampUtc||e.finishedAtUtc||e.startedAtUtc||null,phase:e.phase,state:e.state||e.phase||null,scope:e.scope||null,toolName:e.toolName||null,commandName:e.commandName||null,logicalToolName:e.logicalToolName||null,executionKind:e.executionKind||null,taskName:e.taskName||null,parentTaskName:e.parentTaskName||null,parentTaskIdPresent:!!(e.parentTaskIdPresent||e.parentTaskId),guardSource:e.guardSource||n?.guardSource||null,startedAtUtc:e.startedAtUtc||null,finishedAtUtc:e.finishedAtUtc||null,durationMs:e.durationMs??null};return n&&(r.success=n.success,r.guarded=n.guarded,r.action=n.action,r.errorMessage=n.errorMessage,r.messageHash=n.messageHash),Object.fromEntries(Object.entries(r).filter(([,o])=>o!=null))}function Io(e=10,t="summary"){let n=Qe(e,10,0,100),r=t==="full"?"full":"summary",i=(r==="full"?$e:$e.filter(a=>a.phase!=="started")).slice(0,n).map(a=>vs(a,r));return{mode:r,activeTask:pn(_o(),r),activeTasks:[...Dt.values()].map(a=>pn(a,r)),recentActivity:i,recentActivityCount:i.length,recentActivityStoredCount:$e.length,recentActivityCapacity:or()}}function Cs(e){if(!e||typeof e!="object")return null;let t=e.result&&typeof e.result=="object"?e.result:e;return{capturedAtUtc:new Date().toISOString(),activeTask:po(t.activeTask),recentTasks:(Array.isArray(t.recentTasks)?t.recentTasks:[]).map(po).filter(Boolean).slice(0,100),recentHistoryCount:t.recentHistoryCount??null,recentHistoryCapacity:t.recentHistoryCapacity??null}}function gn(e){if(Xe())return;let t=Cs(e);t&&(go=t,hn("revit.status"))}function _o(){let e=[...Dt.values()];return e.length===0?null:e.sort((t,n)=>{let r=i=>i.scope==="revit.command"?2:1,o=r(n)-r(t);return o!==0?o:String(n.startedAtUtc||"").localeCompare(String(t.startedAtUtc||""))})[0]}function Ts(e="activity"){let n=kt()?.version||null,r=new Date().toISOString();return co=r,{schemaVersion:Xa,generatedAtUtc:r,lastHeartbeatUtc:co,reason:e,machineName:Ue(process.env.COMPUTERNAME||nr.hostname()),userName:process.env.USERNAME||process.env.USER||"",sessionId:fn,runtime:{version:n,buildHash:Pt(n)},process:{pid:process.pid,nodeVersion:process.version,startedAtUtc:fo},activeTask:pn(_o(),"full"),activeTasks:[...Dt.values()].map(o=>pn(o,"full")),recentActivity:$e.slice(0,or()),revitStatus:go,writeHealth:io(ir())}}function Rs(e){let t=Array.isArray(e?.revitStatus?.recentTasks)?e.revitStatus.recentTasks:[],n=Array.isArray(e?.activeTasks)?e.activeTasks:[],r=Array.isArray(e?.recentActivity)?e.recentActivity:[];return!!(e?.activeTask||n.length>0||r.length>0||e?.revitStatus?.activeTask||t.length>0)}function Is(e){let t=Date.parse(String(e?.generatedAtUtc||e?.lastHeartbeatUtc||""));return Number.isFinite(t)?Math.max(0,Date.now()-t):Number.POSITIVE_INFINITY}function _s(e,t){let n=ct(e);if(!n||Ue(n.machineName)!==Ue(t.machineName))return t;let r=Math.max(600*1e3,yo()*6);return!Rs(n)||Is(n)>r?t:{...t,recentActivity:Array.isArray(t.recentActivity)&&t.recentActivity.length>0?t.recentActivity:Array.isArray(n.recentActivity)?n.recentActivity:[],revitStatus:lo(t.revitStatus,n.revitStatus)}}function hn(e="activity"){let t=Ts(e);for(let n of To(["status.json"]))Qn(n.path,r=>ro(r,_s(r,t)),{disabled:Xe,maxInFlight:ir})}function Ms(e){let t={liveTaskId:e.liveTaskId,scope:e.scope,toolName:e.toolName,commandName:e.commandName,logicalToolName:e.logicalToolName,executionKind:e.executionKind,taskName:e.taskName,taskId:e.taskId,parentTaskName:e.parentTaskName,parentTaskId:e.parentTaskId,guardSource:e.guardSource,state:e.state,startedAtUtc:e.startedAtUtc,finishedAtUtc:e.finishedAtUtc,durationMs:e.durationMs,result:e.result};e.phase==="started"?Dt.set(e.liveTaskId,t):Dt.delete(e.liveTaskId),$e.unshift({timestampUtc:e.timestampUtc,phase:e.phase,state:e.state,scope:e.scope,toolName:e.toolName||null,commandName:e.commandName||null,logicalToolName:e.logicalToolName||null,executionKind:e.executionKind||null,taskName:e.taskName||null,parentTaskName:e.parentTaskName||null,parentTaskIdPresent:!!e.parentTaskId,guardSource:e.guardSource||null,startedAtUtc:e.startedAtUtc,finishedAtUtc:e.finishedAtUtc||null,durationMs:e.durationMs??null,result:e.result||null});let n=or();$e.length>n&&$e.splice(n)}function Mo(e){Ms(e);let t=Co(new Date(e.timestampUtc||Date.now()));for(let n of To(["activity",`${t.ymd}.ndjson`]))Qn(n.path,r=>$n(r,e),{disabled:Xe,maxInFlight:ir});hn(e.phase)}function Ns(e={},t){return e.taskId?String(e.taskId):dt([fn,e.scope||"",e.toolName||"",e.commandName||"",e.logicalToolName||"",t||Date.now(),e.taskName||""].join("|"))}function mt(e={}){if(Xe())return null;let t=e.startedAtMs||Date.now(),n=new Date(t).toISOString(),r=Ns(e,t),o=ar({schemaVersion:ho,eventType:"live.activity",phase:"started",state:"running",liveTaskId:r,scope:e.scope||"runtime",toolName:e.toolName||null,commandName:e.commandName||null,logicalToolName:e.logicalToolName||null,executionKind:e.executionKind||null,taskName:e.taskName||null,taskId:e.taskId||null,taskIdPresent:!!e.taskId,parentTaskName:e.parentTaskName||null,parentTaskId:e.parentTaskId||null,parentTaskIdPresent:!!e.parentTaskId,startedAtUtc:n,params:dn(e.params)});return Mo(o),{liveTaskId:r,scope:o.scope,toolName:o.toolName,commandName:o.commandName,logicalToolName:o.logicalToolName,executionKind:o.executionKind,taskName:o.taskName,taskId:o.taskId,parentTaskName:o.parentTaskName,parentTaskId:o.parentTaskId,guardSource:o.guardSource,startedAtMs:t,startedAtUtc:n}}function Se(e,t={}){if(!e||Xe())return;let n=Date.now(),r=t.durationMs??Math.max(0,n-(e.startedAtMs||n)),o=t.responseSummary||Ft(t.response,t.error),i=o.guarded?"guarded":o.success===!1?"failed":"completed",a=o.guarded?So(t.guardSource||e.guardSource||o.guardSource)||"runtime":null,s=ar({schemaVersion:ho,eventType:"live.activity",phase:i,state:i,liveTaskId:e.liveTaskId,scope:e.scope||t.scope||"runtime",toolName:e.toolName||t.toolName||null,commandName:e.commandName||t.commandName||null,logicalToolName:e.logicalToolName||t.logicalToolName||null,executionKind:e.executionKind||t.executionKind||null,taskName:e.taskName||t.taskName||null,taskId:e.taskId||t.taskId||null,taskIdPresent:!!(e.taskId||t.taskId),parentTaskName:e.parentTaskName||t.parentTaskName||null,parentTaskId:e.parentTaskId||t.parentTaskId||null,parentTaskIdPresent:!!(e.parentTaskId||t.parentTaskId),guardSource:a,startedAtUtc:e.startedAtUtc||null,finishedAtUtc:new Date(n).toISOString(),durationMs:r,result:o});Mo(s)}function Es(){if(cn||Xe())return;let e=yo();e<=0||(hn("session.start"),cn=setInterval(()=>{hn("heartbeat")},e),typeof cn.unref=="function"&&cn.unref())}function ar(e={}){let n=kt()?.version||null;return{schemaVersion:Qa,eventId:tr.randomUUID(),eventType:e.eventType||"runtime.event",timestampUtc:e.timestampUtc||new Date().toISOString(),sessionId:fn,sequence:++Ya,source:"runtime-mcp-server",process:{pid:process.pid,nodeVersion:process.version,startedAtUtc:fo},machineName:Ue(process.env.COMPUTERNAME||nr.hostname()),userName:process.env.USERNAME||process.env.USER||"",runtime:{version:n,buildHash:Pt(n)},...e}}async function Lt(e={}){if(rr())return;let t=ar(e),n=ws(t);await Promise.allSettled(n.map(r=>oo(r.path,t)))}function No(){Es(),Lt({eventType:"runtime.session.start"})}function Ee(e={}){let t=Math.max(0,Date.now()-(e.startedAtMs||Date.now())),n=Ft(e.response,e.error);Lt({eventType:"revit.command",commandName:e.commandName,logicalToolName:e.logicalToolName||e.commandName,executionKind:e.executionKind||"bridgeCommand",taskName:e.params?.taskName||e.options?.taskName||null,taskIdPresent:!!(e.params?.taskId||e.options?.taskId),parentTaskName:e.params?.parentTaskName||e.options?.parentTaskName||null,parentTaskIdPresent:!!(e.params?.parentTaskId||e.options?.parentTaskId),transactionMode:e.params?.transactionMode||e.options?.transactionMode||null,connection:{targetPresent:!!e.options?.target,hostPresent:!!e.options?.host,port:e.options?.port||null},durationMs:t,params:dn(e.params),result:n}),er({...e,sourceEventType:"revit.command",durationMs:t,responseSummary:n,taskName:e.params?.taskName||e.options?.taskName||null,taskId:e.params?.taskId||e.options?.taskId||null,parentTaskName:e.params?.parentTaskName||e.options?.parentTaskName||null,parentTaskId:e.params?.parentTaskId||e.options?.parentTaskId||null})}function ks(e){return!(e==="get_revit_mcp_status"&&!lt(process.env.REVAGENT_TELEMETRY_INCLUDE_STATUS))}function Eo(e){return{...e,tool(t,n,r,o){let i=n,a=r,s=o;typeof n=="object"&&(s=r,a=n,i="");let l=async(u,h)=>{let m=Date.now(),g=ks(t),b=g?mt({scope:"mcp.tool",toolName:t,taskName:u?.taskName||null,taskId:u?.taskId||null,parentTaskName:u?.parentTaskName||null,parentTaskId:u?.parentTaskId||null,params:u,startedAtMs:m}):null;try{let y=await s(u,h);if(g){let T=Math.max(0,Date.now()-m),P=uo(y);Lt({eventType:"mcp.tool",toolName:t,taskName:u?.taskName||null,taskIdPresent:!!u?.taskId,parentTaskName:u?.parentTaskName||null,parentTaskIdPresent:!!u?.parentTaskId,durationMs:T,params:dn(u),result:P}),er({sourceEventType:"mcp.tool",toolName:t,taskName:u?.taskName||null,taskId:u?.taskId||null,parentTaskName:u?.parentTaskName||null,parentTaskId:u?.parentTaskId||null,params:u,response:y,durationMs:T,startedAtMs:m,responseSummary:P}),Se(b,{response:y,responseSummary:P,durationMs:T})}return y}catch(y){if(g){let T=Math.max(0,Date.now()-m),P=uo(null,y);Lt({eventType:"mcp.tool",toolName:t,taskName:u?.taskName||null,taskIdPresent:!!u?.taskId,parentTaskName:u?.parentTaskName||null,parentTaskIdPresent:!!u?.parentTaskId,durationMs:T,params:dn(u),result:P}),er({sourceEventType:"mcp.tool",toolName:t,taskName:u?.taskName||null,taskId:u?.taskId||null,parentTaskName:u?.parentTaskName||null,parentTaskId:u?.parentTaskId||null,params:u,error:y,durationMs:T,startedAtMs:m,responseSummary:P}),Se(b,{error:y,responseSummary:P,durationMs:T})}throw y}};return e.tool(t,i,a,l)}}}var Ps=2;function S(e){return{target:e.string().optional().describe("Optional Revit target: registered instance name, port number such as 8081, or host:port. Defaults to REVIT_MCP_TARGET/REVIT_MCP_PORT/8080."),host:e.string().optional().describe("Optional Revit socket host. Defaults to REVIT_MCP_HOST or localhost."),port:e.number().int().positive().max(65535).optional().describe("Optional Revit socket port. Defaults to REVIT_MCP_PORT or 8080.")}}function x(e){return{taskName:e.string().optional().describe("Optional display name shown in Revit while this MCP task is running."),taskId:e.string().optional().describe("Optional client task identifier forwarded to Revit status history."),parentTaskName:e.string().optional().describe("Optional parent workflow display name. Wrappers set this on nested sub-operations so live feed/history preserves the operator-visible parent task."),parentTaskId:e.string().optional().describe("Optional parent workflow identifier. Wrappers set this on nested sub-operations so live feed/history preserves the operator-visible parent task id.")}}function d(e,t,n){if(!e||typeof e!="object")return;let r=n??t.charAt(0).toLowerCase()+t.slice(1);return e[t]??e[r]}function Y(e={}){return{target:e.target,host:e.host,port:e.port,timeoutMs:e.timeoutMs}}function se(e={},t){return{taskName:e.taskName||t,taskId:e.taskId,parentTaskName:e.parentTaskName,parentTaskId:e.parentTaskId}}function I(e={},t){return{...Y(e),...se(e,t)}}function Po(e,t){let n=t.parentTaskName||(t.taskName&&e.taskName&&e.taskName!==t.taskName?t.taskName:void 0),r=t.parentTaskId||(t.taskId&&e.taskName&&e.taskName!==t.taskName?t.taskId:void 0);n&&!e.parentTaskName&&(e.parentTaskName=n),r&&!e.parentTaskId&&(e.parentTaskId=r)}function Ao(e,t,n){let r=n.toolName||t;r&&!e.logicalToolName&&(e.logicalToolName=r),n.toolName&&n.toolName!==t&&!e.wrapperAction&&(e.wrapperAction=n.toolName)}function yn(e){let t=[["Success","success"],["SUCCESS","success"],["Guarded","guarded"],["State","state"],["Action","action"],["Message","message"],["Error","error"],["ResultContractVersion","resultContractVersion"]],n=r=>{if(Array.isArray(r))return r.map(i=>n(i));if(!r||typeof r!="object")return r;let o={};for(let[i,a]of Object.entries(r))o[i]=n(a);for(let[i,a]of t)Object.prototype.hasOwnProperty.call(o,i)&&(Object.prototype.hasOwnProperty.call(o,a)||(o[a]=o[i]),delete o[i]);return o};return n(e)}function p(e){let t=yn(e);return{content:[{type:"text",text:JSON.stringify(t,null,2)}]}}function jt(e,t=0){if(typeof e!="string")return e;let n=e.trim();if(!n.startsWith("{")&&!n.startsWith("[")&&!n.startsWith('"'))return e;try{let r=JSON.parse(n);return t<2&&typeof r=="string"?jt(r,t+1):r}catch{return e}}function bn(e){if(Array.isArray(e))return e.map(n=>bn(n));if(!e||typeof e!="object")return e;let t={};for(let[n,r]of Object.entries(e)){let o=n==="result"||n==="Result"?jt(r):r;t[n]=bn(o)}return t}function As(e){if(!e||typeof e!="object"||Array.isArray(e))return null;let t=e.resultContractVersion??e.ResultContractVersion,n=Number.parseInt(String(t??""),10);return Number.isFinite(n)?n:null}function Os(e){let t=As(e);return t!==null&&t>=Ps}function ke(e,t={}){let n=jt(e);if(Os(n))return t.parseResultStrings===!0?yn(bn(n)):n;if(n&&typeof n=="object"&&!Array.isArray(n)){let r=n;return t.parseResultStrings===!0?r=bn(r):("result"in r||"Result"in r)&&(r={...r},"result"in r?r.result=jt(r.result):r.Result=jt(r.Result)),yn(r)}return yn(n)}function Oo(e,t,n,r){let o=Number.parseInt(String(e??""),10);return Number.isFinite(o)?Math.max(n,Math.min(r,o)):t}function Ke(e,t={}){let n=t.verboseCandidates===!0,r=Oo(t.maxPlanCandidates,3,0,100);if(n)return e;let o=i=>{if(Array.isArray(i))return i.map(s=>o(s));if(!i||typeof i!="object")return i;let a={};for(let[s,l]of Object.entries(i)){if((s==="PlanCandidates"||s==="planCandidates")&&Array.isArray(l)){let u=s==="PlanCandidates"?"PlanCandidatesTotal":"planCandidatesTotal",h=s==="PlanCandidates"?"PlanCandidatesTruncated":"planCandidatesTruncated";a[u]=l.length,a[h]=l.length>r,a[s]=l.slice(0,r).map(m=>o(m));continue}a[s]=o(l)}return a};return o(e)}function ko(e,t){if(!e||typeof e!="object")return e;let n=e.commandName||e.method,r=e.wrapperAction||e.logicalToolName||e.toolName||n,o={id:e.id,requestId:e.requestId,method:r,toolName:r,commandName:n,wrapperAction:e.wrapperAction,logicalToolName:e.logicalToolName,taskName:e.taskName,parentTaskName:e.parentTaskName,parentTaskIdPresent:!!(e.parentTaskIdPresent||e.parentTaskId),state:e.state,startedAtUtc:e.startedAtUtc,finishedAtUtc:e.finishedAtUtc,elapsedMs:e.elapsedMs,port:e.port,error:e.error};return t&&(o.framing=e.framing,o.requestBytes=e.requestBytes,o.receiveMs=e.receiveMs,o.parseMs=e.parseMs,o.executeMs=e.executeMs,o.responseBytes=e.responseBytes),o}function Bt(e,t={}){let n=t.includeRecentTasks!==!1,r=t.includeDiagnostics===!0,o=Oo(t.recentLimit,3,0,100),i=e&&typeof e=="object"&&e.result&&typeof e.result=="object"?e.result:e;if(!i||typeof i!="object")return e;let a={...i};return a.activeTask=ko(i.activeTask,r),Array.isArray(i.recentTasks)&&(a.recentHistoryCount=i.recentHistoryCount??i.recentTasks.length,a.recentHistoryCapacity=i.recentHistoryCapacity??100,delete a.recentTasksTotal,n?(a.recentTasks=i.recentTasks.slice(0,o).map(s=>ko(s,r)),a.recentTasksTruncated=i.recentTasks.length>o):(delete a.recentTasks,a.recentTasksIncluded=!1)),e&&typeof e=="object"&&e.result&&typeof e.result=="object"?{...e,result:a}:a}async function H(e,t={}){let n={code:e,parameters:t.parameters||[],transactionMode:t.transactionMode||"none",taskName:t.taskName||"Run Revit code"};t.taskId&&(n.taskId=t.taskId),Ao(n,"send_code_to_revit",t),Po(n,t);let r=Date.now(),o=mt({scope:"revit.command",commandName:"send_code_to_revit",logicalToolName:t.toolName||n.taskName,executionKind:"dynamicCode",taskName:n.taskName,taskId:n.taskId,parentTaskName:n.parentTaskName,parentTaskId:n.parentTaskId,params:n,startedAtMs:r});try{let i=await be(async l=>await l.sendCommand("send_code_to_revit",n,t),t),a=t.parseJsonResult===!1?i:ke(i,{parseResultStrings:!0}),s=Math.max(0,Date.now()-r);return Ee({commandName:"send_code_to_revit",logicalToolName:t.toolName||n.taskName,executionKind:"dynamicCode",params:n,options:t,response:a,startedAtMs:r}),Se(o,{response:a,durationMs:s}),Ye(t),a}catch(i){let a=Math.max(0,Date.now()-r);throw Ee({commandName:"send_code_to_revit",logicalToolName:t.toolName||n.taskName,executionKind:"dynamicCode",params:n,options:t,error:i,startedAtMs:r}),Se(o,{error:i,durationMs:a}),Ye(t),i}}async function Ye(e={}){let t=Math.max(250,Math.min(5e3,Number(e.statusRefreshTimeoutMs||1500)));try{let n=await be(async r=>await r.sendCommand("mcp_status",{},{timeoutMs:t}),{...e,skipLock:!0,connectTimeoutMs:t,timeoutMs:t,logSocketErrors:!1});return gn(n),n}catch{return null}}async function _(e,t={},n={}){let r={...t};r.taskName||(r.taskName=n.taskName||e),Po(r,n),n.taskId&&!r.taskId&&(r.taskId=n.taskId),Ao(r,e,n);let o=Date.now(),i=mt({scope:"revit.command",commandName:e,logicalToolName:n.toolName||e,executionKind:"bridgeCommand",taskName:r.taskName,taskId:r.taskId,parentTaskName:r.parentTaskName,parentTaskId:r.parentTaskId,params:r,startedAtMs:o});try{let a=await be(async u=>await u.sendCommand(e,r,n),n),s=ke(a),l=Math.max(0,Date.now()-o);return Ee({commandName:e,logicalToolName:n.toolName||e,executionKind:"bridgeCommand",params:r,options:n,response:s,startedAtMs:o}),Se(i,{response:s,durationMs:l}),Ye(n),s}catch(a){let s=Math.max(0,Date.now()-o);throw Ee({commandName:e,logicalToolName:n.toolName||e,executionKind:"bridgeCommand",params:r,options:n,error:a,startedAtMs:o}),Se(i,{error:a,durationMs:s}),Ye(n),a}}function R(e){return e==null?"null":`"${String(e).replace(/\\/g,"\\\\").replace(/"/g,'\\"').replace(/\r/g,"\\r").replace(/\n/g,"\\n")}"`}function we(e){return`new string[] { ${(Array.isArray(e)?e:[]).map(R).join(", ")} }`}function Sn(e){return`new int[] { ${(Array.isArray(e)?e:[]).map(n=>Number.parseInt(String(n),10)).filter(n=>Number.isFinite(n)).join(", ")} }`}function Vo(e,t){let n=Number(t||0);return!n||typeof e!="string"||e.length<=n?{text:e,truncated:!1}:{text:`${e.slice(0,n)}
...[truncated ${e.length-n} chars]`,truncated:!0}}function Vs(e){let t=new Set,n=(r,o="")=>{if(r!=null){if(typeof r=="number"&&/(^id$|elementid|elementids)/i.test(o)){t.add(r);return}if(typeof r=="string"&&/^-?\d+$/.test(r)&&/(^id$|elementid|elementids)/i.test(o)){t.add(Number.parseInt(r,10));return}if(Array.isArray(r)){for(let i of r)n(i,o);return}if(typeof r=="object")for(let[i,a]of Object.entries(r))n(a,i)}};return n(e),[...t].filter(r=>Number.isFinite(r)&&r>0)}async function pt(e=100,t={}){let n=await _("get_selected_elements",{limit:e},t);return Vs(n).slice(0,e)}var Ds=new Set(["success","guarded","state","action","error","reason","warnings","notices"]);function Do(e){let t=String(e||"").trim();return t.length>0?t:void 0}function Fo(e){if(!Array.isArray(e))return;let t=e.map(n=>String(n||"").trim()).filter(n=>n.length>0);return t.length>0?t:void 0}function Fs(e){return e?Object.fromEntries(Object.entries(e).filter(([t])=>!Ds.has(t))):{}}function sr(e,t){let n={...Fs(t.extra),...e,action:t.action},r=Do(t.error),o=Do(t.reason),i=Fo(t.warnings),a=Fo(t.notices);return r&&(n.error=r),o&&(n.reason=o),i&&(n.warnings=i),a&&(n.notices=a),n}function Lo(e){return sr({success:!0,guarded:!1,state:"completed",action:e.action},e)}function Ce(e){return sr({success:!1,guarded:!0,state:"guarded",action:e.action},e)}function pe(e){return sr({success:!1,guarded:!1,state:"failed",action:e.action},e)}function Ls(e){let t=String(e||"");return t.match(/^\s*(?:public|private|protected|internal|static|sealed|abstract|partial|\s)*\b(?:class|struct|interface|enum|record)\s+[A-Za-z_][A-Za-z0-9_]*/m)?{reason:"dynamic_snippet_type_declaration_not_supported",message:"Dynamic snippets are inserted inside Execute(Document document, object[] parameters). C# type declarations such as class/struct/interface/enum/record cannot be declared inside that method body. Use local functions, built-in collections, or add a native runtime tool when reusable helper types are needed."}:t.match(/^\s*namespace\s+[A-Za-z_][A-Za-z0-9_.]*/m)?{reason:"dynamic_snippet_namespace_declaration_not_supported",message:"Dynamic snippets are inserted inside Execute(Document document, object[] parameters). namespace declarations cannot be declared inside that method body. Use method-body C# only."}:null}function js(e){let t=ke(e);if(t&&typeof t=="object"&&t.success===!1)return t.error||t.errorMessage||t.message||"Revit code returned success=false.";let n=t&&typeof t=="object"&&"result"in t?t.result:t;return typeof n=="string"&&/^\s*ERROR\s*:/i.test(n)?n.trim():n&&typeof n=="object"&&n.success===!1?n.error||n.message||"Revit code returned success=false.":null}function jo(e){e.tool("send_code_to_revit","Send C# code to Revit for execution. The code will be inserted into a template with access to the Revit Document and parameters. Your code should be written to work within the Execute method of the template.",{...S(Pe),...x(Pe),code:Pe.string().describe("The C# code to execute in Revit. This code will be inserted into the Execute method of a template with access to Document and parameters."),parameters:Pe.array(Pe.any()).optional().describe("Optional execution parameters that will be passed to your code"),transactionMode:Pe.enum(["auto","none"]).optional().describe("Transaction handling mode forwarded to the Revit wrapper. In the bundled plugin build, snippets should not open their own Transaction unless that exact build has been verified."),timeoutMs:Pe.number().int().positive().optional().describe("Socket timeout in milliseconds for this Revit command. Defaults to 120000."),reportErrorResultAsFailure:Pe.boolean().optional().describe("When true, ERROR: string results or { success:false } objects are reported as failed tool calls. Defaults true. This cannot roll back a write if the snippet swallowed its own exception."),parseJsonResult:Pe.boolean().optional().describe("When true, parse JSON-looking result strings, including double-encoded JSON strings. Defaults true. Set false to inspect the raw wire result.")},async(t,n)=>{let r={code:t.code,parameters:t.parameters||[],transactionMode:t.transactionMode||"auto",taskName:t.taskName||"Run Revit code"};t.taskId&&(r.taskId=t.taskId),t.parentTaskName&&(r.parentTaskName=t.parentTaskName),t.parentTaskId&&(r.parentTaskId=t.parentTaskId),r.logicalToolName="send_code_to_revit";let o=Y(t),i=Date.now(),a=mt({scope:"revit.command",commandName:"send_code_to_revit",logicalToolName:"send_code_to_revit",executionKind:"dynamicCode",taskName:r.taskName,taskId:r.taskId,parentTaskName:r.parentTaskName,parentTaskId:r.parentTaskId,params:r,startedAtMs:i}),s=Ls(t.code);if(s){let l=Math.max(0,Date.now()-i),u=Ce({action:"dynamic_snippet_preflight",reason:s.reason,error:s.message});return Ee({commandName:"send_code_to_revit",logicalToolName:"send_code_to_revit",executionKind:"dynamicCode",params:r,options:o,response:u,startedAtMs:i}),Se(a,{response:u,durationMs:l}),{content:[{type:"text",text:`Code execution guarded: ${s.message}`}]}}try{let l=await be(async g=>await g.sendCommand("send_code_to_revit",r,o),o),u=t.parseJsonResult===!1?l:ke(l,{parseResultStrings:!0}),h=Math.max(0,Date.now()-i);Ee({commandName:"send_code_to_revit",logicalToolName:"send_code_to_revit",executionKind:"dynamicCode",params:r,options:o,response:u,startedAtMs:i}),Se(a,{response:u,durationMs:h}),Ye(o);let m=t.parseJsonResult===!1||t.reportErrorResultAsFailure===!1?null:js(u);return m?{content:[{type:"text",text:`Code execution failed: ${m}`}]}:{content:[{type:"text",text:`Code execution successful!
Result: ${JSON.stringify(u,null,2)}`}]}}catch(l){let u=Math.max(0,Date.now()-i);return Ee({commandName:"send_code_to_revit",logicalToolName:"send_code_to_revit",executionKind:"dynamicCode",params:r,options:o,error:l,startedAtMs:i}),Se(a,{error:l,durationMs:u}),Ye(o),{content:[{type:"text",text:`Code execution failed: ${l instanceof Error?l.message:String(l)}`}]}}})}import{z as le}from"zod";function lr(e,t,n){return p(Ce({action:"send_code_to_revit_safe_preflight",error:e,reason:n,extra:{safetyReason:n,writePatterns:t}}))}function Bo(e){e.tool("send_code_to_revit_safe","Run Revit C# through the existing dynamic execution command with read/preview safety checks, JSON result parsing, and output trimming. This MVP does not commit writes.",{...S(le),...x(le),code:le.string().min(1).describe("Body of Execute(Document document, object[] parameters)."),parameters:le.array(le.union([le.string(),le.number(),le.boolean()])).optional().describe("Simple execution parameters. Prefer strings for host portability."),transactionMode:le.enum(["auto","none"]).optional().describe("Safe wrapper execution mode. Only none is executed; auto is rejected for read/preview safety."),intent:le.enum(["read","writePreview","writeCommit"]).optional().describe("Safety intent. writeCommit is not supported by this MVP wrapper."),timeoutMs:le.number().int().positive().optional().describe("Socket timeout in milliseconds for this Revit command. Defaults to 120000."),maxReturnedChars:le.number().int().positive().optional().describe("Maximum JSON characters returned to the model."),parseJsonResult:le.boolean().optional().describe("When true, parse JSON-looking result strings. Defaults true.")},async t=>{let n=t.intent||"read",r=Nt(t.code);if(n==="writeCommit")return lr("send_code_to_revit_safe does not support writeCommit in this MVP. Use raw send_code_to_revit only after explicit user confirmation.",r,"safe_wrapper_write_commit_not_supported");if(t.transactionMode==="auto")return lr("send_code_to_revit_safe only executes with transactionMode 'none'. Use raw send_code_to_revit for an explicitly confirmed write.",r,"safe_wrapper_requires_transactionMode_none");if(r.length>0)return lr(`Rejected write-looking code for intent '${n}'.`,r,"safe_wrapper_rejected_write_looking_code");try{let i=await H(t.code,{...Y(t),...se(t,"Run safe Revit read"),parameters:t.parameters||[],transactionMode:"none",parseJsonResult:t.parseJsonResult!==!1}),a=Lo({action:"send_code_to_revit_safe",extra:{intent:n,response:i}}),s=JSON.stringify(a,null,2),l=Vo(s,t.maxReturnedChars);return l.truncated?{content:[{type:"text",text:l.text}]}:p(a)}catch(o){return p(pe({action:"send_code_to_revit_safe",error:o instanceof Error?o.message:String(o)}))}})}import{z as ht}from"zod";function Bs(e){return e&&typeof e=="object"&&e.result&&typeof e.result=="object"?e.result:e}function zs(e){let t=String(e.detailLevel||"minimal").toLowerCase(),n=e.includeCategoryCounts===!0||t==="counts"||t==="full"?"true":"false",r=e.includeLinks!==!1?"true":"false",o=e.includeLinks===!0&&t==="full"||t==="full"?"true":"false";return`
...
}`}function zo(e){e.tool("get_revit_session_context","Read-only Revit session summary. Defaults to detailLevel=minimal so large-model document checks do not perform heavy MEP category or linked room/space counts. Use detailLevel=counts/full only when those expensive counts are explicitly needed.",{...S(ht),...x(ht),detailLevel:ht.enum(["minimal","counts","full"]).optional().describe("Context detail level. minimal is default and avoids category counts and linked room/space scans; counts adds host MEP category counts; full also scans linked room/space counts."),includeCategoryCounts:ht.boolean().optional().describe("Compatibility flag. true includes known MEP category counts; default false unless detailLevel is counts/full."),includeLinks:ht.boolean().optional().describe("Include cheap Revit link instance summary. Defaults true; linked room/space counts require detailLevel=full."),includeSelection:ht.boolean().optional().describe("Include selected element ids using the existing Revit selection command. Defaults true.")},async t=>{let n=Y(t);try{let r=await H(zs(t),{...n,...se(t,"Read Revit session context"),transactionMode:"none"}),o=Bs(r);if(t.includeSelection!==!1&&o&&typeof o=="object"){let i=await pt(100,{...n,taskName:t.taskName?`${t.taskName}: selection`:"Read Revit selection",taskId:t.taskId});o.selection={count:i.length,elementIds:i}}return p(o)}catch(r){return p({success:!1,error:r instanceof Error?r.message:String(r)})}})}import{z as je}from"zod";function qs(e){let t=e.includeSheetViewports!==!1?"true":"false",n=e.includeSheetScheduleInstances!==!1?"true":"false",r=e.includeModelElements===!0?"true":"false",o=Number.isFinite(e.limit)?Math.max(1,Math.min(500,e.limit)):100,i=we(e.modelCategoryList||[]);return`
...
var requestedElementIds = ${Dl(n.ids)};
...
bool cropToTargetHighlight = ${z?"true":"false"};
bool allowFinalUpscale = ${y?"true":"false"};
double targetMinFillRatio = ${J};
int highlightCropPaddingPx = ${q};
...
return new {
  ...
  warnings = warnings,
  notices = notices
};`;try{let ae=await H(Ie,{...I(t,"Export Revit coordination image"),taskType:"export_revit_coordination_image",transactionMode:"auto"});return p(ae?.result??ae)}catch(ae){return p(pe({action:"export_revit_coordination_image",error:ae instanceof Error?ae.message:String(ae),extra:{tool:"export_revit_coordination_image"}}))}})}import Ll from"node:os";import li from"node:path";import{z as Q}from"zod";var jl=Q.enum(["current_view","visible_region","set_of_views"]),Bl=Q.enum(["png","jpg_lossless","jpg_medium","tiff","bmp","targa"]),zl=Q.enum(["72","150","300","600"]),ql=Q.enum(["horizontal","vertical"]),Wl={png:"PNG",jpg_lossless:"JPEGLossless",jpg_medium:"JPEGMedium",tiff:"TIFF",bmp:"BMP",targa:"TARGA"},Gl={72:"DPI_72",150:"DPI_150",300:"DPI_300",600:"DPI_600"},Jl={horizontal:"Horizontal",vertical:"Vertical"};function Hl(){return li.join(Ll.tmpdir(),"revit-mcp-image-export")}function Ul(e){return(e&&e.trim()?e.trim():`revit-view-${new Date().toISOString().replace(/[:.]/g,"-")}`).replace(/[<>:"/\\|?*\x00-\x1F]/g,"_").slice(0,120)}function $l(e){if(e==null||e==="")return"null";let t=Number(e);return Number.isFinite(t)?String(Math.trunc(t)):"null"}function ci(e){e.tool("export_revit_view_image","[VISUAL_ARTIFACT_EXPORT] Export the active Revit view, DrawingSheet, Schedule view, or a selected view/sheet to PNG/JPEG/TIFF/BMP/TARGA using Document.ExportImage. Use this when the user asks for a raw image file, report/evidence screenshot, schedule/sheet export, or LLM visual artifact from an existing view. Ordinary view/sheet exports do not modify Revit. Direct schedule export creates a temporary sheet, exports it, and deletes that sheet before the wrapper transaction commits.",{...S(Q),viewId:Q.union([Q.number(),Q.string()]).optional().describe("Optional Revit view id. When supplied, export uses set_of_views because Revit cannot export a non-active visible region."),viewName:Q.string().optional().describe("Optional exact or partial view name. When supplied, export uses set_of_views unless range is explicitly current/visible."),exactName:Q.boolean().optional().default(!0),range:jl.optional().describe("current_view and visible_region use the active UI view. set_of_views can export viewId/viewName without switching the UI."),format:Bl.optional().default("png"),pixelSize:Q.number().int().min(200).max(1e4).optional().default(6e3),enforcePixelSize:Q.boolean().optional().default(!0).describe("When true, post-processes PNG/JPEG/BMP/TIFF output so the requested fit direction dimension equals pixelSize. TARGA cannot be resized by this tool."),zoom:Q.number().int().min(1).max(1e3).optional().default(100),dpi:zl.optional().default("300"),fitDirection:ql.optional().default("horizontal"),outputDir:Q.string().optional(),filePrefix:Q.string().optional(),allowTemporaryScheduleSheet:Q.boolean().optional().default(!0).describe("When true, standalone Schedule views are exported through a temporary sheet that is deleted before the wrapper transaction commits. When false, schedule views return guidance with containing sheet candidates."),...x(Q),timeoutMs:Q.number().int().positive().optional()},async t=>{let n=t.viewId!==void 0||!!t.viewName,r=t.range??(n?"set_of_views":"current_view"),o=li.resolve(t.outputDir||Hl()),i=Ul(t.filePrefix),a=Wl[t.format||"png"],s=Gl[String(t.dpi||"150")],l=Jl[t.fitDirection||"horizontal"],u=Math.trunc(t.pixelSize||6e3),h=t.enforcePixelSize!==!1,m=Math.trunc(t.zoom||100),g=t.allowTemporaryScheduleSheet!==!1,b=`
var warnings = new List<string>();
var notices = new List<string>();
...
{
    return new { success = false, error = ex.ToString() };
}`}function hi(e){e.tool("inspect_elements","Read-only inspection for selected or targeted Revit elements: class/category/type/level/key parameters/connector summary.",{...S(ue),...x(ue),elementIds:ue.array(ue.union([ue.number(),ue.string()])).optional().describe("Element ids to inspect."),useSelection:ue.boolean().optional().describe("When true, inspect the current Revit selection."),limit:ue.number().int().positive().max(100).optional().describe("Maximum elements to inspect. Defaults 20."),includeParameters:ue.boolean().optional().describe("Include key or requested parameter summaries. Defaults true."),includeTypeParameters:ue.boolean().optional().describe("Also inspect matching type parameters. Defaults false."),includeConnectors:ue.boolean().optional().describe("Include connector counts when available. Defaults true. When false, connectorCount/openConnectorCount are null and connectorsIncluded=false."),parameterNames:ue.array(ue.string()).optional().describe("Optional targeted parameter names.")},async t=>{let n=Y(t);try{let r=await ic(t,n);if(r.length===0)return p({success:!0,elements:[],warnings:["No element ids supplied and no selected elements found."]});let o=await H(ac(r,t),{...n,...se(t,"Inspect Revit elements"),transactionMode:"none"});return p(o&&o.result?o.result:o)}catch(r){return p({success:!1,error:r instanceof Error?r.message:String(r)})}})}import{z as E}from"zod";var sc=["completed","max_elapsed","max_rows","max_columns","max_cells","max_items","max_bytes","read_failed","needs_scope"],lc=["lastReadSection","lastReadRow","lastReadColumn","lastReadSheetId","lastReadViewId","lastReadViewportId","lastReadItemId"],cc=new Set(sc),uc={done:"completed",success:"completed",timeout:"max_elapsed",timed_out:"max_elapsed",socket_timeout:"max_elapsed",max_schedules:"max_items",max_sheets:"max_items",max_text_notes:"max_items",max_tags:"max_items",max_viewports:"max_items",max_scanned:"max_items",max_schedule_instances:"max_items",max_schedule_cells:"max_cells",max_cells_scanned:"max_cells",rows_truncated:"max_rows",columns_truncated:"max_columns"};function Gt(e){return!!e&&typeof e=="object"&&!Array.isArray(e)}function yt(e){return String(e??"").trim()}function gt(e){return Array.isArray(e)?e.map(t=>yt(t)).filter(t=>t.length>0):[]}function c(e,t){if(!Gt(e))return;let n=t.charAt(0).toUpperCase()+t.slice(1);if(Object.prototype.hasOwnProperty.call(e,t))return e[t];if(Object.prototype.hasOwnProperty.call(e,n))return e[n];let r=t.toLowerCase(),o=Object.keys(e).find(i=>i.toLowerCase()===r);return o?e[o]:void 0}function C(e,t){let n=c(e,t);return Array.isArray(n)?n.filter(r=>Gt(r)):[]}function bt(e,t){let n=c(e,t);return Gt(n)?n:null}function fi(e,t=!1){if(typeof e=="boolean")return e;if(typeof e=="string"){let n=e.trim().toLowerCase();if(n==="true")return!0;if(n==="false")return!1}return t}function gi(e){if(e==null)return null;if(typeof e=="number")return Number.isFinite(e)?e:null;if(typeof e=="string"){let t=e.trim();if(t.length===0)return null;let n=Number(t);return Number.isFinite(n)?n:null}return null}function Jt(e,t="completed"){let n=yt(e).toLowerCase();return n?cc.has(n)?n:uc[n]||t:t}function dc(e,t,n,r){return n?"needs_scope":r==="failed"?"read_failed":t?"max_items":"completed"}function gr(e,t,n){return typeof e=="function"?e(t):e??n}function re(e,t){let n=Gt(e)?{...e}:{value:e},r=yt(c(n,"state")),o=yt(c(n,"error")),i=fi(c(n,"guarded"),!1),a=c(n,"success"),s=typeof a=="boolean"?!!a:o.length===0,l=r||(i?"guarded":s?"completed":"failed"),u=t.partial??fi(c(n,"partial"),!1),h=yt(t.scanStoppedReason??c(n,"scanStoppedReason")),m=dc(n,u,i,l),g=Jt(h,m);n.success=s,n.guarded=i,n.state=l,n.action=t.action,n.partial=u,n.scanStoppedReason=g,h&&h!==g&&n.rawScanStoppedReason===void 0&&(n.rawScanStoppedReason=h);let b=bt(n,"scanPolicy");n.scanPolicy=b||t.scanPolicy||{};let y=gt(c(n,"suggestedNextScopes"));n.suggestedNextScopes=y.length>0?y:gt(t.suggestedNextScopes),n.elapsedMs=gi(c(n,"elapsedMs"))??gi(t.elapsedMs),n.warnings=gt(c(n,"warnings")).concat(gt(t.warnings)),n.notices=gt(c(n,"notices")).concat(gt(t.notices));let T=gr(t.evidenceRows,n,[]),P=C(n,"evidenceRows");n.evidenceRows=P.length>0?P:Array.isArray(T)?T:[];let te=gr(t.summary,n,{}),z=bt(n,"summary");n.summary=z||(Gt(te)?te:{});let J=gr(t.lastRead,n,{});for(let q of lc){let X=c(n,q);n[q]=X!==void 0?X:J[q]??null}return n}function de(e){let t=yt(e.reason)||"needs_scope";return re({...e.extra||{},success:!0,guarded:!0,state:"guarded",action:e.action,reason:t,message:e.message,partial:!1,scanStoppedReason:t},{...e,partial:!1,scanStoppedReason:t,summary:e.summary||{},evidenceRows:e.evidenceRows||[]})}function fe(e){return re({...e.extra||{},success:!1,guarded:!1,state:"failed",action:e.action,error:e.error,partial:!1,scanStoppedReason:"read_failed"},{...e,partial:!1,scanStoppedReason:"read_failed",summary:e.summary||{},evidenceRows:e.evidenceRows||[]})}var mc={fast:{maxElapsedMs:4500,timeoutMs:12e3},balanced:{maxElapsedMs:15e3,timeoutMs:3e4},deep:{maxElapsedMs:45e3,timeoutMs:6e4}};function pc(e){let t=["fast","balanced","deep"].includes(String(e.searchBudget||""))?String(e.searchBudget):"fast",n=mc[t],r=Number.parseInt(String(e.maxElapsedMs??""),10),o=Number.isFinite(r)?Math.max(1,Math.min(119e3,r)):n.maxElapsedMs,i=Number.parseInt(String(e.timeoutMs??""),10),a=Number.isFinite(i)?Math.max(1e3,Math.min(12e4,i)):Math.max(n.timeoutMs,Math.min(12e4,o+5e3));return{searchBudget:t,maxElapsedMs:Math.min(o,Math.max(1,a-1e3)),timeoutMs:a}}function hc(e){return!!(Array.isArray(e.sheetIds)&&e.sheetIds.length>0||String(e.sheetQuery||e.query||"").trim())}function fc(e,t){return de({action:"inspect_sheet_text",reason:"needs_scope",message:"Project-wide sheet annotation, viewport text, tag, or placed schedule-cell scans can be expensive in large models. First pass sheetQuery/sheetIds, or set allowExpensiveSearch=true with bounded caps.",suggestedNextScopes:["sheetQuery","sheetIds","viewNameQuery","maxSheets","allowExpensiveSearch","searchBudget=deep"],scanPolicy:{searchBudget:t.searchBudget,maxElapsedMs:t.maxElapsedMs,timeoutMs:t.timeoutMs,allowExpensiveSearch:!1,textQuery:!!String(e.textQuery||"").trim(),includeViewportTextNotes:e.includeViewportTextNotes===!0,includeViewportTags:e.includeViewportTags===!0,scanScheduleCells:e.scanScheduleCells===!0,maxTags:e.maxTags??e.maxTagsScanned,maxViewports:e.maxViewports??e.maxViewportsPerSheet},summary:{sheetQuery:e.sheetQuery??e.query??null,textQuery:e.textQuery??null,returnedCount:0,matchCount:0}})}function gc(e,t){return{query:e.query,sheetQuery:e.sheetQuery??e.query,textQuery:e.textQuery,sheetIds:e.sheetIds,includeTextNotes:e.includeTextNotes,includeScheduleInstances:e.includeScheduleInstances,scanScheduleCells:e.scanScheduleCells,allowExpensiveSearch:e.allowExpensiveSearch,searchBudget:t.searchBudget,maxElapsedMs:t.maxElapsedMs,includeViewportTextNotes:e.includeViewportTextNotes,includeViewportTags:e.includeViewportTags,viewNameQuery:e.viewNameQuery,maxSheets:e.maxSheets,maxTextNotesPerSheet:e.maxTextNotesPerSheet,maxScheduleInstancesPerSheet:e.maxScheduleInstancesPerSheet,maxRowsPerSchedule:e.maxRowsPerSchedule,maxColumnsPerSchedule:e.maxColumnsPerSchedule,maxTextChars:e.maxTextChars,maxViewportsPerSheet:e.maxViewportsPerSheet,maxViewports:e.maxViewports,maxViewportTextNotesPerView:e.maxViewportTextNotesPerView,maxViewportTagsPerView:e.maxViewportTagsPerView,maxTags:e.maxTags,maxTextNotesScanned:e.maxTextNotesScanned,maxTagsScanned:e.maxTagsScanned,maxScheduleInstancesScanned:e.maxScheduleInstancesScanned,maxScheduleCellsScanned:e.maxScheduleCellsScanned,maxResponseBytes:e.maxResponseBytes,timeoutMs:t.timeoutMs,taskName:e.taskName||"Inspect Revit sheet annotations",taskId:e.taskId}}function yr(e){let t=String(c(e,"kind")||c(e,"sourceType")||"");return t==="scheduleCell"?"placedScheduleCell":t==="scheduleInstance"?"placedScheduleInstance":t||"sheetTextNote"}function St(e){return String(c(e,"textQuery")??"").trim().length>0}function br(e,t=!0){if(!t)return!1;let n=c(e,"matchedTextQuery"),r=c(e,"inventoryOnly");return!(r===!0||String(r).trim().toLowerCase()==="true"||n===!1||String(n).trim().toLowerCase()==="false")}function In(e){let t=C(e,"evidenceRows"),n=t.length>0?t:C(e,"matches"),r=St(e);return n.filter(o=>!!o&&typeof o=="object"&&!Array.isArray(o)).filter(o=>br(o,r)).map(o=>({...o,sourceType:yr(o)}))}function yi(e){let t=C(e,"inventoryRows"),n=C(e,"evidenceRows"),r=St(e),o=[...n,...C(e,"matches")].filter(a=>!!a&&typeof a=="object"&&!Array.isArray(a)).filter(a=>!br(a,r)),i=new Set;return[...t,...o].filter(a=>!!a&&typeof a=="object"&&!Array.isArray(a)).map(a=>({...a,sourceType:yr(a),matchedTextQuery:!1,inventoryOnly:!0})).filter(a=>{let s=[c(a,"sourceType")??"",c(a,"sheetId")??"",c(a,"instanceId")??c(a,"elementId")??c(a,"id")??"",c(a,"scheduleId")??""].join("|");return i.has(s)?!1:(i.add(s),!0)})}function Sr(e,t){let n={};for(let[r,o]of Object.entries(e))t.has(r)||(n[r]=o);return n}function yc(e,t){let n=t&&br(e,t);return{...Sr(e,new Set(["MatchedTextQuery","InventoryOnly","matchedTextQuery","inventoryOnly"])),sourceType:yr({...e,kind:c(e,"kind")??"scheduleInstance"}),MatchedTextQuery:n,InventoryOnly:!n,matchedTextQuery:n,inventoryOnly:!n}}function bc(e){let t=St(e);return C(e,"sheets").map(n=>{let r=Sr(n,new Set(["ScheduleInstances"])),o=C(n,"scheduleInstances");return{...r,scheduleInstances:o.map(i=>yc(i,t))}})}function Sc(e){let t=c(e,"scan");return!t||typeof t!="object"||Array.isArray(t)||St(e)?t:{...t,TotalTextNoteMatches:0,totalTextNoteMatches:0,TotalViewportTextNoteMatches:0,totalViewportTextNoteMatches:0,TotalViewportTagMatches:0,totalViewportTagMatches:0,TotalScheduleCellMatches:0,totalScheduleCellMatches:0,TotalScheduleInstanceMatches:0,totalScheduleInstanceMatches:0}}function bi(e){let t=Jt(c(e,"scanStoppedReason")),n=String(c(e,"rawScanStoppedReason")??c(e,"scanStoppedReason")??t).trim()||t;return{canonicalReason:t,nativeReason:n,nativeLimitField:{max_sheets:"maxSheets",max_text_notes:"maxTextNotesScanned",max_viewports:"maxViewports",max_scanned:"maxScheduleInstancesScanned",max_schedule_instances:"maxScheduleInstancesScanned",max_schedule_cells:"maxScheduleCellsScanned",max_tags:"maxTagsScanned"}[n]??null}}function wc(e){let t=In(e),n=yi(e),r=C(e,"sheets");return{sheetQuery:c(e,"sheetQuery")??null,textQuery:c(e,"textQuery")??null,totalSheets:c(e,"totalSheets")??null,candidateCount:c(e,"candidateCount")??null,returnedCount:c(e,"returnedCount")??(r.length>0?r.length:null),inventoryMode:!St(e),matchCount:t.length,inventoryRowCount:n.length,partial:c(e,"partial")===!0,scanStoppedReason:c(e,"scanStoppedReason")??"completed",rawScanStoppedReason:c(e,"rawScanStoppedReason")??null,scanStopDetail:bi(e),scannedSheetCount:c(e,"scannedSheetCount")??null,scannedViewportCount:c(e,"scannedViewportCount")??null,scannedTextNoteCount:c(e,"scannedTextNoteCount")??null,scannedTagCount:c(e,"scannedTagCount")??null,scannedScheduleInstanceCount:c(e,"scannedScheduleInstanceCount")??null,scannedScheduleCellCount:c(e,"scannedScheduleCellCount")??null}}function xc(e){let t=C(e,"evidenceRows").length>0?C(e,"evidenceRows"):In(e),n=t.length>0?t[t.length-1]:null,r=C(e,"sheets"),o=r.length>0?r[r.length-1]:null;return{lastReadSection:n?c(n,"section")??null:null,lastReadRow:n?c(n,"row")??null:null,lastReadColumn:n?c(n,"column")??null:null,lastReadSheetId:n?c(n,"sheetId")??c(o,"id")??null:c(o,"id")??null,lastReadViewId:n?c(n,"viewId")??null:null,lastReadViewportId:n?c(n,"viewportId")??null:null,lastReadItemId:n?c(n,"elementId")??c(n,"tagId")??c(n,"instanceId")??c(n,"id")??null:null}}function vc(e,t){let n=re(e,{action:"inspect_sheet_text",elapsedMs:t,summary:wc,evidenceRows:In,lastRead:xc,suggestedNextScopes:["sheetQuery","sheetIds","viewNameQuery","maxSheets","allowExpensiveSearch","searchBudget=deep"]}),r=yi(n),o=St(n),i=Sc(n),a=new Set(["Sheets"]);return o||(a.add("Matches"),a.add("EvidenceRows")),{...Sr(n,a),evidenceRows:o?In(n):[],inventoryRows:r,matches:o?C(n,"matches"):[],scan:i,sheets:bc(n),summary:{...n.summary||{},inventoryRowCount:r.length,scanStopDetail:bi(n)}}}function Si(e){e.tool("inspect_sheet_text","[SHEET_TEXT_INSPECTION_READ_ONLY] Read-only native sheet text and annotation inspection for DrawingSheet text notes, titleblock/title block notes, revision schedule instances, placed schedule cells, viewport-linked text notes, viewport plan annotations, and viewport tags. Prefer this dedicated tool over generic send_code_to_revit for sheet text lookup, drawing note searches, plan note searches, titleblock/revision evidence, and large-project sheet or viewport annotation searches. Use sheetQuery/sheetIds first; project-wide text, viewport, tag, or placed-schedule cell scans require allowExpensiveSearch=true.",{...S(E),...x(E),query:E.string().optional().describe("Alias for sheetQuery. Matches sheet number and sheet name with Turkish/diacritic/Cyrillic-U normalization."),sheetQuery:E.string().optional().describe("Sheet number/name filter. Use this first in large projects before broad text or viewport annotation search."),textQuery:E.string().optional().describe("Optional text to search in sheet text notes, viewport text notes, or placed schedule cells."),sheetIds:E.array(E.union([E.number(),E.string()])).optional().describe("Exact ViewSheet element ids to inspect. Preferred when known."),includeTextNotes:E.boolean().optional().describe("Include bounded sheet TextNote results. Defaults true."),includeScheduleInstances:E.boolean().optional().describe("Include placed ScheduleSheetInstance entries on matching sheets. Defaults true."),scanScheduleCells:E.boolean().optional().describe("When true, search bounded body cells of placed schedules for textQuery. Defaults false to avoid broad scans."),allowExpensiveSearch:E.boolean().optional().describe("Explicit approval for project-wide sheet, viewport, tag, or placed-schedule cell scans without sheetIds/sheetQuery. Defaults false."),searchBudget:E.enum(["fast","balanced","deep"]).optional().describe("Native Revit-side scan budget preset. fast is default; deep still respects maxElapsedMs and response-size caps."),maxElapsedMs:E.number().int().positive().max(119e3).optional().describe("Native Revit-side elapsed budget. It is clamped below timeoutMs so partial results can return before transport timeout."),includeViewportTextNotes:E.boolean().optional().describe("Include bounded TextNote results from views placed on matching sheets. Defaults false."),includeViewportTags:E.boolean().optional().describe("Include bounded IndependentTag evidence from views placed on matching sheets. Defaults false."),viewNameQuery:E.string().optional().describe("Optional placed-view name filter used before viewport text-note inspection."),maxSheets:E.number().int().positive().max(200).optional().describe("Maximum sheets to inspect/return. Defaults 30."),maxTextNotesPerSheet:E.number().int().min(0).max(1e3).optional().describe("Maximum matching sheet text notes returned per sheet. Defaults 200."),maxScheduleInstancesPerSheet:E.number().int().min(0).max(300).optional().describe("Maximum schedule instances returned per sheet. Defaults 100."),maxRowsPerSchedule:E.number().int().min(0).max(500).optional().describe("Maximum schedule body rows to scan when scanScheduleCells=true. Defaults 80."),maxColumnsPerSchedule:E.number().int().min(0).max(100).optional().describe("Maximum schedule body columns to scan when scanScheduleCells=true. Defaults 30."),maxTextChars:E.number().int().min(20).max(1e3).optional().describe("Maximum characters retained per returned text value. Defaults 240."),maxViewportsPerSheet:E.number().int().min(0).max(200).optional().describe("Maximum placed viewports inspected per sheet. Defaults 20."),maxViewports:E.number().int().min(0).max(200).optional().describe("Alias for maxViewportsPerSheet. Maximum placed viewports inspected per sheet."),maxViewportTextNotesPerView:E.number().int().min(0).max(1e3).optional().describe("Maximum matching viewport text notes returned per placed view. Defaults 200."),maxViewportTagsPerView:E.number().int().min(0).max(500).optional().describe("Maximum matching viewport tags returned per placed view. Defaults 100."),maxTextNotesScanned:E.number().int().positive().max(2e5).optional().describe("Global native cap across sheet and viewport text notes."),maxTags:E.number().int().positive().max(1e5).optional().describe("Alias for maxTagsScanned. Global native cap across viewport tags."),maxTagsScanned:E.number().int().positive().max(1e5).optional().describe("Global native cap across viewport tags."),maxScheduleInstancesScanned:E.number().int().positive().max(1e5).optional().describe("Global native cap across placed schedule instances."),maxScheduleCellsScanned:E.number().int().positive().max(5e5).optional().describe("Global native cap across placed schedule body cells."),maxResponseBytes:E.number().int().min(4096).max(16*1024*1024).optional().describe("Advanced response-size budget. The native handler stops with scanStoppedReason=max_bytes before the bridge response becomes too large."),timeoutMs:E.number().int().positive().max(12e4).optional().describe("Socket timeout in milliseconds. Defaults from searchBudget with headroom above maxElapsedMs.")},async t=>{let n=Date.now();try{let r=pc(t),o=hc(t),i=!!String(t.textQuery||"").trim()&&!o,a=t.includeViewportTextNotes===!0&&!o,s=t.scanScheduleCells===!0&&!o,l=t.includeViewportTags===!0&&!o;if((i||a||s||l)&&t.allowExpensiveSearch!==!0)return p(fc(t,r));let u=await _("inspect_sheet_text",gc(t,r),{...I({...t,timeoutMs:r.timeoutMs},"Inspect Revit sheet annotations"),toolName:"inspect_sheet_text"});return p(vc(u&&u.result?u.result:u,Date.now()-n))}catch(r){return p(fe({action:"inspect_sheet_text",error:r instanceof Error?r.message:String(r),elapsedMs:Date.now()-n,suggestedNextScopes:["sheetQuery","sheetIds","viewNameQuery","maxSheets","allowExpensiveSearch","searchBudget=deep"]}))}})}import{z as L}from"zod";var Cc=25,Tc=50;function Z(e,t,n,r){if(e==null||e==="")return t;let o=Number.parseInt(String(e??""),10);return Number.isFinite(o)?Math.max(n,Math.min(r,o)):t}function wi(e){let t=Array.isArray(e)&&e.length>0?e:["header","body"];return[...new Set(t.map(n=>String(n||"").toLowerCase()))].filter(n=>["header","body","footer"].includes(n))}var Rc={fast:{maxElapsedMs:4500,timeoutMs:12e3,maxCells:5e3},balanced:{maxElapsedMs:15e3,timeoutMs:3e4,maxCells:25e3},deep:{maxElapsedMs:45e3,timeoutMs:6e4,maxCells:1e5}};function xi(e){let t=["fast","balanced","deep"].includes(String(e.searchBudget||""))?String(e.searchBudget):"fast",n=Rc[t],r=Z(e.maxElapsedMs,n.maxElapsedMs,1,119e3),o=Z(e.timeoutMs,Math.max(n.timeoutMs,Math.min(12e4,r+5e3)),1e3,12e4);return{searchBudget:t,maxElapsedMs:Math.min(r,Math.max(1,o-1e3)),timeoutMs:o,maxCells:Z(e.maxCells,n.maxCells,1,5e5)}}function Ic(e){return(Array.isArray(e)?e:[]).map(t=>Number.parseInt(String(t),10)).filter(t=>Number.isFinite(t)&&t>0)}function _c(e,t){let n=Ic(e.scheduleIds),r=wi(e.sections);return{query:e.query,nameQuery:e.nameQuery??e.query,cellQuery:e.cellQuery,scheduleIds:n,sections:r,includeCells:e.includeCells,scanCells:e.scanCells,allowExpensiveSearch:e.allowExpensiveSearch,searchBudget:t.searchBudget,maxElapsedMs:t.maxElapsedMs,maxSchedules:Z(e.maxSchedules,50,1,200),maxRowsPerSection:Z(e.maxRowsPerSection,80,0,1e3),maxColumnsPerSection:Z(e.maxColumnsPerSection,30,0,200),startRow:Z(e.startRow,0,0,1e5),startColumn:Z(e.startColumn,0,0,1e4),maxCellTextChars:Z(e.maxCellTextChars,180,20,1e3),maxCells:t.maxCells,maxResponseBytes:Z(e.maxResponseBytes,4*1024*1024,4096,16*1024*1024),timeoutMs:t.timeoutMs,taskName:e.taskName||"Inspect Revit schedules",taskId:e.taskId}}function tt(e){return!!e&&typeof e=="object"&&!Array.isArray(e)}function Mc(e){return Array.isArray(e)?e.map(t=>String(t??"").trim()).filter(t=>t.length>0):[]}function _n(e){return C(e,"schedules").filter(tt).flatMap(n=>C(n,"sections").map(o=>({schedule:n,section:o})))}function wt(e){return String(c(e,"cellQuery")??"").trim().length>0}function xr(e){return String(c(e,"nameQuery")??c(e,"query")??"").trim().length>0}function vr(e){return wt(e)?_n(e).flatMap(({schedule:t,section:n})=>C(n,"matches").filter(tt).map(o=>({sourceType:"scheduleCell",scheduleId:c(t,"id"),scheduleName:c(t,"name"),section:c(o,"section")??c(n,"section"),row:c(o,"row"),column:c(o,"column"),text:c(o,"text")}))):[]}function Cr(e){return c(e,"partial")===!0||c(e,"truncated")===!0?!0:_n(e).some(({section:t})=>c(t,"rowsTruncated")===!0||c(t,"columnsTruncated")===!0)}function Nc(e){if(c(e,"success")===!1||String(c(e,"state")||"").toLowerCase()==="failed"||c(e,"error"))return"read_failed";if(!Cr(e))return"completed";if(c(e,"truncated")===!0)return"max_items";for(let{section:t}of _n(e)){if(c(t,"rowsTruncated")===!0)return"max_rows";if(c(t,"columnsTruncated")===!0)return"max_columns"}return"max_cells"}function vi(e){let t=Nc(e),n=c(e,"scanStoppedReason");return!n||n==="completed"&&t!=="completed"?t:n}function Ec(e){let t=Ci(e),n=tt(t)?t:{},r=C(e,"schedules"),o=C(e,"evidenceRows").length>0?C(e,"evidenceRows"):vr(e);return{query:c(e,"query")??null,nameQuery:c(e,"nameQuery")??null,cellQuery:c(e,"cellQuery")??null,totalSchedules:c(e,"totalSchedules")??null,candidateCount:c(e,"candidateCount")??null,returnedCount:c(e,"returnedCount")??(r.length>0?r.length:null),inventoryMode:!xr(e)&&!wt(e),matchCount:o.length,totalCellMatches:c(n,"totalCellMatches")??o.length,scannedScheduleCount:c(n,"scannedScheduleCount")??null,partial:Cr(e),scanStoppedReason:vi(e)}}function kc(e){let t=C(e,"evidenceRows").length>0?C(e,"evidenceRows"):vr(e),n=t.length>0?t[t.length-1]:null,r=_n(e),o=r.length>0?r[r.length-1].section:null,i=C(e,"schedules"),a=r.length>0?r[r.length-1].schedule:i.length>0?i[i.length-1]:null,s=Number(c(o,"returnedRows")??c(o,"scannedRows")??0),l=Number(c(o,"returnedColumns")??c(o,"scannedColumns")??0),u=Number(c(o,"startRow")??0),h=Number(c(o,"startColumn")??0);return{lastReadSection:c(n,"section")??c(o,"section")??null,lastReadRow:c(n,"row")??c(o,"lastReadRow")??(s>0?u+s-1:null),lastReadColumn:c(n,"column")??c(o,"lastReadColumn")??(l>0?h+l-1:null),lastReadSheetId:null,lastReadViewId:null,lastReadViewportId:null,lastReadItemId:c(n,"scheduleId")??c(a,"id")??null}}function wr(e){let t=xi(e);return{searchBudget:t.searchBudget,allowExpensiveSearch:e.allowExpensiveSearch===!0,includeCells:e.includeCells===!0,scanCells:e.scanCells===!0||!!e.cellQuery,sections:wi(e.sections),maxElapsedMs:t.maxElapsedMs,maxSchedules:Z(e.maxSchedules,50,1,200),maxRowsPerSection:Z(e.maxRowsPerSection,80,0,1e3),maxColumnsPerSection:Z(e.maxColumnsPerSection,30,0,200),startRow:Z(e.startRow,0,0,1e5),startColumn:Z(e.startColumn,0,0,1e4),maxCells:t.maxCells,maxResponseBytes:Z(e.maxResponseBytes,4*1024*1024,4096,16*1024*1024),timeoutMs:t.timeoutMs}}function Pc(e,t=!0){let{matches:n,Matches:r,...o}=e;return{...o,section:c(e,"section"),rowCount:c(e,"rowCount"),columnCount:c(e,"columnCount"),startRow:c(e,"startRow"),startColumn:c(e,"startColumn"),returnedRows:c(e,"returnedRows"),returnedColumns:c(e,"returnedColumns"),rowsTruncated:c(e,"rowsTruncated"),columnsTruncated:c(e,"columnsTruncated"),scannedRows:c(e,"scannedRows"),scannedColumns:c(e,"scannedColumns"),scannedCells:c(e,"scannedCells"),lastReadRow:c(e,"lastReadRow"),lastReadColumn:c(e,"lastReadColumn"),matches:t?C(e,"matches").filter(tt).map(i=>({...i,section:c(i,"section"),row:c(i,"row"),column:c(i,"column"),text:c(i,"text")})):[],cells:C(e,"cells").map(i=>({...i,row:c(i,"row"),cells:C(i,"cells").map(a=>({...a,column:c(a,"column"),text:c(a,"text")}))})),readFailed:c(e,"readFailed"),readError:c(e,"readError")}}function Ac(e){let t=!xr(e)&&!wt(e),n=wt(e);return C(e,"schedules").filter(tt).map(r=>{let{nameMatched:o,NameMatched:i,cellMatchCount:a,CellMatchCount:s,sections:l,Sections:u,...h}=r;return{...h,id:c(r,"id"),uniqueId:c(r,"uniqueId"),name:c(r,"name"),viewType:c(r,"viewType"),isTemplate:c(r,"isTemplate"),nameMatched:t?!1:c(r,"nameMatched"),cellMatchCount:n?c(r,"cellMatchCount"):0,sections:C(r,"sections").filter(tt).map(m=>Pc(m,n))}})}function Oc(e,t){for(let[n,r]of Object.entries(t)){let o=n.charAt(0).toUpperCase()+n.slice(1);e[n]=r,e[o]=r}return e}function Ci(e){let t=c(e,"scan");if(!t||typeof t!="object"||Array.isArray(t))return t;let n={...t},r={};return xr(e)||(r.scheduleNameMatchedCount=0),wt(e)||(r.cellMatchedScheduleCount=0,r.totalCellMatches=0),Oc(n,r)}function Vc(e){for(let t of["query","nameQuery","cellQuery","totalSchedules","candidateCount","returnedCount","truncated","maxSchedules","scan","matches"]){let n=c(e,t);n!==void 0&&e[t]===void 0&&(e[t]=n)}return e.scan=Ci(e),e.schedules=Ac(e),wt(e)||(e.matches=[],delete e.Matches),e}function Dc(e){return String(c(e,"id")??c(e,"uniqueId")??c(e,"name")??"")}function Fc(e,t){let n=C(e,"cells"),r=ce(C(e,"matches"),{limit:t}),{cells:o,Cells:i,matches:a,Matches:s,...l}=e;return{...l,matches:r.rows,matchCount:r.totalCount,returnedMatchCount:r.returnedCount,omittedMatchCount:r.omittedCount,duplicateMatchCount:r.duplicateCount,cellsOmitted:n.length>0,cellRowCount:n.length,fullResponseHint:n.length>0?'Use responseMode="full" when downstream schedule adapters need section.cells/body rows.':void 0}}function Lc(e,t){let n=t.responseMode||"compact";if(qe(n))return{...e,responseMode:n};let r=xe(t.maxResultRows,Cc,200),o=xe(t.maxEvidenceRows,Tc,1e3),i=ce(C(e,"schedules"),{limit:r,key:Dc}),a=ce(C(e,"evidenceRows"),{limit:o});return{...e,responseMode:"compact",schedules:i.rows.map(s=>({...s,sections:C(s,"sections").filter(tt).map(l=>Fc(l,o))})),evidenceRows:a.rows,summary:{...e.summary||{},compactResponse:!0,scheduleRowCount:i.totalCount,returnedScheduleRowCount:i.returnedCount,omittedScheduleRowCount:i.omittedCount,duplicateScheduleRowCount:i.duplicateCount,evidenceRowCount:a.totalCount,returnedEvidenceRowCount:a.returnedCount,omittedEvidenceRowCount:a.omittedCount},notices:[...Mc(e.notices),'Compact response omits section.cells and bounds evidence rows. Use responseMode="full" for full schedule cell bodies.']}}function Tr(e,t,n){let r=Cr(e);return Lc(Vc(re(e,{action:"inspect_schedules",elapsedMs:n,partial:r,scanStoppedReason:vi(e),scanPolicy:wr(t),suggestedNextScopes:["nameQuery","scheduleIds","sections","startRow","startColumn","maxRowsPerSection","maxColumnsPerSection","maxCells","maxResponseBytes","maxElapsedMs","allowExpensiveSearch"],summary:Ec,evidenceRows:vr,lastRead:kc})),t)}function Ti(e){e.tool("inspect_schedules","[SCHEDULE_INSPECTION_READ_ONLY] Read-only native Revit schedule discovery and bounded cell inspection with partial-result continuation state. Prefer this over generic send_code_to_revit when finding schedules or reading schedule cells. For large models, use nameQuery/scheduleIds first; broad cell scans require allowExpensiveSearch=true. Default responseMode=compact omits bulky section.cells; use responseMode=full when the next step needs raw schedule body rows, such as reconcile_schedule_excel schedule adaptation.",{...S(L),...x(L),query:L.string().optional().describe("Alias for nameQuery. Matches schedule names with Turkish/diacritic/Cyrillic-U normalization."),nameQuery:L.string().optional().describe("Schedule name filter. Use this first in large projects before scanning cells."),cellQuery:L.string().optional().describe("Optional text to search inside bounded schedule cells. Use with nameQuery or scheduleIds for large projects."),scheduleIds:L.array(L.union([L.number(),L.string()])).optional().describe("Exact ViewSchedule element ids to inspect. Preferred when known."),sections:L.array(L.enum(["header","body","footer"])).optional().describe("Schedule sections to read/scan. Defaults to header and body."),includeCells:L.boolean().optional().describe("Return a bounded cell snapshot for each returned schedule. Defaults false."),scanCells:L.boolean().optional().describe("Scan bounded cells for cellQuery. Defaults true when cellQuery is provided, otherwise false."),allowExpensiveSearch:L.boolean().optional().describe("Explicit approval for scanning schedule cells without scheduleIds/nameQuery. Defaults false."),searchBudget:L.enum(["fast","balanced","deep"]).optional().describe("Native Revit-side scan budget preset. fast is default; deep still respects maxElapsedMs and response-size caps."),maxElapsedMs:L.number().int().positive().max(119e3).optional().describe("Native Revit-side elapsed budget. It is clamped below timeoutMs so partial schedule results can return before transport timeout."),maxSchedules:L.number().int().positive().max(200).optional().describe("Maximum schedules to inspect/return. Defaults 50."),maxRowsPerSection:L.number().int().min(0).max(1e3).optional().describe("Maximum rows per section to read/scan. Defaults 80."),maxColumnsPerSection:L.number().int().min(0).max(200).optional().describe("Maximum columns per section to read/scan. Defaults 30."),startRow:L.number().int().min(0).max(1e5).optional().describe("Zero-based first schedule row to read in each requested section. Defaults 0."),startColumn:L.number().int().min(0).max(1e4).optional().describe("Zero-based first schedule column to read in each requested section. Defaults 0."),maxCells:L.number().int().positive().max(5e5).optional().describe("Global native cap across schedule cells read or scanned. Defaults by searchBudget."),maxResponseBytes:L.number().int().min(4096).max(16*1024*1024).optional().describe("Approximate native response-size cap. Defaults 4 MB."),maxCellTextChars:L.number().int().min(20).max(1e3).optional().describe("Maximum characters retained per returned cell text. Defaults 180."),responseMode:ze,maxResultRows:L.number().int().positive().max(200).optional().describe("Compact-mode cap for returned schedule entries. Defaults 25; full/debug returns all native rows within maxSchedules."),maxEvidenceRows:L.number().int().positive().max(1e3).optional().describe("Compact-mode cap for evidenceRows and per-section matches. Defaults 50."),timeoutMs:L.number().int().positive().max(12e4).optional().describe("Socket timeout in milliseconds. Defaults 120000.")},async t=>{let n=Date.now();try{let r=!!(Array.isArray(t.scheduleIds)&&t.scheduleIds.length>0||String(t.nameQuery||t.query||"").trim());if(!!(t.includeCells===!0||t.scanCells===!0||String(t.cellQuery||"").trim())&&!r&&t.allowExpensiveSearch!==!0)return p(de({action:"inspect_schedules",reason:"needs_scope",message:"Schedule cell scanning without scheduleIds/nameQuery can be expensive in large models. First discover schedules by name, pass exact scheduleIds, or set allowExpensiveSearch=true.",suggestedNextScopes:["nameQuery","scheduleIds","sections","startRow","startColumn","maxRowsPerSection","maxColumnsPerSection","maxCells","maxResponseBytes","maxElapsedMs","allowExpensiveSearch"],scanPolicy:wr(t),elapsedMs:Date.now()-n,summary:{nameQuery:t.nameQuery??t.query??null,cellQuery:t.cellQuery??null,returnedCount:0,matchCount:0}}));let i=xi(t),a=await _("inspect_schedules",_c(t,i),{...I(t,"Inspect Revit schedules"),toolName:"inspect_schedules",timeoutMs:i.timeoutMs});return p(Tr(a&&a.result?a.result:a,t,Date.now()-n))}catch(r){return p(fe({action:"inspect_schedules",error:r instanceof Error?r.message:String(r),elapsedMs:Date.now()-n,scanPolicy:wr(t),suggestedNextScopes:["nameQuery","scheduleIds","sections","startRow","startColumn","maxRowsPerSection","maxColumnsPerSection","maxCells","maxResponseBytes","maxElapsedMs","allowExpensiveSearch"]}))}})}import{z as Dr}from"zod";import*as Uc from"node:fs";import Ii from"node:fs/promises";import $c from"node:path";import{performance as Rr}from"node:perf_hooks";import*as rt from"@e965/xlsx";import{parse as Qc}from"csv-parse/sync";import{z as w}from"zod";var Mn=["identity","comparisonText"],Nn=["identity","comparisonText","code","description","quantity","unit","system","discipline","notes"],En={identity:["identity","id","key","name","item","row","code","type","mark","tag","poz","kod","ad","isim"],comparisonText:["comparisontext","comparison text","description","desc","aciklama","text","name","item","type","mark","tag","ad","isim"],code:["code","kod","type code","mark","tag","poz"],description:["description","desc","text","aciklama"],quantity:["quantity","qty","count","adet","miktar"],unit:["unit","units","birim"],system:["system","sistem"],discipline:["discipline","disiplin"],notes:["notes","note","remarks","remark","not"]},jc={\u0410:"A",\u0430:"A",\u0412:"B",\u0432:"B",\u0415:"E",\u0435:"E",\u041A:"K",\u043A:"K",\u041C:"M",\u043C:"M",\u041D:"H",\u043D:"H",\u041E:"O",\u043E:"O",\u0420:"P",\u0440:"P",\u0421:"C",\u0441:"C",\u0422:"T",\u0442:"T",\u0423:"Y",\u0443:"Y",\u0425:"X",\u0445:"X"},Bc={\u00C7:"C",\u00E7:"C",\u011E:"G",\u011F:"G",\u00D6:"O",\u00F6:"O",\u015E:"S",\u015F:"S",\u00DC:"U",\u00FC:"U"},xt=new Set(["DN","MM","CM","M","KW","KCALH","LPS","M3H"]);function j(e){return String(e??"").replace(/\s+/g," ").trim()}function Oe(e){return j(e).replace(/\u0131/g,"i").replace(/\u0130/g,"I").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().replace(/[^a-z0-9]+/g," ").trim()}function Ut(e){return Oe(e).replace(/\s+/g,"")}function vt(e){let t=String(e??"");return t=t.replace(/[\u0000-\u001f\u007f-\u009f]/g," "),t=t.normalize("NFKC"),t=t.replace(/\u0131/g,"i").replace(/\u0130/g,"I"),t=t.replace(/[\u0400-\u04ff]/g,n=>jc[n]||n),t=t.replace(/[\u00c7\u00e7\u011e\u011f\u00d6\u00f6\u015e\u015f\u00dc\u00fc]/g,n=>Bc[n]||n),t=t.toUpperCase(),t=t.replace(/[\u00d8\u00f8\u2205\u2300\u0424\u0444]/g," DN "),t=t.replace(/\b(?:DIAMETER|DIA)\b/g," DN "),t=Wc(t),t=t.replace(/(\d),(\d)/g,"$1.$2"),t=t.replace(/(\d)\.(\d)/g,"$1DECIMALDOT$2"),t=t.replace(/[^A-Z0-9]+/g," "),t=t.replace(/(\d)DECIMALDOT(\d)/g,"$1.$2"),t=t.replace(/\bM\s*3\s*H\b/g,"M3H"),t.replace(/\s+/g," ").trim()}function zc(e){return e.map(n=>vt(n)).filter((n,r,o)=>n.length>0&&o.indexOf(n)===r).join(" | ")}function Ct(e){let t=zc(e);return{profileVersion:1,normalizedKey:t,tokens:qc(t)}}function qc(e){let t=vt(e),n=t.length>0?t.split(" "):[],r=[];for(let o=0;o<n.length;o++){let i=n[o],a=n[o+1];if(Ht(i)&&a&&xt.has(a)){r.push({type:"dimension",value:`${i}${a}`}),o++;continue}if(xt.has(i)&&a&&Ht(a)){r.push({type:"dimension",value:`${i}${a}`}),o++;continue}let s=Hc(i);if(s){r.push({type:"dimension",value:s});continue}if(xt.has(i)){r.push({type:"unit",value:i});continue}if(Ht(i)){r.push({type:"number",value:i});continue}let l=n[o+2]||"",u=xt.has(l)&&Ht(n[o+3]||""),h=xt.has(l)&&!u;if(Gc(i)&&a&&Ht(a)&&!xt.has(i)&&!h){r.push({type:"code",value:`${i}${a}`}),o++;continue}if(Jc(i)){r.push({type:"code",value:i});continue}r.push({type:"word",value:i})}return r}function Wc(e){return e.replace(/\bM\s*(?:3|\^3)\s*\/\s*H\b/g," M3H ").replace(/\bM3H\b/g," M3H ").replace(/\b(?:L|LT)\s*\/\s*S\b/g," LPS ").replace(/\bLPS\b/g," LPS ").replace(/\bKCAL\s*\/\s*H\b/g," KCALH ").replace(/\bKCALH\b/g," KCALH ").replace(/\bKW\b/g," KW ").replace(/\bMM\b/g," MM ").replace(/\bCM\b/g," CM ").replace(/\bDN\b/g," DN ")}function Ht(e){return/^\d+(?:\.\d+)?$/.test(e)}function Gc(e){return/^[A-Z]+$/.test(e)}function Jc(e){return/[A-Z]/.test(e)&&/\d/.test(e)}function Hc(e){let t=e.match(/^(\d+(?:\.\d+)?)(DN|MM|CM|M|KW|KCALH|LPS|M3H)$/);if(t)return`${t[1]}${t[2]}`;let n=e.match(/^(DN)(\d+(?:\.\d+)?)$/);return n?`${n[1]}${n[2]}`:null}rt.set_fs(Uc);var $t="reconcile_schedule_excel",Vn="excel_ingestion",Tt={maxWorkbookBytes:25*1024*1024,maxSheets:20,maxRows:5e3,maxColumns:100,maxCells:25e4,maxElapsedMs:5e3},Rt={maxWorkbookBytes:100*1024*1024,maxSheets:200,maxRows:5e4,maxColumns:300,maxCells:1e6,maxElapsedMs:119e3},kn=Mn,Pn=Nn,Xc=En,Yc=w.object({sheetName:w.string().min(1).optional(),sheetIndex:w.number().int().positive().optional(),range:w.string().min(1).optional(),headerRow:w.number().int().positive().optional(),dataStartRow:w.number().int().positive().optional()}).strict(),_i=w.object({identity:w.union([w.string().min(1),w.number().int().positive()]).optional(),comparisonText:w.union([w.string().min(1),w.number().int().positive()]).optional(),code:w.union([w.string().min(1),w.number().int().positive()]).optional(),description:w.union([w.string().min(1),w.number().int().positive()]).optional(),quantity:w.union([w.string().min(1),w.number().int().positive()]).optional(),unit:w.union([w.string().min(1),w.number().int().positive()]).optional(),system:w.union([w.string().min(1),w.number().int().positive()]).optional(),discipline:w.union([w.string().min(1),w.number().int().positive()]).optional(),notes:w.union([w.string().min(1),w.number().int().positive()]).optional()}).strict(),Mi=w.object({maxWorkbookBytes:w.number().int().positive().optional(),maxSheets:w.number().int().positive().optional(),maxRows:w.number().int().nonnegative().optional(),maxColumns:w.number().int().positive().optional(),maxCells:w.number().int().positive().optional(),maxElapsedMs:w.number().int().positive().optional()}).strict(),Kc=w.object({kind:w.literal("file"),path:w.string().min(1),format:w.enum(["xlsx","csv","tsv","xls"]).optional(),selection:Yc.optional(),columnMapping:_i.optional(),budgets:Mi.optional()}).strict(),Zc=w.object({kind:w.literal("rows"),sheetName:w.string().min(1).optional(),rows:w.array(w.record(w.unknown())),selection:w.object({headerRow:w.number().int().positive().optional(),dataStartRow:w.number().int().positive().optional()}).strict().optional(),columnMapping:_i.optional(),budgets:Mi.optional()}).strict(),_r=w.discriminatedUnion("kind",[Kc,Zc]);function Ve(e){return j(e)}function An(e){return Oe(e)}function Ri(e){return Ut(e)}function eu(e){return{maxWorkbookBytes:It(e?.maxWorkbookBytes,Tt.maxWorkbookBytes,Rt.maxWorkbookBytes),maxSheets:It(e?.maxSheets,Tt.maxSheets,Rt.maxSheets),maxRows:It(e?.maxRows,Tt.maxRows,Rt.maxRows),maxColumns:It(e?.maxColumns,Tt.maxColumns,Rt.maxColumns),maxCells:It(e?.maxCells,Tt.maxCells,Rt.maxCells),maxElapsedMs:It(e?.maxElapsedMs,Tt.maxElapsedMs,Rt.maxElapsedMs)}}function It(e,t,n){return typeof e!="number"||!Number.isFinite(e)?t:Math.max(0,Math.min(Math.floor(e),n))}function Ni(e,t){let n=(t||$c.extname(e).replace(/^\./,"")).trim().toLowerCase();return n==="xlsx"||n==="csv"||n==="tsv"||n==="xls"?n:"unsupported"}function nt(e,t,n={}){let{warnings:r=[],notices:o=[],suggestedNextScopes:i=[],...a}=n;return de({action:$t,reason:e,message:t,extra:{stage:Vn,ingestionContractVersion:1,...a},summary:n.summary||{},evidenceRows:[],scanPolicy:n.scanPolicy||{},suggestedNextScopes:i,warnings:r,notices:o})}function tu(e,t={}){let{warnings:n=[],notices:r=[],...o}=t;return fe({action:$t,error:e,extra:{stage:Vn,ingestionContractVersion:1,...o},summary:t.summary||{},evidenceRows:[],scanPolicy:t.scanPolicy||{},warnings:n,notices:r})}function nu(e){let t=e.table.warnings.concat(e.mappingWarnings),n=e.table.notices.concat(e.mappingNotices),r=e.table.partial,o=e.table.scanStoppedReason,i=e.records.map(a=>({sourceType:"excelRecord",excelRowId:a.excelRowId,sheetName:a.sheetName,rowNumber:a.rowNumber,identityText:a.identityText,comparisonText:a.comparisonText,normalizedKey:a.normalizedKey}));return re({success:!0,guarded:!1,state:"completed",action:$t,stage:Vn,ingestionContractVersion:1,sourceKind:e.sourceKind,format:e.format,sheetName:e.table.sheetName,excelRecords:e.records,partial:r,scanStoppedReason:o,elapsedMs:e.elapsedMs},{action:$t,partial:r,scanStoppedReason:o,elapsedMs:e.elapsedMs,scanPolicy:{budgets:e.budgets,sourceKind:e.sourceKind,format:e.format,sheetName:e.table.sheetName,sourceRange:e.table.sourceRange,headerRow:e.table.headerRow,dataStartRow:e.table.dataStartRow,columnMapping:ru(e.mapping,e.table)},summary:{sourceKind:e.sourceKind,format:e.format,sheetName:e.table.sheetName,sourceRange:e.table.sourceRange,headerCount:e.table.headers.length,scannedRows:e.table.rows.length,scannedCells:e.table.scannedCells,excelRows:e.records.length,excelRecordCount:e.records.length,emptyExcelRows:e.table.rows.length-e.records.length,formulaCachedValueCount:e.table.formulaCachedValueCount,formulaWithoutCachedValueCount:e.table.formulaWithoutCachedValueCount,partial:r,scanStoppedReason:o},evidenceRows:i,warnings:t,notices:n,lastRead:{lastReadRow:e.table.lastReadRow,lastReadColumn:e.table.lastReadColumn,lastReadItemId:e.records.length>0?e.records[e.records.length-1].excelRowId:null}})}function ru(e,t){let n={};for(let r of Pn){let o=e[r];typeof o=="number"&&(n[r]=t.headers[o]||De(t.startColumn+o))}return n}function De(e){let t=Math.max(1,Math.floor(e)),n="";for(;t>0;){let r=(t-1)%26;n=String.fromCharCode(65+r)+n,t=Math.floor((t-1)/26)}return n}function Ir(e){let t=e.trim().toUpperCase();if(!/^[A-Z]+$/.test(t))return null;let n=0;for(let r of t)n=n*26+(r.charCodeAt(0)-64);return n}function Ei(e,t){if(!e)return t;let n=e.trim().toUpperCase().match(/^([A-Z]+)([0-9]+)(?::([A-Z]+)([0-9]+))?$/);if(!n)return null;let r=Ir(n[1]),o=Number(n[2]),i=n[3]?Ir(n[3]):r,a=n[4]?Number(n[4]):o;return!r||!i||o<1||a<o||i<r?null:{startRow:o,startColumn:r,endRow:a,endColumn:i}}function ou(e,t,n,r){return`${De(t)}${e}:${De(r)}${n}`}function iu(e){return Ve(e).length===0}function au(e){return e.every(t=>iu(t.text))}function su(e,t){let n=new Map;return e.map((r,o)=>{let i=`Column ${De(t+o)}`,a=Ve(r.text)||i,s=An(a)||An(i),l=n.get(s)||0;return n.set(s,l+1),l===0?a:`${a} ${l+1}`})}function On(e){if(e==null)return"";if(e instanceof Date)return Number.isNaN(e.getTime())?"":e.toISOString();if(typeof e=="object"){let t=e;return Array.isArray(t.richText)?Ve(t.richText.map(n=>String(n.text??"")).join("")):t.text!==void 0?Ve(t.text):t.result!==void 0?On(t.result):""}return Ve(e)}function lu(e,t,n,r){let o=rt.utils.encode_cell({r:t-1,c:n-1}),i=`${r}!${o}`,a=e[o];if(!a)return{value:"",text:"",address:i};if(typeof a.f=="string"&&a.f.length>0)return a.v!==void 0&&a.v!==null&&!(typeof a.v=="string"&&a.v.length===0&&(a.w===void 0||a.w===""))?{value:a.v,text:On(a.v)||Ve(a.w),address:i,formulaWithCachedValue:!0}:{value:"",text:"",address:i,formulaWithoutCachedValue:!0};let l=a.v??"";return{value:l,text:On(l)||Ve(a.w),address:i}}function cu(e,t,n,r){return{value:e,text:On(e),address:`${r}!${De(n)}${t}`}}function uu(e,t){return Rr.now()-e>t.maxElapsedMs}function du(e,t,n){let r=[],o=[],i={},a=new Set,s=new Set;for(let u of Pn){let h=n?.[u];if(h!==void 0){let m=fu(h,e,t);if(m===null)return{error:{role:u,reason:"unresolved_column_ref",value:h}};i[u]=m,a.add(m),s.add(u)}}for(let u of Pn){if(i[u]!==void 0)continue;let h=ki(u,e);if(h.length===0)continue;let m=hu(h,a);if(m.kind==="ambiguous")return{error:{role:u,reason:"ambiguous_alias",candidates:m.candidates}};m.kind==="resolved"&&(i[u]=m.match.index,a.add(m.match.index))}for(let u of kn)if(i[u]===void 0)return{error:{role:u,reason:"missing_required_role"}};let l=kn.filter(u=>!s.has(u));if(l.length>0){let u=l.map(h=>`${h}=${e[i[h]]||De(t+i[h])}`).join(", ");o.push(`column_mapping_inferred_from_headers: ${u}. Review or pass explicit columnMapping when first-pass reconciliation looks surprising.`)}return{mapping:i,warnings:r,notices:o}}function mu(e,t){let n={},r={},o=new Set;for(let i of kn){let a=ki(i,e).filter(s=>!o.has(s.index)).sort((s,l)=>s.priority-l.priority||s.index-l.index);n[i]=a.map(s=>({header:s.header,column:De(t+s.index),priority:s.priority})),a.length>0&&(r[i]=a[0].header,o.add(a[0].index))}return{requiredRoles:kn,candidates:n,suggestedColumnMapping:r}}function pu(e,t){let n=Ri(t),r=Xc[e];for(let o=0;o<r.length;o++)if(Ri(r[o])===n)return o;return Number.POSITIVE_INFINITY}function ki(e,t){return t.map((n,r)=>({header:n,index:r,priority:pu(e,n)})).filter(n=>Number.isFinite(n.priority))}function hu(e,t){let n=e.filter(a=>!t.has(a.index)),r=n.length>0?n:e,o=Math.min(...r.map(a=>a.priority)),i=r.filter(a=>a.priority===o);return i.length===1?{kind:"resolved",match:i[0]}:{kind:"ambiguous",candidates:i.map(a=>a.header)}}function fu(e,t,n){if(typeof e=="number"){let s=e-1;return s>=0&&s<t.length?s:null}let r=e.trim(),o=An(r),i=t.map((s,l)=>({header:s,index:l})).filter(s=>An(s.header)===o);if(i.length===1)return i[0].index;let a=Ir(r);if(a!==null){let s=a-n;return s>=0&&s<t.length?s:null}return null}function gu(e,t){let n=[];for(let r of e.rows){if(au(r.cells))continue;let o={};for(let[m,g]of e.headers.entries())o[g]=r.cells[m]?.text??"";let i={};for(let m of Pn){let g=t[m];typeof g=="number"&&(i[m]=r.cells[g]?.text??"")}let a=Ve(i.identity),s=Ve(i.comparisonText),l=Ct([a,s]),u=l.normalizedKey,h=`${e.sheetName}!${r.rowNumber}`;n.push({excelRowId:h,sheetName:e.sheetName,rowNumber:r.rowNumber,sourceRange:e.sourceRange,rawValues:o,mappedValues:i,identityText:a,comparisonText:s,normalizedKey:u,tokenProfile:l})}return n}async function yu(e,t,n){let r=rt.readFile(e.path,{cellDates:!0,cellFormula:!0,cellText:!0,nodim:!0}),o=r.SheetNames.map(h=>({name:h,worksheet:r.Sheets[h]||{}})),i=e.selection||{},a=!!(i.sheetName||i.sheetIndex),s=o.filter(h=>wu(h.worksheet));if(!a&&o.length>t.maxSheets&&s.length!==1)return nt("max_items","Workbook sheet count exceeds maxSheets and cannot be auto-scoped to one non-empty sheet. Provide sheetName or sheetIndex.",{partial:!0,scanStoppedReason:"max_items",summary:{workbookSheets:o.length,nonEmptySheets:s.length,maxSheets:t.maxSheets},scanPolicy:{budgets:t},suggestedNextScopes:["excel.selection.sheetName","excel.selection.sheetIndex","excel.budgets.maxSheets"]});let l=bu(r,i,s);if(!l)return nt("excel_sheet_selection_required","Select a worksheet with sheetName or 1-based sheetIndex.",{summary:{workbookSheets:o.length,sheetNames:o.map(h=>h.name)},scanPolicy:{budgets:t,selection:i},suggestedNextScopes:["excel.selection.sheetName","excel.selection.sheetIndex"]});let u=Su(l,i,t,n);return!a&&s.length===1&&u.notices.push("Selected the only non-empty worksheet."),u}function bu(e,t,n){if(t.sheetName){let r=e.Sheets[t.sheetName];return r?{name:t.sheetName,worksheet:r}:null}if(t.sheetIndex){let r=e.SheetNames[t.sheetIndex-1];return r&&e.Sheets[r]?{name:r,worksheet:e.Sheets[r]}:null}return n.length===1?n[0]:null}function Su(e,t,n,r){let o=xu(e.worksheet);return Ai({sheetName:e.name,fallbackRange:o,selection:t,budgets:n,startedAt:r,readCell:(i,a)=>lu(e.worksheet,i,a,e.name)})}function wu(e){return Object.keys(e).some(t=>!t.startsWith("!"))}function xu(e){let t=Number.POSITIVE_INFINITY,n=Number.POSITIVE_INFINITY,r=1,o=1;for(let i of Object.keys(e))if(!i.startsWith("!"))try{let a=rt.utils.decode_cell(i);t=Math.min(t,a.r+1),n=Math.min(n,a.c+1),r=Math.max(r,a.r+1),o=Math.max(o,a.c+1)}catch{continue}return!Number.isFinite(t)||!Number.isFinite(n)?{startRow:1,startColumn:1,endRow:1,endColumn:1}:{startRow:t,startColumn:n,endRow:r,endColumn:o}}async function vu(e,t,n,r){let o=await Ii.readFile(e.path,"utf8"),i=Cu(e.selection||{},t),a=Qc(o,{bom:!0,delimiter:r==="tsv"?"	":",",relax_column_count:!0,skip_empty_lines:!1,to:i.recordLimit+1}),s=a.length>i.recordLimit?{partial:!0,scanStoppedReason:i.scanStoppedReason}:void 0,l=s?a.slice(0,i.recordLimit):a,u=e.selection?.sheetName||(r==="tsv"?"TSV":"CSV");return Pi(l,u,e.selection||{},t,n,s)}function Cu(e,t){let r=Ei(e.range,{startRow:1,startColumn:1,endRow:1,endColumn:1})?.startRow||1,o=e.headerRow||r,i=e.dataStartRow||o+1;return{recordLimit:Math.max(r,o,i+t.maxRows-1),scanStoppedReason:"max_rows"}}function Tu(e,t,n){let r=e.sheetName||"Rows",o=Ru(e.rows),i=e.selection?.headerRow||1,a=e.selection?.dataStartRow||i+1,s=[];for(;s.length<i-1;)s.push([]);for(s.push(o);s.length<a-1;)s.push([]);for(let l of e.rows)s.push(o.map(u=>l[u]));return Pi(s,r,{headerRow:i,dataStartRow:a},t,n)}function Ru(e){let t=[],n=new Set;for(let r of e)for(let o of Object.keys(r))n.has(o)||(n.add(o),t.push(o));return t}function Pi(e,t,n,r,o,i){let a=e.reduce((l,u)=>Math.max(l,u.length),1),s={startRow:1,startColumn:1,endRow:Math.max(e.length,1),endColumn:Math.max(a,1)};return Ai({sheetName:t,fallbackRange:s,selection:n,budgets:r,startedAt:o,prelimited:i,readCell:(l,u)=>cu(e[l-1]?.[u-1],l,u,t)})}function Ai(e){let t=Ei(e.selection.range,e.fallbackRange);if(!t)throw new Error(`Invalid range selection: ${e.selection.range}`);let n=e.selection.headerRow||t.startRow,r=e.selection.dataStartRow||n+1;if(r<=n)throw new Error("dataStartRow must be greater than headerRow.");let o=t.endColumn,i=e.prelimited?.partial||!1,a=e.prelimited?.scanStoppedReason||"completed";o-t.startColumn+1>e.budgets.maxColumns&&(o=t.startColumn+e.budgets.maxColumns-1,i=!0,a="max_columns");let s=[],l=0,u=0,h=0,m=[],g=[];for(let z=t.startColumn;z<=o;z++){let J=e.readCell(n,z);s.push(J),l++,J.formulaWithCachedValue&&u++,J.formulaWithoutCachedValue&&(h++,m.push(`Formula cell ${J.address||`${e.sheetName}!${De(z)}${n}`} has no cached value and was read as blank.`))}let b=su(s,t.startColumn),y=[],T=null,P=null,te=Math.max(r,t.startRow);for(let z=te;z<=t.endRow;z++){if(y.length>=e.budgets.maxRows){i=!0,a=a==="completed"?"max_rows":a;break}if(uu(e.startedAt,e.budgets)){i=!0,a="max_elapsed";break}if(l+b.length>e.budgets.maxCells){i=!0,a=a==="completed"?"max_cells":a;break}let J=[];for(let q=t.startColumn;q<=o;q++){let X=e.readCell(z,q);J.push(X),l++,T=z,P=q,X.formulaWithCachedValue&&u++,X.formulaWithoutCachedValue&&(h++,m.push(`Formula cell ${X.address||`${e.sheetName}!${De(q)}${z}`} has no cached value and was read as blank.`))}y.push({rowNumber:z,cells:J})}return{sheetName:e.sheetName,sourceRange:ou(t.startRow,t.startColumn,t.endRow,o),headerRow:n,dataStartRow:r,startColumn:t.startColumn,headers:b,rows:y,notices:g,warnings:m,formulaCachedValueCount:u,formulaWithoutCachedValueCount:h,scannedCells:l,partial:i,scanStoppedReason:a,lastReadRow:T,lastReadColumn:P}}function Iu(e){return!!(e&&typeof e=="object"&&e.action===$t&&e.stage===Vn)}async function Oi(e){let t=Rr.now(),n=_r.safeParse(e);if(!n.success)return nt("needs_scope","Excel ingestion input failed schema validation.",{validationIssues:n.error.issues.map(i=>`${i.path.join(".")||"<root>"}: ${i.message}`),suggestedNextScopes:["excel.kind","excel.rows","excel.path","excel.selection","excel.columnMapping.identity","excel.columnMapping.comparisonText"]});let r=n.data,o=eu(r.budgets);try{let i=await _u(r,o,t);if(Iu(i))return i;let a=i,s=du(a.headers,a.startColumn,r.columnMapping);if("error"in s)return nt("excel_column_mapping_required","Resolve identity and comparisonText column mapping before ingestion.",{mappingError:s.error,mappingSuggestion:mu(a.headers,a.startColumn),summary:{sheetName:a.sheetName,headers:a.headers},scanPolicy:{budgets:o},suggestedNextScopes:["excel.columnMapping.identity","excel.columnMapping.comparisonText"],warnings:a.warnings,notices:a.notices});let l=gu(a,s.mapping);return nu({sourceKind:r.kind,format:r.kind==="file"?Ni(r.path,r.format):"rows",table:a,records:l,budgets:o,mapping:s.mapping,mappingNotices:s.notices,mappingWarnings:s.warnings,elapsedMs:Rr.now()-t})}catch(i){return tu(i instanceof Error?i.message:String(i),{scanPolicy:{budgets:o}})}}async function _u(e,t,n){if(e.kind==="rows")return Tu(e,t,n);let r=Ni(e.path,e.format);if(r==="xls")return nt("unsupported_excel_format",".xls is not supported. Save the workbook as .xlsx, .csv, or .tsv.",{format:r,scanPolicy:{budgets:t},suggestedNextScopes:["excel.path","excel.format"]});if(r==="unsupported")return nt("unsupported_excel_format","Unsupported spreadsheet format. Use .xlsx, .csv, or .tsv.",{format:r,scanPolicy:{budgets:t},suggestedNextScopes:["excel.path","excel.format"]});let o=await Ii.stat(e.path);return o.size>t.maxWorkbookBytes?nt("max_bytes","Workbook exceeds maxWorkbookBytes.",{format:r,partial:!0,scanStoppedReason:"max_bytes",summary:{workbookBytes:o.size,maxWorkbookBytes:t.maxWorkbookBytes},scanPolicy:{budgets:t},suggestedNextScopes:["excel.budgets.maxWorkbookBytes","excel.selection.sheetName","excel.selection.range"]}):r==="xlsx"?yu(e,t,n):vu(e,t,n,r)}import{z as f}from"zod";var Dn="reconcile_schedule_records",Er="schedule_record_adapter",We="displayedScheduleCells",Mu=["body"],Mr=Nn,Di=Mn,Nu=En,Eu=f.object({column:f.number().int().nonnegative(),header:f.string().min(1)}).strict(),Fi=f.union([f.array(f.string()),f.array(Eu),f.record(f.union([f.string().min(1),f.number().int().nonnegative()]))]),Li=f.object({identity:f.union([f.string().min(1),f.number().int().nonnegative()]).optional(),comparisonText:f.union([f.string().min(1),f.number().int().nonnegative()]).optional(),code:f.union([f.string().min(1),f.number().int().nonnegative()]).optional(),description:f.union([f.string().min(1),f.number().int().nonnegative()]).optional(),quantity:f.union([f.string().min(1),f.number().int().nonnegative()]).optional(),unit:f.union([f.string().min(1),f.number().int().nonnegative()]).optional(),system:f.union([f.string().min(1),f.number().int().nonnegative()]).optional(),discipline:f.union([f.string().min(1),f.number().int().nonnegative()]).optional(),notes:f.union([f.string().min(1),f.number().int().nonnegative()]).optional()}).strict(),ku=f.object({kind:f.literal("inspect_schedules_result"),result:f.record(f.unknown()),columnMapping:Li.optional(),columnHeaders:Fi.optional(),sections:f.array(f.enum(["header","body","footer"])).optional()}).strict(),Pu=f.object({kind:f.literal("revit_schedule"),scheduleIds:f.array(f.union([f.number().int().positive(),f.string().min(1)])).optional(),nameQuery:f.string().min(1).optional(),sections:f.array(f.enum(["header","body","footer"])).optional(),columnMapping:Li.optional(),columnHeaders:Fi.optional(),target:f.string().optional(),host:f.string().optional(),port:f.number().int().positive().max(65535).optional(),taskName:f.string().optional(),taskId:f.string().optional(),parentTaskName:f.string().optional(),parentTaskId:f.string().optional(),allowExpensiveSearch:f.boolean().optional(),searchBudget:f.enum(["fast","balanced","deep"]).optional(),maxElapsedMs:f.number().int().positive().max(119e3).optional(),maxSchedules:f.number().int().positive().max(200).optional(),maxRowsPerSection:f.number().int().min(0).max(1e3).optional(),maxColumnsPerSection:f.number().int().min(0).max(200).optional(),startRow:f.number().int().min(0).max(1e5).optional(),startColumn:f.number().int().min(0).max(1e4).optional(),maxCells:f.number().int().positive().max(5e5).optional(),maxResponseBytes:f.number().int().min(4096).max(16*1024*1024).optional(),maxCellTextChars:f.number().int().min(20).max(1e3).optional(),timeoutMs:f.number().int().positive().max(12e4).optional()}).strict(),kr=f.discriminatedUnion("kind",[ku,Pu]);async function ji(e,t={}){let n=Date.now(),r=kr.safeParse(e);return r.success?r.data.kind==="revit_schedule"?Au(r.data,n,t):Bi(r.data,Date.now()-n):Fn("needs_scope","Schedule adapter input failed schema validation.",{validationIssues:r.error.issues.map(o=>`${o.path.join(".")||"<root>"}: ${o.message}`),elapsedMs:Date.now()-n,suggestedNextScopes:["schedule.kind","schedule.result","schedule.columnMapping.identity","schedule.columnMapping.comparisonText"]})}async function Au(e,t,n){if(!!!(Array.isArray(e.scheduleIds)&&e.scheduleIds.length>0||String(e.nameQuery||"").trim())&&e.allowExpensiveSearch!==!0)return Fn("needs_scope","Direct live schedule reconciliation requires scheduleIds or nameQuery. Set allowExpensiveSearch=true only when a broad schedule scan is intentional.",{sourceKind:e.kind,elapsedMs:Date.now()-t,suggestedNextScopes:["schedule.scheduleIds","schedule.nameQuery","schedule.allowExpensiveSearch=true"],scanPolicy:{sourceKind:e.kind,bridgeExecution:"inspect_schedules",scheduleIds:[],nameQuery:null,allowExpensiveSearch:!1,visibilityBasis:We}});let i=["header",...Wi(e.sections).filter(g=>g!=="header")],a={query:e.nameQuery,nameQuery:e.nameQuery,scheduleIds:e.scheduleIds,sections:i,includeCells:!0,scanCells:!1,allowExpensiveSearch:e.allowExpensiveSearch,searchBudget:e.searchBudget,maxElapsedMs:e.maxElapsedMs,maxSchedules:e.maxSchedules,maxRowsPerSection:e.maxRowsPerSection,maxColumnsPerSection:e.maxColumnsPerSection,startRow:e.startRow,startColumn:e.startColumn,maxCells:e.maxCells,maxResponseBytes:e.maxResponseBytes,maxCellTextChars:e.maxCellTextChars,responseMode:"full",timeoutMs:e.timeoutMs,taskName:e.taskName||"Inspect live Revit schedule for reconciliation",taskId:e.taskId,parentTaskName:e.parentTaskName,parentTaskId:e.parentTaskId},l=await(n.sendCommand||_)("inspect_schedules",a,{target:e.target,host:e.host,port:e.port,timeoutMs:e.timeoutMs,taskName:a.taskName,taskId:e.taskId,parentTaskName:e.parentTaskName,parentTaskId:e.parentTaskId,toolName:"reconcile_schedule_excel"}),u=Date.now()-t,h=Tr(l&&l.result?l.result:l,a,u),m=Bi({kind:"inspect_schedules_result",result:h,columnMapping:e.columnMapping,columnHeaders:e.columnHeaders,sections:e.sections},u);return m.sourceKind="revit_schedule",m.bridgeSourceKind="inspect_schedules_result",m.scanPolicy={...m.scanPolicy||{},sourceKind:"revit_schedule",bridgeExecution:"inspect_schedules",inspectSections:i,scheduleIds:e.scheduleIds||[],nameQuery:e.nameQuery||null,allowExpensiveSearch:e.allowExpensiveSearch===!0},m.notices=[...ot(m,"notices"),"Live Revit schedule input was read through bounded inspect_schedules before reconciliation."],m}function Bi(e,t){let n=e.result,r=j(c(n,"state")).toLowerCase();if(c(n,"success")===!1||r==="failed"||c(n,"error"))return qu(j(c(n,"error"))||"inspect_schedules_result failed before schedule adaptation.",{sourceKind:e.kind,elapsedMs:t,warnings:ot(n,"warnings"),notices:ot(n,"notices")});if(c(n,"guarded")===!0)return Fn(j(c(n,"reason"))||"needs_scope","inspect_schedules_result was guarded before schedule adaptation.",{sourceKind:e.kind,elapsedMs:t,warnings:ot(n,"warnings"),notices:ot(n,"notices"),summary:c(n,"summary")||{},suggestedNextScopes:['inspect_schedules responseMode="full"',"schedule.result","schedule.columnMapping.identity","schedule.columnMapping.comparisonText"]});let o=Wi(e.sections),i=C(n,"schedules"),a=ot(n,"warnings"),s=ot(n,"notices"),l=[],u=0,h=0,m=0;for(let T of i){let P=Gi(c(T,"id"));if(!P){a.push("Skipped a schedule without id while adapting schedule records.");continue}let te=Ar(c(T,"name")),z=Du(T,e.columnHeaders),J=Lu(z,e.columnMapping);if("error"in J)return Fn("schedule_column_mapping_required","Resolve identity and comparisonText schedule column mapping before adaptation.",{sourceKind:e.kind,scheduleId:P,scheduleName:te,mappingError:J.error,summary:{scheduleId:P,scheduleName:te,headers:z.map(q=>({column:q.column,header:q.header}))},scanPolicy:Vi(e,o),suggestedNextScopes:["schedule.columnMapping.identity","schedule.columnMapping.comparisonText",'inspect_schedules responseMode="full"'],warnings:a,notices:s});for(let q of C(T,"sections")){let X=Pr(c(q,"section"));if(o.includes(X))for(let ye of zi(q,P,te,X)){if(u++,h+=ye.cells.length,X==="body"&&Ou(ye,J.mapping,z)){m++;continue}let Re=Vu(ye,J.mapping);Re&&l.push(Re)}}}let g=c(n,"partial")===!0,b=Jt(c(n,"scanStoppedReason"),g?"max_items":"completed"),y=l.length>0?l[l.length-1]:null;return re({success:!0,guarded:!1,state:"completed",action:Dn,stage:Er,adapterContractVersion:1,sourceKind:e.kind,visibilityBasis:We,scheduleRecords:l,partial:g,scanStoppedReason:b,elapsedMs:t},{action:Dn,partial:g,scanStoppedReason:b,elapsedMs:t,scanPolicy:Vi(e,o),summary:{sourceKind:e.kind,scheduleCount:i.length,scannedRows:u,scannedCells:h,skippedHeaderLikeRows:m,scheduleRecordCount:l.length,visibilityBasis:We,partial:g,scanStoppedReason:b},evidenceRows:l.map(T=>({sourceType:"scheduleRecord",scheduleRowId:T.scheduleRowId,scheduleId:T.scheduleId,scheduleName:T.scheduleName,section:T.section,row:T.row,identityText:T.identityText,comparisonText:T.comparisonText,normalizedKey:T.normalizedKey,visibilityBasis:We})),warnings:a,notices:m>0?[...s,`Skipped ${m} header-like body row(s) during schedule adaptation.`]:s,lastRead:{lastReadSection:c(n,"lastReadSection")??y?.section??null,lastReadRow:c(n,"lastReadRow")??y?.row??null,lastReadColumn:c(n,"lastReadColumn")??null,lastReadItemId:c(n,"lastReadItemId")??y?.scheduleRowId??null}})}function Ou(e,t,n){let r=new Map;for(let a of e.cells)r.set(a.column,a.text);let o=Di.filter(a=>typeof t[a]=="number");if(o.length===0)return!1;let i=new Map;for(let a of o){let s=t[a];typeof s=="number"&&i.set(s,[...i.get(s)||[],a])}return[...i.entries()].every(([a,s])=>{let l=j(r.get(a));if(!l)return!1;let u=Oe(l);return n.some(m=>m.column===a&&Oe(m.header)===u)?!0:s.some(m=>Number.isFinite(qi(m,l))||m==="identity"&&["number","no","numara"].includes(u)?!0:m==="comparisonText"&&["name","description","desc","text","aciklama"].includes(u))})}function Vu(e,t){let n=new Map;for(let s of e.cells)n.set(s.column,s.text);let r={};for(let s of Mr){let l=t[s];typeof l=="number"&&(r[s]=j(n.get(l)))}let o=j(r.identity),i=j(r.comparisonText);if(!o&&!i)return null;let a=Ct([o,i]);return{scheduleRowId:`${e.scheduleId}:${e.section}:${e.row}`,scheduleId:e.scheduleId,scheduleName:e.scheduleName,section:e.section,row:e.row,rawCells:e.cells.map(s=>({column:s.column,text:s.text})),mappedValues:r,identityText:o,comparisonText:i,normalizedKey:a.normalizedKey,tokenProfile:a,visibilityBasis:We}}function zi(e,t,n,r){let o=C(e,"rows"),i=C(e,"cells");return(o.length>0?o:i).flatMap(s=>{let l=Nr(c(s,"row"));if(l===null)return[];let u=C(s,"cells").map(h=>({column:Nr(c(h,"column")),text:j(c(h,"text"))})).filter(h=>h.column!==null);return[{scheduleId:t,scheduleName:n,section:r,row:l,cells:u}]})}function Du(e,t){let n=[],r=new Set,o=(i,a)=>{let s=j(a);if(s.length===0)return;let l=`${i}:${Oe(s)}`;r.has(l)||(r.add(l),n.push({column:i,header:s}))};for(let i of C(e,"sections"))if(Pr(c(i,"section"))==="header")for(let a of zi(i,Gi(c(e,"id"))||"unknown",Ar(c(e,"name")),"header"))for(let s of a.cells)o(s.column,s.text);for(let i of Fu(t))o(i.column,i.header);return n.sort((i,a)=>i.column-a.column)}function Fu(e){if(!e)return[];if(Array.isArray(e))return e.map((n,r)=>typeof n=="string"?{column:r,header:j(n)}:{column:n.column,header:j(n.header)}).filter(n=>n.header.length>0);let t=[];for(let[n,r]of Object.entries(e)){let o=Nr(n);if(o!==null&&typeof r=="string"){let i=j(r);i.length>0&&t.push({column:o,header:i});continue}if(typeof r=="number"){let i=j(n);i.length>0&&t.push({column:r,header:i})}}return t.sort((n,r)=>n.column-r.column)}function Lu(e,t){let n=[],r=[],o={},i=new Set;for(let a of Mr){let s=t?.[a];if(s!==void 0){let l=ju(s,e);if(l===null)return{error:{role:a,reason:"unresolved_column_ref",value:s}};o[a]=l,i.add(l)}}for(let a of Mr){if(o[a]!==void 0)continue;let s=Bu(a,e);if(s.length===0)continue;let l=zu(s,i);if(l.kind==="ambiguous")return{error:{role:a,reason:"ambiguous_alias",candidates:l.candidates}};o[a]=l.match.column,i.add(l.match.column)}for(let a of Di)if(o[a]===void 0)return{error:{role:a,reason:"missing_required_role"}};return{mapping:o,warnings:n,notices:r}}function ju(e,t){if(typeof e=="number")return t.length>0&&!t.some(i=>i.column===e)?null:e;let n=e.trim(),r=Oe(n),o=t.filter(i=>Oe(i.header)===r);return o.length===1?o[0].column:null}function qi(e,t){let n=Ut(t),r=Nu[e];for(let o=0;o<r.length;o++)if(Ut(r[o])===n)return o;return Number.POSITIVE_INFINITY}function Bu(e,t){return t.map(n=>({header:n.header,column:n.column,priority:qi(e,n.header)})).filter(n=>Number.isFinite(n.priority))}function zu(e,t){let n=e.filter(s=>!t.has(s.column)),r=n.length>0?n:e,o=Math.min(...r.map(s=>s.priority)),i=r.filter(s=>s.priority===o);return i.length===1?{kind:"resolved",match:i[0]}:[...new Set(i.map(s=>s.column))].length===1?{kind:"resolved",match:i[0]}:{kind:"ambiguous",candidates:i.map(s=>s.header)}}function Vi(e,t){return{sourceKind:e.kind,sections:t,columnMapping:e.columnMapping||null,numericColumnBase:"zero_based_revit_schedule_column",visibilityBasis:We}}function Fn(e,t,n={}){let{warnings:r=[],notices:o=[],elapsedMs:i,scanPolicy:a,summary:s,suggestedNextScopes:l=[],...u}=n;return de({action:Dn,reason:e,message:t,elapsedMs:i,extra:{stage:Er,adapterContractVersion:1,visibilityBasis:We,...u},summary:s||{},evidenceRows:[],scanPolicy:a||{},suggestedNextScopes:l,warnings:r,notices:o})}function qu(e,t={}){let{warnings:n=[],notices:r=[],elapsedMs:o,scanPolicy:i,summary:a,...s}=t;return fe({action:Dn,error:e,elapsedMs:o,extra:{stage:Er,adapterContractVersion:1,visibilityBasis:We,...s},summary:a||{},evidenceRows:[],scanPolicy:i||{},warnings:n,notices:r})}function Wi(e){let t=Array.isArray(e)&&e.length>0?e:Mu;return[...new Set(t.map(Pr))].filter(n=>["header","body","footer"].includes(n))}function Pr(e){let t=j(e).toLowerCase();return["header","body","footer"].includes(t)?t:"body"}function ot(e,t){let n=c(e,t);return Array.isArray(n)?n.map(j).filter(r=>r.length>0):[]}function Nr(e){if(typeof e=="number")return Number.isFinite(e)?e:null;if(typeof e=="string"){let t=e.trim();if(t.length===0)return null;let n=Number(t);return Number.isFinite(n)?n:null}return null}function Gi(e){return Ar(e)}function Ar(e){let t=j(e);return t.length>0?t:null}import{z as N}from"zod";var Qt={score:{exact:100,diceTokenOverlap:35,code:20,dimension:20,order:15,context:10},thresholds:{highConfidenceMin:86,highConfidenceMax:99,candidateMin:65,possibleRenameMin:72,possibleRenameMax:85,ambiguousMin:65,ambiguousMax:71,candidateGap:8,tieGap:8},caps:{conflictingCode:64,conflictingDimension:60,unitMismatch:79},candidateGeneration:{minSharedSignificantWordTokens:2},contextFields:["system","unit","quantity","discipline"]},Wu=N.object({exact:N.number().min(0).max(100).optional(),diceTokenOverlap:N.number().min(0).max(100).optional(),code:N.number().min(0).max(100).optional(),dimension:N.number().min(0).max(100).optional(),order:N.number().min(0).max(100).optional(),context:N.number().min(0).max(100).optional()}).strict(),Gu=N.object({highConfidenceMin:N.number().min(0).max(100).optional(),highConfidenceMax:N.number().min(0).max(100).optional(),candidateMin:N.number().min(0).max(100).optional(),possibleRenameMin:N.number().min(0).max(100).optional(),possibleRenameMax:N.number().min(0).max(100).optional(),ambiguousMin:N.number().min(0).max(100).optional(),ambiguousMax:N.number().min(0).max(100).optional(),candidateGap:N.number().min(0).max(100).optional(),tieGap:N.number().min(0).max(100).optional()}).strict(),Ju=N.object({conflictingCode:N.number().min(0).max(100).optional(),conflictingDimension:N.number().min(0).max(100).optional(),unitMismatch:N.number().min(0).max(100).optional()}).strict(),Hu=N.object({minSharedSignificantWordTokens:N.number().int().min(0).max(20).optional()}).strict(),jn=N.object({score:Wu.optional(),thresholds:Gu.optional(),caps:Ju.optional(),candidateGeneration:Hu.optional(),contextFields:N.array(N.string().min(1)).optional()}).strict(),Uu=N.object({excelRecords:N.array(N.record(N.unknown())).optional(),scheduleRecords:N.array(N.record(N.unknown())).optional(),excelResult:N.record(N.unknown()).optional(),scheduleResult:N.record(N.unknown()).optional(),config:jn.optional()}).strict();function Xi(e){let t=Date.now(),n=Uu.safeParse(e);if(!n.success)return re({success:!0,guarded:!0,state:"guarded",action:"reconcile_schedule_excel",stage:"matching_scoring",reconciliationContractVersion:1,reason:"reconciliation_input_required",message:"Provide excelRecords and scheduleRecords, or normalized ingestion result envelopes containing those arrays.",validationIssues:n.error.issues.map(l=>l.message),partial:!1,scanStoppedReason:"needs_scope"},{action:"reconcile_schedule_excel",partial:!1,scanStoppedReason:"needs_scope",elapsedMs:Date.now()-t,summary:{},evidenceRows:[]});let r=od(n.data.config),o=Ji("excel",n.data.excelRecords??Hi(n.data.excelResult,"excelRecords")),i=Ji("schedule",n.data.scheduleRecords??Hi(n.data.scheduleResult,"scheduleRecords")),a=$u(o,i,r),s=id(o,i,a);return re({success:!0,guarded:!1,state:"review_ready",action:"reconcile_schedule_excel",stage:"matching_scoring",reconciliationContractVersion:1,partial:!1,scanStoppedReason:"completed",reviewRows:a,reviewTable:ad(a),suggestedNextActions:["review_ambiguous","accept_match","create_schedule_row","remove_or_ignore_schedule_row","rename_excel_or_schedule_text"],scoringConfig:r},{action:"reconcile_schedule_excel",partial:!1,scanStoppedReason:"completed",elapsedMs:Date.now()-t,summary:s,evidenceRows:a.map(l=>({sourceType:"reconciliationReviewRow",bucket:l.bucket,score:l.score,excelRowId:l.excelRow?.excelRowId??l.excelRow?.recordId??null,scheduleRowId:l.scheduleRow?.scheduleRowId??l.scheduleRow?.recordId??null,reason:l.reason}))})}function $u(e,t,n){let r=[],o=new Set,i=new Set,a=Qi(e),s=Qi(t);for(let l of e){let u=Xu(l,t,n),h=l.normalizedKey.length>0&&(a.has(l.normalizedKey)||s.has(l.normalizedKey)),m=u[0]||null;if(h&&u.some(b=>b.score===n.score.exact||b.schedule.normalizedKey===l.normalizedKey)){let b=u.filter(y=>y.schedule.normalizedKey===l.normalizedKey||y.score>=n.thresholds.candidateMin).slice(0,5);r.push(Or("ambiguousMatches",b[0]||null,l,null,b,"duplicate_exact_key","review_ambiguous")),o.add(l.id),b.forEach(y=>i.add(y.schedule.id));continue}if(!m||m.score<n.thresholds.candidateMin&&m.hardConflicts.length===0){r.push(ed(l)),o.add(l.id);continue}if(i.has(m.schedule.id)){r.push(Or("ambiguousMatches",m,l,m.schedule,u.slice(0,5),"schedule_row_already_claimed","review_ambiguous")),o.add(l.id);continue}let g=Qu(m,u[1]||null,n);r.push(Or(g.bucket,m,l,m.schedule,u.slice(0,5),g.reason,g.action)),o.add(l.id),i.add(m.schedule.id),g.bucket==="ambiguousMatches"&&u.filter(b=>b.score>=n.thresholds.candidateMin).slice(0,5).forEach(b=>i.add(b.schedule.id))}for(let l of t)i.has(l.id)||r.push(td(l));return r.sort(pd)}function Qu(e,t,n){let r=t?e.score-t.score:Number.POSITIVE_INFINITY,o=t!==null&&e.score===t.score;if(o||r<n.thresholds.tieGap||e.score>=n.thresholds.ambiguousMin&&e.score<=n.thresholds.ambiguousMax)return{bucket:"ambiguousMatches",reason:o?"best_score_tie":r<n.thresholds.tieGap?"candidate_gap_below_threshold":"ambiguous_score_band",action:"review_ambiguous"};if(e.components.exact>0&&e.hardConflicts.length===0&&e.score===n.score.exact)return{bucket:"exactMatches",reason:"exact_normalized_key",action:"accept_match"};let i=(e.sharedCodeTokens.length>0||e.sharedDimensionTokens.length>0)&&e.descriptiveTokensDiffer;return!e.hardConflicts.length&&e.score>=n.thresholds.highConfidenceMin&&i?{bucket:"possibleRenames",reason:"shared_key_tokens_with_description_change",action:"rename_excel_or_schedule_text"}:e.score>=n.thresholds.highConfidenceMin&&e.score<=n.thresholds.highConfidenceMax&&!e.capped&&r>=n.thresholds.candidateGap?{bucket:"highConfidenceMatches",reason:"high_confidence_score_and_gap",action:"accept_match"}:!e.hardConflicts.length&&(e.score>=n.thresholds.highConfidenceMin&&i||e.score>=n.thresholds.possibleRenameMin&&e.score<=n.thresholds.possibleRenameMax)?{bucket:"possibleRenames",reason:i?"shared_key_tokens_with_description_change":"possible_rename_score_band",action:"rename_excel_or_schedule_text"}:{bucket:"ambiguousMatches",reason:e.hardConflicts.length>0?"hard_conflict_requires_review":"requires_review",action:"review_ambiguous"}}function Xu(e,t,n){return t.filter(r=>Yu(e,r,n)).map(r=>({...Ku(e,r,n),excel:e,schedule:r})).sort(md)}function Yu(e,t,n){return e.normalizedKey.length>0&&e.normalizedKey===t.normalizedKey||ve(G(e,"code"),G(t,"code")).length>0||ve(G(e,"dimension"),G(t,"dimension")).length>0?!0:ve(G(e,"word"),G(t,"word")).length>=n.candidateGeneration.minSharedSignificantWordTokens}function Ku(e,t,n){let r=e.normalizedKey.length>0&&e.normalizedKey===t.normalizedKey,o=_t(e.tokenProfile.tokens.map(y=>y.value)),i=_t(t.tokenProfile.tokens.map(y=>y.value)),a=ve(o,i),s=_t(o.concat(i).filter(y=>!a.includes(y))),l=ve(G(e,"code"),G(t,"code")),u=ve(G(e,"dimension"),G(t,"dimension")),h=Zu(e,t),m={exact:r?n.score.exact:0,dice:r?0:Ln(ld(o,i)*n.score.diceTokenOverlap),code:r?0:$i(G(e,"code"),G(t,"code"),n.score.code),dimension:r?0:$i(G(e,"dimension"),G(t,"dimension"),n.score.dimension),order:r?0:Ln(cd(o,i)*n.score.order),context:r?0:sd(e,t,n)},g=r?n.score.exact:Vr(m.dice+m.code+m.dimension+m.order+m.context),b=g;for(let y of h)y==="conflicting_code"&&(b=Math.min(b,n.caps.conflictingCode)),y==="conflicting_dimension"&&(b=Math.min(b,n.caps.conflictingDimension)),y==="unit_mismatch"&&(b=Math.min(b,n.caps.unitMismatch));return{score:Vr(b),rawScore:Vr(g),components:m,matchedTokens:a,differingTokens:s,hardConflicts:h,sharedCodeTokens:l,sharedDimensionTokens:u,descriptiveTokensDiffer:dd(e,t),capped:b<g}}function Zu(e,t){let n=[],r=G(e,"code"),o=G(t,"code");r.length>0&&o.length>0&&ve(r,o).length===0&&n.push("conflicting_code");let i=G(e,"dimension"),a=G(t,"dimension");i.length>0&&a.length>0&&ve(i,a).length===0&&n.push("conflicting_dimension");let s=Ui(e),l=Ui(t);return s.length>0&&l.length>0&&ve(s,l).length===0&&n.push("unit_mismatch"),n}function Or(e,t,n,r,o,i,a){return{bucket:e,score:t?.score??0,rawScore:t?.rawScore??0,reason:i,matchedTokens:t?.matchedTokens??[],differingTokens:t?.differingTokens??[],hardConflicts:t?.hardConflicts??[],scoreComponents:t?.components??null,excelRow:n?Xt(n):null,scheduleRow:r?Xt(r):null,candidateRows:o.map(s=>({score:s.score,rawScore:s.rawScore,scheduleRow:Xt(s.schedule),matchedTokens:s.matchedTokens,hardConflicts:s.hardConflicts})),recommendedNextAction:a}}function ed(e){return{bucket:"missingInSchedule",score:0,rawScore:0,reason:"no_schedule_candidate_at_threshold",matchedTokens:[],differingTokens:e.tokenProfile.tokens.map(t=>t.value),hardConflicts:[],scoreComponents:null,excelRow:Xt(e),scheduleRow:null,candidateRows:[],recommendedNextAction:"create_schedule_row"}}function td(e){return{bucket:"missingInExcel",score:0,rawScore:0,reason:"no_excel_candidate_at_threshold",matchedTokens:[],differingTokens:e.tokenProfile.tokens.map(t=>t.value),hardConflicts:[],scoreComponents:null,excelRow:null,scheduleRow:Xt(e),candidateRows:[],recommendedNextAction:"remove_or_ignore_schedule_row"}}function Xt(e){return{...e.raw,recordId:e.id,normalizedKey:e.normalizedKey,tokenProfile:e.tokenProfile}}function Ji(e,t){return Array.isArray(t)?t.filter(n=>!!n&&typeof n=="object"&&!Array.isArray(n)).map((n,r)=>nd(e,n,r)):[]}function nd(e,t,n=0){let r=e==="excel"?j(t.excelRowId||t.recordId||t.id):j(t.scheduleRowId||t.recordId||t.id),o=Yt(t.mappedValues)?t.mappedValues:{},i=rd(t,[t.identityText,t.comparisonText]);return{side:e,id:r||`${e}:${i.normalizedKey||"row"}:${n}`,normalizedKey:j(t.normalizedKey)||i.normalizedKey,tokenProfile:i,raw:t,mappedValues:o}}function rd(e,t){let n=Yt(e.tokenProfile)?e.tokenProfile:null;return n&&Array.isArray(n.tokens)&&typeof n.normalizedKey=="string"?{profileVersion:1,normalizedKey:j(n.normalizedKey),tokens:n.tokens.filter(r=>Yt(r)&&typeof r.type=="string"&&typeof r.value=="string").map(r=>({type:r.type,value:j(r.value)})).filter(r=>r.value.length>0)}:Ct(t)}function Hi(e,t){return Yt(e)&&Array.isArray(e[t])?e[t].filter(n=>Yt(n)):[]}function od(e){let t=jn.safeParse(e||{}),n=t.success?t.data:{};return{score:{...Qt.score,...n.score||{}},thresholds:{...Qt.thresholds,...n.thresholds||{}},caps:{...Qt.caps,...n.caps||{}},candidateGeneration:{...Qt.candidateGeneration,...n.candidateGeneration||{}},contextFields:n.contextFields||Qt.contextFields}}function id(e,t,n){let r=Object.fromEntries(["exactMatches","highConfidenceMatches","possibleRenames","ambiguousMatches","missingInSchedule","missingInExcel"].map(o=>[o,0]));for(let o of n)r[o.bucket]=(r[o.bucket]||0)+1;return{excelRows:e.length,scheduleRows:t.length,...r,reviewRowCount:n.length}}function ad(e){return{columns:[{key:"bucket",label:"Bucket"},{key:"score",label:"Score"},{key:"reason",label:"Reason"},{key:"excelRowId",label:"Excel Row"},{key:"scheduleRowId",label:"Schedule Row"},{key:"excelText",label:"Excel Text"},{key:"scheduleText",label:"Schedule Text"},{key:"hardConflicts",label:"Hard Conflicts"},{key:"recommendedNextAction",label:"Recommended Action"}],rows:e.map(n=>({bucket:n.bucket,score:n.score,reason:n.reason,excelRowId:n.excelRow?.excelRowId??n.excelRow?.recordId??"",scheduleRowId:n.scheduleRow?.scheduleRowId??n.scheduleRow?.recordId??"",excelText:n.excelRow?[n.excelRow.identityText,n.excelRow.comparisonText].filter(Boolean).join(" | "):"",scheduleText:n.scheduleRow?[n.scheduleRow.identityText,n.scheduleRow.comparisonText].filter(Boolean).join(" | "):"",hardConflicts:(n.hardConflicts||[]).join(", "),recommendedNextAction:n.recommendedNextAction}))}}function G(e,t){return _t(e.tokenProfile.tokens.filter(n=>n.type===t).map(n=>n.value))}function Ui(e){let t=G(e,"unit");for(let r of G(e,"dimension")){let o=r.match(/^[A-Z]+|[A-Z]+$/)?.[0];o&&t.push(o)}let n=vt(e.mappedValues.unit);return n&&t.push(n),_t(t)}function $i(e,t,n){if(e.length===0||t.length===0)return 0;let r=ve(e,t).length,o=Math.max(e.length,t.length);return Ln(r/o*n)}function sd(e,t,n){let r=n.contextFields.map(i=>[vt(e.mappedValues[i]),vt(t.mappedValues[i])]).filter(([i,a])=>i.length>0&&a.length>0);if(r.length===0)return 0;let o=r.filter(([i,a])=>i===a).length;return Ln(o/r.length*n.score.context)}function ld(e,t){return e.length===0&&t.length===0?1:e.length===0||t.length===0?0:2*ve(e,t).length/(e.length+t.length)}function cd(e,t){let n=Math.min(e.length,t.length);return n===0?0:ud(e,t)/n}function ud(e,t){let n=Array.from({length:e.length+1},()=>Array(t.length+1).fill(0));for(let r=1;r<=e.length;r++)for(let o=1;o<=t.length;o++)n[r][o]=e[r-1]===t[o-1]?n[r-1][o-1]+1:Math.max(n[r-1][o],n[r][o-1]);return n[e.length][t.length]}function dd(e,t){let n=G(e,"word"),r=G(t,"word");return n.length>0&&r.length>0&&!hd(n,r)}function Qi(e){let t=new Map;for(let n of e)n.normalizedKey.length>0&&t.set(n.normalizedKey,(t.get(n.normalizedKey)||0)+1);return new Set([...t.entries()].filter(([,n])=>n>1).map(([n])=>n))}function md(e,t){return t.score!==e.score?t.score-e.score:e.schedule.id.localeCompare(t.schedule.id)}function pd(e,t){let n={exactMatches:0,highConfidenceMatches:1,possibleRenames:2,ambiguousMatches:3,missingInSchedule:4,missingInExcel:5},r=n[e.bucket]??99,o=n[t.bucket]??99;if(r!==o)return r-o;if((t.score||0)!==(e.score||0))return(t.score||0)-(e.score||0);let i=e.excelRow?.recordId||e.scheduleRow?.recordId||"",a=t.excelRow?.recordId||t.scheduleRow?.recordId||"";return String(i).localeCompare(String(a))}function ve(e,t){let n=new Set(t);return _t(e.filter(r=>n.has(r)))}function _t(e){return[...new Set(e.filter(t=>j(t).length>0))]}function hd(e,t){let n=new Set(e),r=new Set(t);return n.size!==r.size?!1:[...n].every(o=>r.has(o))}function Ln(e){return Math.round(e)}function Vr(e){return Math.max(0,Math.min(100,Math.round(e)))}function Yt(e){return!!e&&typeof e=="object"&&!Array.isArray(e)}var Zi="reconcile_schedule_excel",fd=50,it=Dr.object({excel:_r.describe('Excel/CSV source. Use kind:"file" for .xlsx/.csv/.tsv or kind:"rows" for deterministic CI/dry-run records.'),schedule:kr.describe('Schedule source. Use kind:"inspect_schedules_result" with a normalized inspect_schedules result, or kind:"revit_schedule" to read bounded live Revit schedule rows through inspect_schedules before reconciliation.'),config:jn.optional().describe("Optional scoring/cap/threshold override. Defaults are conservative and can be tuned from real-data dry-runs."),responseMode:ze,maxReviewRows:Dr.number().int().positive().max(1e3).optional().describe("Compact-mode cap for returned reviewTable/evidenceRows rows. Defaults 50; full/debug returns all reviewRows."),maxCandidateRows:Dr.number().int().positive().max(10).optional().describe("Compatibility input for older callers. Compact mode omits nested candidateRows; full/debug returns all candidates.")}).strict();function Fr(e,t,n,r={}){let{warnings:o=[],notices:i=[],scanPolicy:a={},summary:s={},suggestedNextScopes:l=[],...u}=r;return de({action:Zi,reason:t,message:n,extra:{stage:e,reconciliationContractVersion:1,...u},summary:s,evidenceRows:[],scanPolicy:a,suggestedNextScopes:l,warnings:o,notices:i})}function Lr(e,t,n={}){let{warnings:r=[],notices:o=[],scanPolicy:i={},summary:a={},suggestedNextScopes:s=[],...l}=n;return fe({action:Zi,error:t,extra:{stage:e,reconciliationContractVersion:1,...l},summary:a,evidenceRows:[],scanPolicy:i,suggestedNextScopes:s,warnings:r,notices:o})}function Yi(e){return e.guarded===!0||e.state==="guarded"}function Ki(e){return e.success===!1||e.state==="failed"||!!e.error}function at(e){return Array.isArray(e)?e.map(t=>String(t??"").trim()).filter(t=>t.length>0):[]}function gd(...e){for(let t of e){let n=String(t.scanStoppedReason||"").trim();if(n&&n!=="completed")return n}return null}var yd={requiredRoles:["identity","comparisonText"],optionalRoles:["code","description","quantity","unit","system","discipline","notes"]},bd={rowsSource:{excel:{kind:"rows",sheetName:"Items",rows:[{Identity:"FCU-101",Description:"Fan coil supply DN100",Unit:"PCS"}],columnMapping:{identity:"Identity",comparisonText:"Description",unit:"Unit"}},schedule:{kind:"inspect_schedules_result",result:{success:!0,schedules:[{id:7001,name:"Mechanical Equipment Schedule",sections:[{section:"header",rows:[{row:0,cells:[{column:0,text:"Identity"},{column:1,text:"Description"}]}]},{section:"body",rows:[{row:1,cells:[{column:0,text:"FCU-101"},{column:1,text:"Fan coil supply DN100"}]}]}]}]}},responseMode:"compact"},fileSource:{excel:{kind:"file",path:"C:\\path\\items.xlsx",format:"xlsx",selection:{sheetName:"Items",headerRow:1,dataStartRow:2},columnMapping:{identity:"Identity",comparisonText:"Description"}},schedule:{kind:"inspect_schedules_result",result:'inspect_schedules result with responseMode="full" when schedule body cells are needed'}}};function Sd(e){return[e.bucket,e.reason,e.score,e.excelRow?.excelRowId??e.excelRow?.recordId??"",e.scheduleRow?.scheduleRowId??e.scheduleRow?.recordId??""].join("|")}function wd(e,t){let n=Array.isArray(t.columns)?t.columns:[{key:"bucket",label:"Bucket"},{key:"score",label:"Score"},{key:"reason",label:"Reason"},{key:"excelRowId",label:"Excel Row"},{key:"scheduleRowId",label:"Schedule Row"},{key:"excelText",label:"Excel Text"},{key:"scheduleText",label:"Schedule Text"},{key:"hardConflicts",label:"Hard Conflicts"},{key:"recommendedNextAction",label:"Recommended Action"}];return{...t,columns:n,rows:e.map(r=>({bucket:r.bucket,score:r.score,reason:r.reason,excelRowId:r.excelRow?.excelRowId??r.excelRow?.recordId??"",scheduleRowId:r.scheduleRow?.scheduleRowId??r.scheduleRow?.recordId??"",excelText:r.excelRow?[r.excelRow.identityText,r.excelRow.comparisonText].filter(Boolean).join(" | "):"",scheduleText:r.scheduleRow?[r.scheduleRow.identityText,r.scheduleRow.comparisonText].filter(Boolean).join(" | "):"",hardConflicts:Array.isArray(r.hardConflicts)?r.hardConflicts.join(", "):"",recommendedNextAction:r.recommendedNextAction}))}}function xd(e,t){let n=t.responseMode||"compact";if(qe(n))return{...e,responseMode:n};let r=xe(t.maxReviewRows,fd,1e3),o=ce(e.reviewRows,{limit:r,key:Sd}),i=ce(e.evidenceRows,{limit:r}),{reviewRows:a,reviewTable:s,scoringConfig:l,sourceSummary:u,...h}=e;return{...h,responseMode:"compact",reviewTable:wd(o.rows,e.reviewTable||{}),evidenceRows:i.rows,summary:{...e.summary||{},compactResponse:!0,reviewRowCount:o.totalCount,returnedReviewRowCount:o.returnedCount,omittedReviewRowCount:o.omittedCount,duplicateReviewRowCount:o.duplicateCount,evidenceRowCount:i.totalCount,returnedEvidenceRowCount:i.returnedCount,omittedEvidenceRowCount:i.omittedCount},notices:[...at(e.notices),'Compact response returns summary, reviewTable, evidenceRows, and count metadata only. Use responseMode="full" for reviewRows, token profiles, raw cells, and nested candidates.']}}async function vd(e,t={}){let n=it.safeParse(e);if(!n.success)return Fr("input_validation","reconciliation_input_required","Provide excel and schedule sources before reconciliation.",{validationIssues:n.error.issues.map(l=>`${l.path.join(".")||"<root>"}: ${l.message}`),requiredColumnMapping:yd,schemaExamples:bd,suggestedNextScopes:["excel.kind","excel.rows","excel.path","excel.selection","excel.columnMapping.identity","excel.columnMapping.comparisonText","schedule.kind","schedule.result","schedule.columnMapping.identity","schedule.columnMapping.comparisonText"]});let r=await Oi(n.data.excel);if(Yi(r))return Fr("excel_ingestion",r.reason||"excel_ingestion_guarded",r.message||"Excel ingestion was guarded before reconciliation.",{excelResult:r,summary:r.summary||{},scanPolicy:r.scanPolicy||{},suggestedNextScopes:r.suggestedNextScopes||["excel.selection","excel.columnMapping.identity","excel.columnMapping.comparisonText"],warnings:r.warnings||[],notices:r.notices||[]});if(Ki(r))return Lr("excel_ingestion",r.error||"Excel ingestion failed before reconciliation.",{excelResult:r,summary:r.summary||{},scanPolicy:r.scanPolicy||{},suggestedNextScopes:r.suggestedNextScopes||["excel.selection","excel.columnMapping.identity","excel.columnMapping.comparisonText"],warnings:r.warnings||[],notices:r.notices||[]});let o=await ji(n.data.schedule,t.scheduleAdapter);if(Yi(o))return Fr("schedule_record_adapter",o.reason||"schedule_adapter_guarded",o.message||"Schedule adaptation was guarded before reconciliation.",{scheduleResult:o,summary:o.summary||{},scanPolicy:o.scanPolicy||{},suggestedNextScopes:o.suggestedNextScopes||["schedule.result","schedule.columnMapping.identity","schedule.columnMapping.comparisonText"],warnings:o.warnings||[],notices:o.notices||[]});if(Ki(o))return Lr("schedule_record_adapter",o.error||"Schedule adaptation failed before reconciliation.",{scheduleResult:o,summary:o.summary||{},scanPolicy:o.scanPolicy||{},suggestedNextScopes:o.suggestedNextScopes||["schedule.result","schedule.columnMapping.identity","schedule.columnMapping.comparisonText"],warnings:o.warnings||[],notices:o.notices||[]});let i=Xi({excelResult:r,scheduleResult:o,config:n.data.config}),a=r.partial===!0||o.partial===!0,s=a&&gd(o,r)||i.scanStoppedReason;return xd({...i,partial:i.partial===!0||a,scanStoppedReason:s,scanPolicy:{...i.scanPolicy||{},excel:r.scanPolicy||{},schedule:o.scanPolicy||{}},warnings:[...at(i.warnings),...at(r.warnings),...at(o.warnings)],notices:[...at(i.notices),...at(r.notices),...at(o.notices)],sourceSummary:{excel:r.summary||{},schedule:o.summary||{}},sourceResults:{excel:{sourceKind:r.sourceKind,format:r.format,sheetName:r.sheetName,partial:r.partial,scanStoppedReason:r.scanStoppedReason,recordCount:Array.isArray(r.excelRecords)?r.excelRecords.length:0},schedule:{sourceKind:o.sourceKind,visibilityBasis:o.visibilityBasis,partial:o.partial,scanStoppedReason:o.scanStoppedReason,recordCount:Array.isArray(o.scheduleRecords)?o.scheduleRecords.length:0}}},n.data)}function ea(e){e.tool("reconcile_schedule_excel",'[SCHEDULE_EXCEL_RECONCILIATION_REVIEW_ONLY] Review-first/write-free schedule-to-Excel reconciliation. Ingests explicit Excel/CSV data plus either normalized inspect_schedules output or bounded live revit_schedule input, normalizes rows, scores deterministic matches, and returns compact review tables by default. excel.kind="rows" expects an object with rows:[...] plus columnMapping.identity and columnMapping.comparisonText; file sources use path/format/selection with the same required mapping. schedule.kind="revit_schedule" requires scheduleIds or nameQuery unless allowExpensiveSearch=true. schedule.columnHeaders can be an index-ordered string array, an array of {column, header} objects, or a header/index map; explicit headers override native header labels for string columnMapping resolution. Default responseMode=compact returns summary, reviewTable, evidenceRows, and count metadata only; use responseMode=full/debug for reviewRows, token profiles, raw cells, and nested candidateRows. Does not write Revit or workbook data; route any accepted follow-up write through set_schedule_cells or set_schedule_cells_by_text after human review.',{excel:it.shape.excel,schedule:it.shape.schedule,config:it.shape.config,responseMode:it.shape.responseMode,maxReviewRows:it.shape.maxReviewRows,maxCandidateRows:it.shape.maxCandidateRows},async(t={})=>{try{return p(await vd(t))}catch(n){return p(Lr("runtime_failure",n instanceof Error?n.message:String(n)))}})}import{z as M}from"zod";var Cd={fast:{maxElapsedMs:4500,timeoutMs:12e3,maxMatches:1e3},balanced:{maxElapsedMs:15e3,timeoutMs:3e4,maxMatches:5e3},deep:{maxElapsedMs:45e3,timeoutMs:6e4,maxMatches:2e4}},zn=["sheetQuery","sheetIds","viewNameQuery","sources","profiles","countMode","groupBy","maxSheets","maxViewports","maxMatches","maxResponseBytes","allowExpensiveSearch"];function ee(e,t,n,r){if(e==null||e==="")return t;let o=Number.parseInt(String(e??""),10);return Number.isFinite(o)?Math.max(n,Math.min(r,o)):t}function ta(e){let t=["fast","balanced","deep"].includes(String(e.searchBudget||""))?String(e.searchBudget):"fast",n=Cd[t],r=ee(e.maxElapsedMs,n.maxElapsedMs,1,119e3),o=ee(e.timeoutMs,Math.max(n.timeoutMs,Math.min(12e4,r+5e3)),1e3,12e4);return{searchBudget:t,maxElapsedMs:Math.min(r,Math.max(1,o-1e3)),timeoutMs:o,maxMatches:ee(e.maxMatches,n.maxMatches,1,2e5)}}function Td(e){let t=String(e??"").trim();return/^sheet_?text_?notes?$/i.test(t)||/^sheetTextNotes?$/i.test(t)?"sheet_text_notes":/^viewport_?tags?$/i.test(t)||/^viewportTags?$/i.test(t)?"viewport_tags":/^viewport_?text_?notes?$/i.test(t)||/^viewportTextNotes?$/i.test(t)||/^view_?text_?notes?$/i.test(t)||/^viewTextNotes?$/i.test(t)?"viewport_text_notes":/^placed_?schedule_?cells?$/i.test(t)||/^placedScheduleCells?$/i.test(t)||/^schedule_?cells?$/i.test(t)||/^scheduleCells?$/i.test(t)?"placed_schedule_cells":t}function st(e){let t=String(e??"").trim();return/^unique_?text$/i.test(t)?"uniqueText":/^unique_?tag$/i.test(t)?"uniqueTag":/^unique_?tagged_?element$/i.test(t)?"uniqueTaggedElement":"occurrence"}function na(e){return e==="uniqueTag"||e==="uniqueTaggedElement"}function Bn(e,t,n,r){return e==="deep"?r:e==="balanced"?n:t}function jr(e){let t=st(e.countMode),n=Array.isArray(e.sources)?e.sources:[],r=[...new Set(n.map(Td).filter(o=>o.length>0))];return r.length>0?r:na(t)?["viewport_tags"]:["sheet_text_notes","viewport_text_notes","placed_schedule_cells","viewport_tags"]}function Rd(e){return Array.isArray(e.sources)&&e.sources.length>0}function ra(e){return!!(Array.isArray(e.sheetIds)&&e.sheetIds.length>0||String(e.sheetQuery||"").trim())}function Br(e){let t=ta(e);return{searchBudget:t.searchBudget,allowExpensiveSearch:e.allowExpensiveSearch===!0,sources:jr(e),countMode:st(e.countMode),groupBy:Array.isArray(e.groupBy)?e.groupBy:[],maxElapsedMs:t.maxElapsedMs,timeoutMs:t.timeoutMs,maxSheets:ee(e.maxSheets,30,1,200),maxViewportsPerSheet:ee(e.maxViewportsPerSheet??e.maxViewports,20,0,200),maxTextNotesScanned:ee(e.maxTextNotesScanned,Bn(t.searchBudget,1e3,5e3,2e4),1,2e5),maxTagsScanned:ee(e.maxTagsScanned??e.maxTags,Bn(t.searchBudget,500,2500,1e4),1,1e5),maxScheduleInstancesPerSheet:ee(e.maxScheduleInstancesPerSheet,20,0,200),maxRowsPerSchedule:ee(e.maxRowsPerSchedule,250,1,2e3),maxColumnsPerSchedule:ee(e.maxColumnsPerSchedule,20,1,200),maxScheduleInstancesScanned:ee(e.maxScheduleInstancesScanned,Bn(t.searchBudget,200,1e3,5e3),1,2e4),maxScheduleCellsScanned:ee(e.maxScheduleCellsScanned,Bn(t.searchBudget,1e3,5e3,2e4),1,2e5),maxMatches:t.maxMatches,maxTextChars:ee(e.maxTextChars,240,1,1e3),maxRegexPatternLength:ee(e.maxRegexPatternLength,240,1,1e3),regexTimeoutMs:ee(e.regexTimeoutMs,25,1,250),maxResponseBytes:ee(e.maxResponseBytes,4*1024*1024,4096,16*1024*1024),sheetScoped:ra(e)}}function Id(e,t){return{query:e.query,regex:e.regex,normalizedRegex:e.normalizedRegex,matchMode:e.matchMode,sheetQuery:e.sheetQuery,sheetIds:e.sheetIds,viewNameQuery:e.viewNameQuery,sources:jr(e),profiles:e.profiles,profileName:e.profileName,countMode:st(e.countMode),groupBy:e.groupBy,allowExpensiveSearch:e.allowExpensiveSearch,searchBudget:t.searchBudget,maxElapsedMs:t.maxElapsedMs,maxSheets:e.maxSheets,maxViewportsPerSheet:e.maxViewportsPerSheet,maxViewports:e.maxViewports,maxTextNotesScanned:e.maxTextNotesScanned,maxTagsScanned:e.maxTagsScanned,maxTags:e.maxTags,maxScheduleInstancesPerSheet:e.maxScheduleInstancesPerSheet,maxRowsPerSchedule:e.maxRowsPerSchedule,maxColumnsPerSchedule:e.maxColumnsPerSchedule,maxScheduleInstancesScanned:e.maxScheduleInstancesScanned,maxScheduleCellsScanned:e.maxScheduleCellsScanned,maxMatches:t.maxMatches,maxTextChars:e.maxTextChars,maxRegexPatternLength:e.maxRegexPatternLength,regexTimeoutMs:e.regexTimeoutMs,maxResponseBytes:e.maxResponseBytes,timeoutMs:t.timeoutMs,taskName:e.taskName||"Count Revit annotations",taskId:e.taskId}}function qn(e){let t=String(c(e,"sourceType")||""),n=String(c(e,"kind")||""),r=[t,n];return r.some(o=>o==="viewportTag"||o==="viewport_tags")?"viewportTag":r.some(o=>o==="viewportTextNote"||o==="viewport_text_notes")?"viewportTextNote":r.some(o=>o==="sheetTextNote"||o==="sheet_text_notes")?"sheetTextNote":r.some(o=>o==="placedScheduleCell"||o==="placed_schedule_cells"||o==="scheduleCell")?"placedScheduleCell":t||n||"annotation"}function Wn(e){let t=C(e,"evidenceRows");return(t.length>0?t:C(e,"matches")).map(r=>({...r,sourceType:qn(r)}))}function _d(e){let t=String(e??"").trim();return/^source_?type$/i.test(t)?"sourceType":/^(profile|profileName)$/i.test(t)?"profile":/^(pattern|patternName)$/i.test(t)?"pattern":/^(matchedCode|matchedText|uniqueText)$/i.test(t)?"matchedText":/^tagFamilyType$/i.test(t)?"tagFamilyType":/^(taggedElement|taggedElementId)$/i.test(t)?"taggedElement":/^view$/i.test(t)?"view":/^sheet$/i.test(t)?"sheet":t}function Md(e,t){let n={};if(t.length===0)return n.group="all",n;for(let r of t){let o=_d(r);o==="sheet"?(n.sheetId=c(e,"sheetId")??null,n.sheetNumber=c(e,"sheetNumber")??null):o==="view"?(n.viewId=c(e,"viewId")??null,n.viewName=c(e,"viewName")??null):o==="sourceType"?n.sourceType=qn(e):o==="profile"?n.profileName=c(e,"profileName")??null:o==="pattern"?n.patternName=c(e,"patternName")??null:o==="matchedText"?n.matchedTextNormalized=c(e,"matchedTextNormalized")??null:o==="tagFamilyType"?(n.tagFamilyName=c(e,"tagFamilyName")??null,n.tagTypeName=c(e,"tagTypeName")??null):o==="taggedElement"&&(n.taggedElementId=c(e,"taggedElementId")??null)}return Object.keys(n).length===0&&(n.group="all"),n}function Nd(e){return Object.keys(e).sort().map(t=>`${t}=${String(e[t]??"")}`).join("|")}function Ed(e,t){let n=qn(e);if(t==="occurrence")return"";if(t==="uniqueText")return`profile:${String(c(e,"profileName")??"").trim()}|text:${String(c(e,"matchedTextNormalized")??c(e,"textNormalized")??"").trim()}`;if(t==="uniqueTag"){if(n!=="viewportTag")return"";let r=String(c(e,"tagId")??"").trim();return r?`tag:${r}`:""}if(t==="uniqueTaggedElement"){if(n!=="viewportTag")return"";let r=c(e,"taggedElementResolved"),o=String(c(e,"taggedElementId")??"").trim();return!r||!o?"":`taggedElement:${o}`}return""}function oa(e,t,n){let r=new Map,o=new Set,i=0,a=0,s=e.map(l=>{let u={...l,sourceType:qn(l)},h=Md(u,n),m=Nd(h),g=r.get(m);g||(g={groupKey:m,...h,count:0,occurrenceCount:0,evidenceRowCount:0},r.set(m,g)),g.occurrenceCount+=1,g.evidenceRowCount+=1;let b=t==="occurrence"?`occurrence:${a++}`:Ed(u,t),y=!!b&&!o.has(`${m}||${b}`);return y&&(o.add(`${m}||${b}`),g.count+=1,i+=1),{...u,groupKey:m,countKey:b,counted:y,countMode:t}});return{count:i,evidenceRows:s,groups:[...r.values()].sort((l,u)=>String(l.groupKey).localeCompare(String(u.groupKey)))}}function ia(e,t){let n=bt(e,"scanPolicy"),r=c(n,"groupBy")??c(e,"groupBy")??t?.groupBy;return Array.isArray(r)?r.map(String):[]}function aa(e,t){return st(c(e,"countMode")??c(bt(e,"summary"),"countMode")??t?.countMode)}function sa(e,t){let n=Wn(e),r=aa(e,t),o=oa(n,r,ia(e,t));return{count:c(e,"count")??o.count,countMode:r,occurrenceCount:c(e,"matchedOccurrenceCount")??o.evidenceRows.length,matchCount:o.evidenceRows.length,evidenceRowCount:o.evidenceRows.length,groupCount:C(e,"groups").length||o.groups.length,scannedSheetCount:c(e,"scannedSheetCount")??null,scannedViewportCount:c(e,"scannedViewportCount")??null,scannedTextNoteCount:c(e,"scannedTextNoteCount")??null,scannedTagCount:c(e,"scannedTagCount")??null,scannedScheduleInstanceCount:c(e,"scannedScheduleInstanceCount")??null,scannedScheduleCellCount:c(e,"scannedScheduleCellCount")??null,partial:c(e,"partial")===!0,scanStoppedReason:c(e,"scanStoppedReason")??"completed"}}function kd(e){let t=Wn(e),n=t.length>0?t[t.length-1]:null;return{lastReadSection:c(e,"lastReadSection")??null,lastReadRow:c(e,"lastReadRow")??null,lastReadColumn:c(e,"lastReadColumn")??null,lastReadSheetId:c(n,"sheetId")??c(e,"lastReadSheetId")??null,lastReadViewId:c(n,"viewId")??c(e,"lastReadViewId")??null,lastReadViewportId:c(n,"viewportId")??c(e,"lastReadViewportId")??null,lastReadItemId:c(n,"tagId")??c(n,"elementId")??c(n,"scheduleInstanceId")??c(n,"scheduleId")??c(n,"id")??c(e,"lastReadItemId")??null}}function Pd(e,t){let n=aa(e,t),r=oa(Wn(e),n,ia(e,t)),o=C(e,"groups");return e.countMode=n,e.evidenceRows=r.evidenceRows,e.matches=C(e,"matches").length>0?C(e,"matches"):e.evidenceRows,e.groups=o.length>0?o:r.groups,e.count=c(e,"count")??c(e.summary,"count")??r.count,e.summary={...sa(e,t),...bt(e,"summary")||{},count:c(e.summary,"count")??e.count,countMode:n,matchCount:c(e.summary,"matchCount")??e.evidenceRows.length,groupCount:c(e.summary,"groupCount")??e.groups.length},e}function Ad(e,t={},n){return Pd(re(e,{action:"count_annotations",elapsedMs:n,scanPolicy:Br(t),summary:r=>sa(r,t),evidenceRows:Wn,lastRead:kd,suggestedNextScopes:zn}),t)}function Od(e,t){return de({action:"count_annotations",reason:"needs_scope",message:"Annotation counting can scan many sheets and placed views. Pass sheetQuery/sheetIds, or set allowExpensiveSearch=true with bounded caps.",suggestedNextScopes:zn,scanPolicy:Br({...e,maxElapsedMs:t.maxElapsedMs,timeoutMs:t.timeoutMs}),summary:{count:0,countMode:st(e.countMode),matchCount:0,groupCount:0}})}function Vd(e){return de({action:"count_annotations",reason:"invalid_count_mode_for_sources",message:"uniqueTag and uniqueTaggedElement count modes require viewport_tags as the only source. Omit sources to let the tool default to viewport_tags.",suggestedNextScopes:zn,scanPolicy:Br(e),summary:{count:0,countMode:st(e.countMode),matchCount:0,groupCount:0}})}function la(e){e.tool("count_annotations","[ANNOTATION_COUNT_READ_ONLY] Read-only native Revit annotation inventory/count for DrawingSheet text notes, viewport text notes, placed schedule cells, and viewport tag evidence. Use sheetQuery/sheetIds first; project-wide annotation counts require allowExpensiveSearch=true. Supports occurrence, uniqueText, uniqueTag, and uniqueTaggedElement count modes with bounded regex profiles.",{...S(M),...x(M),query:M.string().optional().describe("Anonymous text query. Defaults to contains matching unless matchMode is supplied."),regex:M.string().optional().describe("Anonymous raw regex pattern. Regex matching is bounded by maxRegexPatternLength and regexTimeoutMs."),normalizedRegex:M.string().optional().describe("Anonymous regex pattern evaluated against normalized annotation text."),matchMode:M.enum(["exact","contains","startsWith","regex","normalizedRegex"]).optional().describe("Match mode for query when using the anonymous profile."),profileName:M.string().optional().describe("Optional anonymous profile name when query/regex is used without profiles."),profiles:M.array(M.any()).optional().describe("Explicit profile objects with profileName/name and patterns. Patterns support exact, contains, startsWith, regex, and normalizedRegex."),sheetQuery:M.string().optional().describe("Sheet number/name scope. Use this first in large projects."),sheetIds:M.array(M.union([M.number(),M.string()])).optional().describe("Exact ViewSheet element ids to inspect. Preferred when known."),viewNameQuery:M.string().optional().describe("Optional placed-view name filter before viewport tag inspection."),sources:M.array(M.enum(["sheet_text_notes","viewport_text_notes","viewport_text_note","placed_schedule_cells","placed_schedule_cell","viewport_tags","sheetTextNotes","viewportTextNotes","viewportTextNote","view_text_notes","viewTextNotes","placedScheduleCells","placedScheduleCell","schedule_cells","schedule_cell","scheduleCells","scheduleCell","viewportTags"])).optional().describe("Annotation sources. Defaults to sheet_text_notes + viewport_text_notes + placed_schedule_cells + viewport_tags except tag-specific count modes, which default to viewport_tags."),countMode:M.enum(["occurrence","uniqueText","uniqueTag","uniqueTaggedElement"]).optional().describe("Count semantics. Tag-specific modes require viewport_tags as the only explicit source."),groupBy:M.array(M.enum(["sheet","view","sourceType","profile","profileName","pattern","patternName","matchedText","matchedCode","tagFamilyType","taggedElement","taggedElementId"])).optional().describe("Optional grouping dimensions for count rows."),allowExpensiveSearch:M.boolean().optional().describe("Explicit approval for project-wide sheet and placed-view annotation counting without sheetIds/sheetQuery. Defaults false."),searchBudget:M.enum(["fast","balanced","deep"]).optional().describe("Native Revit-side scan budget preset. fast is default; deep still respects maxElapsedMs and response-size caps."),maxElapsedMs:M.number().int().positive().max(119e3).optional().describe("Native Revit-side elapsed budget. It is clamped below timeoutMs so partial results can return before transport timeout."),maxSheets:M.number().int().positive().max(200).optional().describe("Maximum matching sheets to inspect. Defaults 30."),maxViewportsPerSheet:M.number().int().min(0).max(200).optional().describe("Maximum placed viewports inspected per sheet. Defaults 20."),maxViewports:M.number().int().min(0).max(200).optional().describe("Alias for maxViewportsPerSheet."),maxTextNotesScanned:M.number().int().positive().max(2e5).optional().describe("Global native cap across sheet text notes."),maxScheduleInstancesPerSheet:M.number().int().min(0).max(200).optional().describe("Maximum placed schedule instances inspected per sheet. Defaults 20."),maxRowsPerSchedule:M.number().int().positive().max(2e3).optional().describe("Maximum body rows scanned per placed schedule. Defaults 250."),maxColumnsPerSchedule:M.number().int().positive().max(200).optional().describe("Maximum body columns scanned per placed schedule. Defaults 20."),maxScheduleInstancesScanned:M.number().int().positive().max(2e4).optional().describe("Global native cap across placed schedule instances."),maxScheduleCellsScanned:M.number().int().positive().max(2e5).optional().describe("Global native cap across placed schedule body cells before scanStoppedReason=max_cells."),maxTags:M.number().int().positive().max(1e5).optional().describe("Alias for maxTagsScanned. Global native cap across viewport tags."),maxTagsScanned:M.number().int().positive().max(1e5).optional().describe("Global native cap across viewport tags."),maxMatches:M.number().int().positive().max(2e5).optional().describe("Maximum returned matching evidence rows before scanStoppedReason=max_items."),maxTextChars:M.number().int().min(1).max(1e3).optional().describe("Maximum characters retained and matched per annotation candidate. Defaults 240."),maxRegexPatternLength:M.number().int().min(1).max(1e3).optional().describe("Maximum regex pattern length. Defaults 240."),regexTimeoutMs:M.number().int().min(1).max(250).optional().describe("Per-candidate regex timeout in milliseconds. Defaults 25."),maxResponseBytes:M.number().int().min(4096).max(16*1024*1024).optional().describe("Advanced response-size budget. The native handler stops with scanStoppedReason=max_bytes before the bridge response becomes too large."),timeoutMs:M.number().int().positive().max(12e4).optional().describe("Socket timeout in milliseconds. Defaults from searchBudget with headroom above maxElapsedMs.")},async t=>{let n=Date.now();try{let r=ta(t),o=jr(t),i=st(t.countMode);if(na(i)&&Rd(t)&&o.some(s=>s!=="viewport_tags"))return p(Vd(t));if(!ra(t)&&t.allowExpensiveSearch!==!0)return p(Od(t,r));let a=await _("count_annotations",Id(t,r),{...I({...t,timeoutMs:r.timeoutMs},"Count Revit annotations"),toolName:"count_annotations"});return p(Ad(a&&a.result?a.result:a,t,Date.now()-n))}catch(r){return p(fe({action:"count_annotations",error:r instanceof Error?r.message:String(r),elapsedMs:Date.now()-n,suggestedNextScopes:zn}))}})}import{z as ge}from"zod";function Dd(e){let t=Sn(e.elementIds||[]),n=R(e.category||""),r=Number.isFinite(e.sampleLimit)?Math.max(1,Math.min(25,e.sampleLimit)):5,o=e.includeTypeParameters===!0?"true":"false",i=we(e.parameterNameFilter||[]),a=e.parameterNameMatchMode==="exact"?"exact":"contains";return`
...
{
    return new { success = false, error = ex.ToString() };
}`}function Fd(e){return!e||typeof e!="object"?{}:{source:e.source,displayBuiltInParameter:e.displayBuiltInParameter,builtInParameterId:e.builtInParameterId,rawBuiltInParameterAlias:e.rawBuiltInParameterAlias,storageType:e.storageType,isShared:e.isShared,isReadOnly:e.isReadOnly,dataType:e.dataType,unitType:e.unitType,noValueState:e.noValueState,clearability:e.clearability}}function Ld(e,t){if(t.parameterNameMatchMode!=="exact"||!e||typeof e!="object"||!Array.isArray(e.elements))return e;let n=[],r=Array.isArray(e.warnings)?[...e.warnings]:[];for(let o of e.elements){let i=Array.isArray(o?.parameters)?o.parameters:[],a=new Map;for(let s of i){let l=typeof s?.name=="string"?s.name.trim():"";if(!l)continue;let u=l.toLocaleLowerCase("en-US");a.has(u)||a.set(u,{name:l,matches:[]}),a.get(u)?.matches.push(s)}for(let s of a.values()){if(s.matches.length<2)continue;let l={elementId:o?.id,parameterName:s.name,count:s.matches.length,severity:"write_preflight_warning",message:`Duplicate display name '${s.name}' matched ${s.matches.length} parameters on element ${o?.id}. Display name alone is ambiguous for write-back; choose by source, builtInParameterId, shared flag, storage type, or read-only state.`,matches:s.matches.map(Fd)};n.push(l),r.push(`duplicate_display_name: elementId=${o?.id}; parameterName=${s.name}; count=${s.matches.length}; display name alone is ambiguous for write-back.`)}}return n.length===0?e:{...e,warnings:r,duplicateDisplayNameWarnings:n}}function ca(e){e.tool("inspect_parameter_schema","Read-only parameter schema inspection for selected ids or a category sample: user-facing BIP display label/id, raw enum alias, storage type, unit type, shared/read-only flags, raw/display values, no-value state, and clearability metadata.",{...S(ge),...x(ge),elementIds:ge.array(ge.union([ge.number(),ge.string()])).optional().describe("Element ids to inspect."),category:ge.string().optional().describe("BuiltInCategory name such as OST_DuctCurves or OST_DuctTerminal."),sampleLimit:ge.number().int().positive().max(25).optional().describe("Maximum sample elements. Defaults 5."),includeTypeParameters:ge.boolean().optional().describe("Include type parameters. Defaults false."),parameterNameFilter:ge.array(ge.string()).optional().describe("Optional parameter name filters."),parameterNameMatchMode:ge.enum(["contains","exact"]).optional().describe("Filter matching mode. contains is discovery mode and default; exact is write-preflight mode.")},async t=>{if((!t.elementIds||t.elementIds.length===0)&&!t.category)return p({success:!0,matchMode:t.parameterNameMatchMode==="exact"?"exact":"contains",sampleCount:0,elements:[],warnings:["Provide elementIds or category."]});try{let n=await H(Dd(t),{...I(t,"Inspect Revit parameter schema"),transactionMode:"none"}),r=n&&n.result?n.result:n;return p(Ld(r,t))}catch(n){return p({success:!1,error:n instanceof Error?n.message:String(n)})}})}import{z as W}from"zod";function da(e){return e==="clear"?"clear":e==="clearVisibleValue"?"clearVisibleValue":"set"}function ua(e){return typeof e=="boolean"?e?"true":"false":String(e??"")}async function jd(e,t){if(e.elementId!==void 0&&e.elementId!==null&&String(e.elementId).trim()!==""){let n=Number.parseInt(String(e.elementId),10);return Number.isFinite(n)&&n>0?n:null}if(e.useSelection===!0){let n=await pt(2,t);return n.length===1?n[0]:{...Ce({action:"set_element_parameter",reason:"single_selection_required",error:n.length===0?"No selected Revit element was found. Provide elementId or select exactly one element.":"Multiple selected elements were found. Provide one explicit elementId for a production parameter write."}),tool:"set_element_parameter",guardReason:"single_selection_required",selectedElementIds:n}}return null}function Bd(e,t){let n=da(e.operation),r=R(e.parameterName||""),o=R(e.parameterSource||"instance"),i=R(n==="clearVisibleValue"?"":ua(e.value)),a=R(e.valueMode||"raw"),s=R(e.mode==="commit"?"commit":"dryRun"),l=R(n),u=e.value===void 0||e.value===null?"false":"true",h=Number.isInteger(e.builtInParameterId)?String(e.builtInParameterId):"null",m=R(e.expectedStorageType||""),g=R(e.expectedCurrentRaw===void 0||e.expectedCurrentRaw===null?"":ua(e.expectedCurrentRaw)),b=e.expectedCurrentRaw===void 0||e.expectedCurrentRaw===null?"false":"true",y=e.allowTypeParameterWrite===!0?"true":"false";return`
int elementId = ${t};
...
{
    return new { success = false, error = ex.ToString() };
}`;function Xd(e){let t=ke(e);return t&&typeof t=="object"&&t.result?t.result:t}async function Yd(e,t){let n=null;try{n=await be(async o=>await o.sendCommand("mcp_status",{},{timeoutMs:t,statusPreflight:!1}),{host:e.host,port:e.port,connectTimeoutMs:t,lockWaitMs:Math.max(t,500),logSocketErrors:!1,skipLock:!0})}catch(o){return{reachable:!1,target:{name:e.name,host:e.host,port:e.port,source:e.source},error:o instanceof Error?o.message:String(o)}}let r=Math.max(t,1e4);try{let o=await be(async(i,a)=>await i.sendCommand("send_code_to_revit",{code:Qd,parameters:[`${a.host}:${a.port}`],transactionMode:"none",taskName:"Probe Revit instance"},{timeoutMs:r}),{host:e.host,port:e.port,connectTimeoutMs:t,lockWaitMs:Math.max(r,500),logSocketErrors:!1});return{reachable:!0,target:{name:e.name,host:e.host,port:e.port,source:e.source},status:Bt(n,{recentLimit:3,includeDiagnostics:!1}),info:Xd(o)}}catch(o){return{reachable:!0,target:{name:e.name,host:e.host,port:e.port,source:e.source},status:Bt(n,{recentLimit:3,includeDiagnostics:!1}),info:null,infoError:o instanceof Error?o.message:String(o)}}}function ba(e){e.tool("list_revit_instances","Discover reachable revAgent Revit bridge instances by probing configured ports. Use this before targeting a specific Revit instance.",{host:Ge.string().optional().describe("Host to scan. Defaults to REVIT_MCP_HOST or localhost."),ports:Ge.array(Ge.union([Ge.number(),Ge.string()])).optional().describe("Ports to scan. Defaults to REVIT_MCP_PORTS, or 8080-8085."),includeRegistry:Ge.boolean().optional().describe("Include targets from the revAgent instance registry file. Defaults true."),includeUnreachable:Ge.boolean().optional().describe("Include unreachable ports in the result. Defaults false."),timeoutMs:Ge.number().int().positive().max(15e3).optional().describe("Per-port connection timeout in milliseconds. Defaults 3000.")},async t=>{let n=t.timeoutMs||3e3,r=Kr({host:t.host,ports:t.ports,includeRegistry:t.includeRegistry}),o=[];for(let i of r){let a=await Yd(i,n);(a.reachable||t.includeUnreachable)&&o.push(a)}return p({success:!0,count:o.filter(i=>i.reachable).length,scanned:r.length,instances:o})})}import wa from"node:path";import{z as Je}from"zod";var Kd=new Date().toISOString(),Zd="revit-mcp-status.v3",em="revit-mcp-runtime-tools.38";function tm(){let e=ct(wa.join(Hn(),"package.json"));return{packageName:e?.name||"revit-mcp",packageVersion:e?.version||"unknown"}}function Sa(){let e=tm(),t=kt([wa.join(process.cwd(),"..","updater","installed.json")]),n=t?.version||e.packageVersion;return{runtimeVersion:n,schemaVersion:Zd,toolSurfaceVersion:em,processStartedAtUtc:Kd,buildTimestampUtc:t?.installedAtUtc||null,buildHash:Pt(n),packageName:e.packageName,packageVersion:e.packageVersion,nodeVersion:process.version}}function xa(e){e.tool("get_revit_mcp_status","Read the revAgent task status without waiting behind the active Revit command lock. Includes runtimeVersion, schemaVersion, toolSurfaceVersion, processStartedAtUtc, buildTimestampUtc, buildHash, bridge resultContractVersion when available, and summary runtimeActivity for revAgent-side/client-side guarded operations that may not reach Revit.",{...S(Je),includeRecentTasks:Je.boolean().optional().describe("Include recent completed task records. Defaults true, with a compact limit."),recentLimit:Je.number().int().min(0).max(100).optional().describe("Maximum recent task records to return when includeRecentTasks is true. Defaults 3."),includeRuntimeActivity:Je.boolean().optional().describe("Include MCP-side/client-side active and recent activity. Defaults true so guard-only tasks that did not reach Revit remain auditable."),runtimeActivityLimit:Je.number().int().min(0).max(100).optional().describe("Maximum runtimeActivity.recentActivity rows to return. Defaults 10."),runtimeActivityMode:Je.enum(["summary","full"]).optional().describe("runtimeActivity shape. summary is the default and collapses started/completed pairs into latest completed/guarded/failed rows without responseKeys. full includes started rows and full result summaries."),includeDiagnostics:Je.boolean().optional().describe("Include transport timing/byte diagnostics on task records. Defaults false."),timeoutMs:Je.number().int().positive().max(1e4).optional().describe("Connection timeout in milliseconds. Defaults 3000.")},async t=>{let n=t.includeRuntimeActivity===!1?void 0:Io(t.runtimeActivityLimit??10,t.runtimeActivityMode||"summary");try{let r=t.timeoutMs||3e3,o=await be(async s=>await s.sendCommand("mcp_status",{},{timeoutMs:r}),{...Y(t),skipLock:!0,connectTimeoutMs:r}),i=Bt(ke(o),{includeRecentTasks:t.includeRecentTasks,recentLimit:t.recentLimit,includeDiagnostics:t.includeDiagnostics});gn(o);let a=i&&typeof i=="object"&&!Array.isArray(i)?i:{status:i};return p({...a,...n?{runtimeActivity:n}:{},runtimeIdentity:Sa()})}catch(r){return p({success:!1,error:r instanceof Error?r.message:String(r),...n?{runtimeActivity:n}:{},runtimeIdentity:Sa()})}})}async function va(e){let t=Eo(e);ba(t),xa(t),jo(t),Bo(t),zo(t),qo(t),Go(t),Jo(t),Ho(t),Uo(t),$o(t),Qo(t),ti(t),ni(t),ri(t),oi(t),ii(t),ci(t),si(t),ui(t),pi(t),hi(t),Si(t),Ti(t),ea(t),la(t),ca(t),ma(t),ha(t),ya(t),console.error("Registered 30 revAgent tools")}var Ca=new nm({name:"revAgent",version:"1.0.0"});async function om(){await va(Ca);let e=new rm;await Ca.connect(e),No(),console.error("revAgent runtime start success")}om().catch(e=>{console.error("Error starting revAgent runtime:",e),process.exit(1)});
